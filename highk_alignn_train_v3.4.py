...
class DatasetExtractor:
    ...
    def pull_materials_project(
        ...
        with MPRester(MP_API_KEY) as mpr:
            ...
            # -- Summary batch lookup: get structure + Ef + Eg for dielectric entries --
            # The DielectricDoc has no structure field; structure lives on SummaryDoc.
            # This is the critical step: without a successful summary_doc lookup,
            # _parse_mp_entry sets has_structure=False and the entry is excluded
            # from Tier 2's has_structure==True filter.
            dielectric_mp_ids = [doc.material_id for doc in docs_dielectric]
            summary_data_map  = {}

            if dielectric_mp_ids:
                log.info("  Fetching summary data for %d dielectric IDs...",
                         len(dielectric_mp_ids))
                SUMMARY_BATCH = 500   # smaller batch → more reliable API responses
                for i in range(0, len(dielectric_mp_ids), SUMMARY_BATCH):
                    batch_ids = dielectric_mp_ids[i:i + SUMMARY_BATCH]
                    for attempt in range(3):         # up to 3 retries per batch
                        try:
                            docs_s = mpr.materials.summary.search(
                            ...
                            for s in docs_s:
                                summary_data_map[s.material_id] = s
                            ...
                            if attempt < 2:
                                ...
                                wait = 2 ** attempt
                                log.warning(
                                    "Summary batch %d attempt %d failed (%s)"
                                    " -- retrying in %ds", i, attempt + 1, exc, wait
                                )
                                time.sleep(wait)
                            else:
                                log.warning(
                                    "Summary batch %d failed after 3 attempts: %s"
                                    " -- affected entries will have has_structure=False",
                                    i, exc
                                )

                n_found   = len(summary_data_map)
                n_missing = len(dielectric_mp_ids) - n_found
                log.info(
                    "  Summary lookup result: %d/%d IDs found in summary DB"
                    "  (%d missing -- will attempt structures fallback)",
                    n_found, len(dielectric_mp_ids), n_missing,
                )

                # Fallback: for IDs not found via summary search, query the
                # structures endpoint directly.  This ensures has_structure=True
                # for these entries so they are NOT excluded from Tier 2.
                # Uses types.SimpleNamespace as a lightweight stand-in for SummaryDoc.
                if n_missing > 0:
                    import types as _types
                    missing_ids = [m for m in dielectric_mp_ids
                                   if m not in summary_data_map]
                    log.info("  Structures fallback for %d missing IDs...", n_missing)
                    STRUCT_BATCH = 200
                    n_recovered  = 0
                    for j in range(0, len(missing_ids), STRUCT_BATCH):
                        s_batch = missing_ids[j:j + STRUCT_BATCH]
                        for attempt in range(3):
                            try:
                                struct_docs = mpr.materials.structures.search(
                                    material_ids=s_batch,
                                    fields=["material_id", "structure"],
                                )
                                for sd in struct_docs:
                                    if sd.material_id not in summary_data_map:
                                        summary_data_map[sd.material_id] = \
                                ...
                                if attempt < 2:
                                    ...
                                    log.warning(
                                        "Structures fallback batch %d failed: %s", j, exc2
                                    )

                    n_still_missing = len(dielectric_mp_ids) - len(summary_data_map)
                    log.info(
                        "  Structures fallback: recovered %d  |  "
                        "still missing after fallback: %d",
                        n_recovered, n_still_missing,
                    )

            log.info(
                "  Final summary_data_map coverage: %d/%d  (%.1f%%)",
                len(summary_data_map), len(dielectric_mp_ids),
                100.0 * len(summary_data_map) / max(len(dielectric_mp_ids), 1),
            )

            # -- Diagnostic counters for k_total + has_structure audit -----
            n_parsed         = 0
            n_e_total_set    = 0
            n_k_total_set    = 0
            n_k_above_10     = 0
            n_has_structure  = 0   # NEW: tracks how many end up with valid structure
            n_no_summary     = 0   # entries where summary_doc was None
            _sample_etotal   = []
            ...
            for doc in tqdm(docs_dielectric, desc="Parsing MP dielectric entries"):
                ...
                if row is not None:
                    ...
                    if row.get("k_total") is not None:
                        ...
                        if row["k_total"] > 10:
                            n_k_above_10 += 1
                    if row.get("has_structure"):      # NEW
                        n_has_structure += 1

            log.info(
                "MP dielectric parse audit:"
                "\n  docs returned by search    : %d"
                "\n  docs with e_total present  : %d  (%.1f%%)"
                "\n  docs with no summary_doc   : %d  (these get has_structure=False)"
                ...
                "\n  ── if has_structure << k_total, structure fallback did not work ──",
                len(docs_dielectric),
                n_e_total_set,
            ...
            if _sample_etotal:
                log.info(
                    "MP e_total sample (first 5 docs):\n  %s",
                    "\n  ".join(_sample_etotal),
                )
