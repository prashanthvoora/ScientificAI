...
class ALIGNNTrainer:
    ...
    def train_epoch(
        ...
        """
        _frozen_ablation = bool(
            self.ablate_context
            and self.cfg.get("bounded_process_residual", False)
            and not self.cfg.get("process_delta_train_base_head", False)
        )
        if _frozen_ablation:
            # v4.41: a no-gradient ablation must be fully deterministic. Keeping
            # the frozen model in train() can still update normalization buffers
            # or activate dropout, changing validation metrics despite zero
            # optimizer steps.
            self.model.eval()
        else:
            self.model.train()
    ...
    def train(
        ...
        for epoch in range(1, n_epochs + 1):
            ...
            log.info(
                ...
                "delta_param=%.3e delta_change=%.3e  %s",
