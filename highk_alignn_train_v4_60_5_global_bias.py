...
TIER3_TRAIN_CONFIG = dict(
    ...
    # v4.60.5 GLOBAL-BIAS: single trainable scalar added to k_total_log,
    # exempt from center penalties and weight decay. Gives the systematic
    # DFT->experiment log offset a sanctioned home instead of forcing it into
    # the bounded process head (tanh saturation) or the structural adapter.
    tier3_use_global_bias          = True,
    tier3_global_bias_init         = 0.0,     # initial value of the scalar (log units)
    tier3_global_bias_learning_rate = 1e-3,   # own group; offset converges fast
...
class HighKALIGNN(nn.Module):
    ...
    def __init__(
        ...
        # v4.60.5 GLOBAL-BIAS: single scalar offset on k_total_log. Always
        # registered (so state_dict shape is stable across on/off runs) but
        # applied and trained only when use_tier3_global_bias is True. Disabled
        # by default here; configure_global_bias() wires it from cfg/CLI.
        self.tier3_global_bias = nn.Parameter(torch.zeros(1))
        self.use_tier3_global_bias = False
        self._last_global_bias_value = 0.0
        # v4.60.5: forward-time ablation flag. Set by freeze/train-mode helpers.
        # When True, both the global bias and (by existing design) the trainable
        # residual contribute nothing, keeping the DFT-only baseline constant.
        self._ablate_context = False
    ...
    def freeze_for_bounded_process_residual(self, train_base_head: bool = False):
        ...
        if self.use_tier3_structural_adapter:
            for p in self.tier3_structural_adapter.parameters():
                p.requires_grad = True
        # v4.60.5: global bias trains only when enabled. (Ablation additionally
        # forces it off before this call — see run_tier3_finetune wiring.)
        self.tier3_global_bias.requires_grad = bool(self.use_tier3_global_bias)
        object.__setattr__(self, "_tier3_base_head_reference", None)
    ...
    def set_residual_only_train_mode(self, ablate_context: bool = False):
        ...
        """
        self.eval()
        self._ablate_context = bool(ablate_context)
    ...
    def configure_global_bias(self, enabled: bool = True, init: float = 0.0,
                              reset: bool = False):
        """v4.60.5 Configure the single global-bias scalar on k_total_log.

        ``reset=True`` (fresh finetune) sets the scalar to ``init``. Evaluation /
        checkpoint restore uses reset=False so the learned value survives
        state_dict loading. When disabled, the parameter is frozen at its current
        value and excluded from the forward sum (see _global_bias_value / forward).
        """
        self.use_tier3_global_bias = bool(enabled)
        if reset:
            with torch.no_grad():
                self.tier3_global_bias.fill_(float(init))
        # requires_grad follows enablement; freeze method re-affirms this too.
        self.tier3_global_bias.requires_grad = bool(enabled)
        log.info(
            "v4.60.5 GLOBAL-BIAS: enabled=%s init=%.4f reset=%s current=%.6f",
            enabled, float(init), reset, float(self.tier3_global_bias.detach().item()),
        )

    def _global_bias_value(self, ablate_context: bool = False) -> torch.Tensor:
        """Return the scalar to add to k_total_log (0 when off or ablated).

        Gated identically to the adapter: contributes nothing under
        ``ablate_context`` so the DFT-only frozen baseline stays a true constant
        baseline with no trainable global offset.
        """
        if (not self.use_tier3_global_bias) or ablate_context:
            self._last_global_bias_value = 0.0
            return self.tier3_global_bias.new_zeros(())
        self._last_global_bias_value = float(self.tier3_global_bias.detach().item())
        return self.tier3_global_bias.reshape(())
    ...
    def forward(
        ...
        if self.use_bounded_process_residual:
            ...
            if task == "__all__":
                ...
                for t, head in self.task_heads.items():
                    ...
                    if t == "k_total_log":
                        ...
                        gbias = self._global_bias_value(self._ablate_context)
                        out[t] = base + structural_delta + gbias + (delta if delta is not None else 0.0)
                    else:
                        out[t] = base
                return out
            ...
            if task == "k_total_log":
                structural_delta = self._structural_adapter_delta(embedding)
                gbias = self._global_bias_value(self._ablate_context)
                return base + structural_delta + gbias + (delta if delta is not None else 0.0)
            return base
...
class ALIGNNTrainer:
    ...
    def rebuild_optimizer_for_current_trainables(self):
        ...
        # v4.60.5: the global bias gets its OWN group (wd=0). Exclude it from the
        # process group so a calibration offset is never decayed toward zero.
        gbias = self.model_core.tier3_global_bias
        gbias_params = [gbias] if gbias.requires_grad else []
        gbias_ids = {id(p) for p in gbias_params}
        process_params = [p for p in self.model_core.parameters()
                          if p.requires_grad and id(p) not in adapter_ids and id(p) not in gbias_ids]
        groups = []
        if process_params:
            groups.append({
        ...
        if gbias_params:
            groups.append({
                "params": gbias_params,
                "lr": float(self.cfg.get("tier3_global_bias_learning_rate", 1e-3)),
                "weight_decay": 0.0,
                "group_name": "global_bias",
            })
        if adapter_params:
            groups.append({
    ...
    def save_checkpoint(self, epoch: int, val_mae: float, tag: str = "best"):
        ...
        torch.save({
            ...
            "adapter_runtime": {
                "enabled": bool(self.model_core.use_tier3_structural_adapter),
                "rank": int(self.model_core.tier3_structural_adapter_rank),
                "global_bias_enabled": bool(self.model_core.use_tier3_global_bias),
                "global_bias_value": float(self.model_core.tier3_global_bias.detach().item()),
            },
        }, tmp_path)
...
def run_tier3_finetune(
    ...
    # v4.60.5 GLOBAL-BIAS wiring: honor cfg/CLI, and force OFF under ablation so
    # the DFT-only baseline never trains a global offset (keeps the ablation
    # ceiling a true constant baseline). This is the wiring the earlier branch
    # was missing (flag defined but never applied to the model).
    _gbias_enabled = bool(cfg.get("tier3_use_global_bias", True)) and not ablate_context
    model.configure_global_bias(
        enabled=_gbias_enabled,
        init=float(cfg.get("tier3_global_bias_init", 0.0)),
        reset=True,
    )
    log.info("v4.60.5 GLOBAL-BIAS finetune wiring: cfg=%s ablate=%s -> enabled=%s",
             cfg.get("tier3_use_global_bias", True), ablate_context, _gbias_enabled)
    ...
    def _write_tier3_prediction_audit(
        ...
        with torch.no_grad():
            for batch in loader:
                ...
                # -------------------------------------------------------------
                # v4.60.4 AUDIT-ADAPTER-FIX
                # The hand-reconstructed pred below historically omitted the
                # structural adapter delta (added to the real forward in v4.58),
                # so k_pred/k_pred_log in this CSV were the NO-ADAPTER prediction
                # while the headline MAD:MAE was scored on the full forward
                # (base + adapter + process). That made every per-row error,
                # residual table, and any bootstrap CI computed off this file
                # describe a different model than the one being reported.
                #
                # Fix: (1) compute the adapter delta from the SAME `embedding`
                # the real forward uses (backbone -> conditional dropout ->
                # func-conditioning; see MultiTaskALIGNN.forward k_total_log
                # branch), and (2) source the AUTHORITATIVE prediction straight
                # from forward_all_tasks so the CSV pred is bit-identical to the
                # scored headline. The manual decomposition is retained only for
                # diagnostics, and a parity guard hard-fails if the two diverge
                # (this is what catches the NEXT component someone adds to
                # forward() but forgets to mirror here).
                # -------------------------------------------------------------
                structural_adapter_delta = torch.zeros_like(base_log)
                if (
                    getattr(model_core, "use_tier3_structural_adapter", False)
                    and not trainer.ablate_context
                ):
                    _sd = model_core._structural_adapter_delta(embedding)
                    structural_adapter_delta = _sd.detach().cpu().view(-1)

                # v4.60.5: include the global-bias scalar in the reconstruction so
                # the parity guard matches forward_all_tasks (which adds it).
                _gb = 0.0
                if (
                    getattr(model_core, "use_tier3_global_bias", False)
                    and not trainer.ablate_context
                ):
                    _gb = float(model_core.tier3_global_bias.detach().item())
                global_bias_value = torch.full_like(base_log, _gb)

                # Reconstructed prediction (base + adapter + global_bias + process/family).
                pred_log_recon = base_log + structural_adapter_delta + global_bias_value + delta_log

                # Authoritative prediction from the exact scored code path.
                _auth = model_core.forward_all_tasks(
                    graph, line_graph,
                    proc_context=proc_ctx,
                    stack_context=stack_ctx,
                    functional_code=func_code,
                )
                pred_log = _auth[task_name].detach().cpu().view(-1)

                # Parity guard: reconstruction must match the scored forward.
                # Tolerance is loose enough for float noise, tight enough to
                # catch a missing/extra term (a dropped adapter is ~0.1-0.5 in
                # log space, far above this).
                _pl = min(len(pred_log), len(pred_log_recon))
                if _pl > 0:
                    _max_gap = float(
                        (pred_log[:_pl] - pred_log_recon[:_pl]).abs().max().item()
                    )
                    if _max_gap > 1e-4:
                        raise RuntimeError(
                            "v4.60.4 AUDIT-PARITY FAIL: reconstructed pred_log "
                            f"diverges from forward_all_tasks by {_max_gap:.3e} "
                            f"(>1e-4) on split={split_name} stage={stage_name}. "
                            "A forward() component is not mirrored in the audit "
                            "writer. Do NOT trust this CSV until reconciled."
                        )
                ...
                for j, row_idx in enumerate(row_indices):
                    ...
                    rows.append({
                        ...
                        # v4.60.4 AUDIT-ADAPTER-FIX: expose the structural adapter
                        # contribution per row so base/adapter/process/family
                        # decompose the full prediction exactly.
                        "structural_adapter_delta": float(structural_adapter_delta[j].item()) if j < len(structural_adapter_delta) else 0.0,
                        # v4.60.5: per-row global-bias contribution (constant across
                        # rows when active; 0 when disabled/ablated).
                        "global_bias_delta": float(global_bias_value[j].item()) if j < len(global_bias_value) else 0.0,
...
def run_tier_evaluate(
    ...
    if tier == 3:
        ...
        # v4.60.5: restore global-bias enablement from checkpoint metadata so the
        # eval forward applies exactly the trained runtime (reset=False keeps the
        # learned scalar, which arrives via state_dict load below).
        _gbias_enabled_eval = bool(_saved_runtime.get(
            "global_bias_enabled",
            _saved_cfg.get("tier3_use_global_bias", cfg.get("tier3_use_global_bias", True))))
        model.configure_global_bias(enabled=_gbias_enabled_eval, reset=False)
        log.info("v4.60.5 EVAL-GLOBAL-BIAS: restored enabled=%s (value from state_dict)",
                 _gbias_enabled_eval)
...
def main():
    ...
    parser.add_argument("--disable_tier3_global_bias", action="store_true",
                        help="v4.60.5 turn off the global-bias scalar (for the "
                             "bias-off arm of the paired ablation)")
    ...
    if args.disable_tier3_global_bias:
        TIER3_TRAIN_CONFIG["tier3_use_global_bias"] = False
        log.info("v4.60.5 CLI: --disable_tier3_global_bias -> tier3_use_global_bias=False")
