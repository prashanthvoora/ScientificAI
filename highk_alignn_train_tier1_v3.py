...
==============================================================================
 ...
 FIX-MP2  Moderate: every MP entry was getting functional_code = PBE
          ...
          Two root causes:
          a) run_type was read from doc (DielectricDoc has no run_type attr)
             instead of summary_doc (SummaryDoc carries run_type).
             Fix: _run_src = summary_doc if present, else doc.
          b) "run_type" was absent from the requested fields in both the
             dielectric summary batch query and the oxide query.
             Fix: added "run_type" to both fields lists.
...
==============================================================================
 ...
 FIX5  Functional-aware labeling      APPLIED -- three locations:
       ...
       _parse_mp_entry:     detects PBE/GGA+U/r2SCAN from run_type; adds
                            functional_code, band_gap_pbe (NaN for r2SCAN)
...
class DatasetExtractor:
                ...
                for i in range(0, len(dielectric_mp_ids), BATCH_SIZE):
                    ...
                    docs_summary = mpr.materials.summary.search(
                        ...
                        fields=[
                            ...
                            "run_type",    # Fix 2: needed for functional_code detection
                        ],
                    )
    ...
    def _parse_mp_entry(self, doc, has_dielectric: bool, summary_doc=None) -> Optional[dict]:
        ...
        FIX5: Detects DFT functional from run_type; adds functional_code and
        ...
        try:
            ...
            # Fix 2c: run_type lives on SummaryDoc, not DielectricDoc.
            # For dielectric entries prefer summary_doc; for oxide entries use doc directly.
            # Both now have run_type in their requested fields (Fix 2a/2b).
            _run_src = summary_doc if summary_doc is not None else doc
            run_type = str(getattr(_run_src, "run_type", "") or "").upper()

            # FIX5: detect DFT functional used for this MP entry
            ...
            # Fix 1: in mp-api >= 0.39 the DielectricDoc returned by
            # mpr.materials.dielectric.search() exposes e_total as a direct
            # top-level attribute.  There is NO nested .dielectric sub-object.
            # The old `getattr(doc, "dielectric", None)` always returned None,
            # silently dropping every MP k value.
            k_total = None
            if has_dielectric:
                e_total = getattr(doc, "e_total", None)
                if e_total is not None:
                    try:
                        k_total = float(np.mean(np.diag(np.array(e_total))))
                    except (ValueError, TypeError, np.linalg.LinAlgError) as _te:
                        log.debug(
                            "MP %s: e_total tensor malformed (%s) -- k_total=None",
                            mp_id, _te,
                        )
...
class HighKGraphDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # avail_flag = 0.0 for all Tier 1/2 rows (columns absent → gate = 0).
        # avail_flag = 1.0 for Tier 3 experimental rows with ALD/stack data.
