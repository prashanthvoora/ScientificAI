...
==============================================================================
 ...
 Version 4.5.25 (v4.28: unify evaluate results — all tiers print linear-k MAD:MAE)
==============================================================================
 v4.5.25 fixes  (v4.28: unified evaluate results — all tiers print linear-k MAD:MAE)
 ---------------------------------------------------------------------------
 UNIFY-EVAL  Tier 1/2/3 evaluate printed results inconsistently:
           Tier 1: raw MAE, no label, no benchmark reference
           Tier 2: log + linear-k (correct)
           Tier 3: log + linear-k + MAD:MAE + status (correct)
           Fix: all tiers now print the same structured format.
           Tier 1 prints formation_energy MAE with ALIGNN paper ref.
           Tiers 2+3 print log diagnostic + linear-k MAE/RMSE/MAD/MAD:MAE.
           JSON output now stores linear-k values for all log-transform tiers.
           Stale k_measured key in Tier 3 MAD:MAE summary removed.

 v4.5.24 fixes  (v4.27: tier3_evaluate imputation no longer needs Tier 2 HDF5)
 ---------------------------------------------------------------------------
 FIX-EVAL-DONOR  tier3_evaluate imputation used df_tier2 as donor pool.
           If tier2_domain.h5 was missing or at wrong path, df_tier2=None
           and imputation was skipped entirely → df_eval=28 rows → N_Valid=9.
           Fix: df_structural (the 28 DFT rows already loaded from Tier 3
           HDF5) is used as fallback donor pool when df_tier2 is unavailable.
           28 HfO2/ZrO2 DFT rows + FORMULA_ALIASES + HfO2 fallback in
           _find_donor cover all 120 CSV material variants. Imputation now
           always runs regardless of whether tier2_domain.h5 exists.
           Expected: N_Valid returns to ~42 (30% of 139 rows after imputation).

 v4.5.23 fixes  (v4.26: tier3_evaluate imputation + reset_index + metrics)
 ---------------------------------------------------------------------------
 FIX-EVAL-IMPUTE  N_Valid=9 in tier3_evaluate (should be ~42):
           run_tier_evaluate Tier 3 path used df[atoms_dict.notna()] only.
           The 120 experimental rows have atoms_dict=None in the HDF5 cache
           because _impute_structures runs in run_tier3_finetune, not in
           build_tier3. So df_eval = 28 DFT rows → N_Valid=9 (30% of 28).
           Fix: tier3_evaluate dispatch in main() now loads df_tier2, builds
           df_donor_pool_wide, runs _impute_structures, and passes df_tier2
           into run_tier_evaluate via new parameter. run_tier_evaluate Tier 3
           path now mirrors run_tier3_finetune exactly:
             df_structural + _impute_structures + reset_index + df_phase_b filter
           N_Valid returns to ~42 (30% of 139 rows after imputation).

 FIX-EVAL-METRICS  Tier 3 evaluate reported raw log-space MAE as "k_measured":
           Old: log.info("k_measured (linear) MAE = %.4f", test_mae_log)
           This was wrong on two counts: (a) name is now k_total_log, (b) the
           value was log-space MAE not linear-k MAE. Fix: exact linear-k MAE,
           RMSE, MAD, MAD:MAE with pass/fail status vs 1.63/2.5 benchmarks,
           mirroring the fix already applied to run_tier3_finetune.

 FIX-EVAL-ILOC  reset_index(drop=True) missing in tier3_evaluate:
           df_eval was not reset after filtering, causing iloc vs loc mismatch
           in HighKGraphDataset (same FIX-ILOC bug as in run_tier3_finetune).
           Fix: .reset_index(drop=True) added.

 EVAL-NO-REBUILD  --rebuild_tier3 not needed for tier3_evaluate: CORRECT.
           tier3_evaluate loads the saved HDF5 directly. The raw CSV
           extraction and proc_renames only need to run once (during
           the last --rebuild_tier3 finetune run). This is correct behaviour.
...
def run_tier_evaluate(
    ...
    if tier == 1:
        ...
        df_process_only = df[df["atoms_dict"].isna()].copy()
        n_proc_only   = len(df_process_only)
        log.info(
            "Tier 3 evaluate: structural rows=%d  process-only rows=%d",
        ...
        # FIX-EVAL-IMPUTE: run _impute_structures to assign crystal structures
        # to experimental process-only rows, mirroring run_tier3_finetune exactly.
        # Without this, df_structural has only 28 DFT rows → N_Valid=9.
        # With imputation, df_structural has 148 rows → N_Valid matches training (~42).
        #
        # Donor pool priority:
        #   1. df_tier2 (if available) — broader coverage via wide high-k filter
        #   2. df_structural (the 28 DFT rows already loaded) — always available,
        #      sufficient because _find_donor FORMULA_ALIASES + HfO2 fallback
        #      can map every CSV formula to HfO2/ZrO2 from these 28 rows.
        if n_proc_only > 0:
            # Build donor pool — prefer df_tier2, fall back to df_structural
            if df_tier2 is not None:
                HIGH_K_ELEMS_DONOR = {"Hf", "Zr", "Al", "Ti", "Ta", "Sr", "La", "Y",
                                      "Ba", "Nb", "Ga", "In", "Sc", "Ce", "Pr", "Nd"}
                df_donor_pool = df_tier2[
                    df_tier2["formula"].apply(
                        lambda f: isinstance(f, str) and "O" in f and
                                  any(el in f for el in HIGH_K_ELEMS_DONOR)
                    )
                ...
                log.info(
                    "Tier 3 evaluate: donor pool from Tier 2 (%d rows)", len(df_donor_pool)
                )
            else:
                # Fallback: use the DFT rows already in df_structural.
                # 28 HfO2/ZrO2 rows + FORMULA_ALIASES + HfO2 fallback in
                # _find_donor covers all CSV material variants.
                df_donor_pool = df_structural.copy()
                log.info(
                    "Tier 3 evaluate: Tier 2 HDF5 not available — "
                    "using df_structural (%d DFT rows) as donor pool for imputation",
                    len(df_donor_pool),
                )

            log.info(
                "Tier 3 evaluate: running imputation on %d process-only rows...",
                n_proc_only,
            )
            df_imputed, df_unmatched = _impute_structures(df_process_only, df_donor_pool)
            log.info(
                "Tier 3 evaluate: imputed=%d  unmatched=%d",
                len(df_imputed), len(df_unmatched),
            )
            if len(df_imputed) > 0:
                df_structural = pd.concat(
                    [df_structural, df_imputed], ignore_index=True, sort=False
                ...
                log.info(
                    "Tier 3 evaluate: df_structural after imputation=%d rows",
                    len(df_structural),
                )
        ...
        # FIX-ILOC: reset_index so iloc==loc in HighKGraphDataset (mirrors run_tier3_finetune)
        df_eval = (
            df_structural[
        ...
        log.info("Tier 3 evaluate rows with %s: %d", cfg["target"], len(df_eval))
    ...
    # ── Unified linear-k metrics for all tiers that use log-transform ─────────
    # Tier 1: formation_energy_per_atom (no log-transform) → raw MAE is correct
    # Tier 2: k_total_log → convert to linear-k for benchmark comparison
    # Tier 3: k_total_log → convert to linear-k for benchmark comparison
    # All three tiers now print the same format for the k head so results
    # are directly comparable across tiers.
    mae_lin  = float("nan")
    rmse_lin = float("nan")
    mad_lin  = float("nan")
    mad_mae  = float("nan")
    ...
    uses_log_k = cfg.get("log_transform", False) and tier in (2, 3)

    if uses_log_k:
        valid   = ~torch.isnan(trues_t)
        n_valid = int(valid.sum())
        if n_valid > 0:
            p_lin   = torch.exp(preds_t[valid])
            t_lin   = torch.exp(trues_t[valid])
            mae_lin  = (p_lin - t_lin).abs().mean().item()
            rmse_lin = ((p_lin - t_lin) ** 2).mean().sqrt().item()
            t_np    = t_lin.cpu().numpy()
            mad_lin  = float(np.median(np.abs(t_np - np.median(t_np))))
            mad_mae  = mad_lin / mae_lin if mae_lin > 0 else float("nan")

    if tier == 1:
        log.info("  formation_energy_per_atom  MAE  = %.4f  [eV/atom]", test_mae_log)
        log.info("  formation_energy_per_atom  RMSE = %.4f  [eV/atom]", test_rmse_log)
        log.info("  ALIGNN paper MAE ≈ 0.22 eV/atom  (JARVIS DFPT OptB88vdW)")

    elif tier == 2:
        log.info("  Diagnostic log-space  MAE  = %.4f  [log(k) units]", test_mae_log)
        log.info("  Diagnostic log-space  RMSE = %.4f", test_rmse_log)
        log.info("  exp(MAE)              = %.4f×  → ±%.1f%% avg relative error",
                 _math.exp(test_mae_log), (_math.exp(test_mae_log) - 1) * 100)
        if n_valid > 0:
            log.info("  PRIMARY (linear-k exact)  MAE  = %.4f  [dielectric units]", mae_lin)
            log.info("  PRIMARY (linear-k exact)  RMSE = %.4f  [dielectric units]", rmse_lin)
            log.info("  PRIMARY (linear-k exact)  MAD  = %.4f  → MAD:MAE = %.2f  (N=%d)",
                     mad_lin, mad_mae, n_valid)
        log.info("  ALIGNN paper  MAD:MAE ≥ 1.63  (44K JARVIS DFPT, no transfer)")
        ...
        if n_valid > 0:
            ...
            log.info("─" * 68)
            if mad_mae >= 2.5:
                log.info("  STATUS: ✓ PROJECT GOAL MET  (MAD:MAE %.2f ≥ 2.5)", mad_mae)
            elif mad_mae >= 1.63:
                log.info("  STATUS: ✓ PAPER BENCHMARK MET  (MAD:MAE %.2f ≥ 1.63) — push toward 2.5", mad_mae)
            else:
                log.info("  STATUS: ✗ BELOW PAPER BENCHMARK  (MAD:MAE %.2f < 1.63)", mad_mae)
        log.info("  ALIGNN paper  MAD:MAE ≥ 1.63  |  project goal: MAD:MAE ≥ 2.5")
    ...
    primary  = {
        "mae_log_k":          test_mae_log,
        "rmse_log_k":         test_rmse_log,
        "mae_linear_k":       mae_lin,
        "rmse_linear_k":      rmse_lin,
        "mad_linear_k":       mad_lin,
        "mad_mae_linear_k":   mad_mae,
        "n_test":             n_valid,
        # canonical aliases — always linear-k for k-head tiers
        "mae":                mae_lin  if uses_log_k else test_mae_log,
        "rmse":               rmse_lin if uses_log_k else test_rmse_log,
        "benchmark_scale":    "linear",
        "diagnostic_scale":   "log" if uses_log_k else "linear",
...
def main():
    ...
    if args.mode in ["tier1_evaluate", "tier2_evaluate", "tier3_evaluate"]:
        ...
        # FIX-EVAL-IMPUTE: tier3_evaluate must run _impute_structures to
        # reproduce the same df_phase_b used during tier3_finetune training.
        # Without imputation, df_structural = atoms_dict.notna() = 28 DFT rows only
        # (experimental rows have atoms_dict=None in HDF5 cache because
        # _impute_structures runs in run_tier3_finetune, not in build_tier3).
        # This is why N_Valid was 9 (30% of 28) instead of ~42 (30% of 139).
        df_tier2_eval = None
        if tier_num == 3:
            log.info("Tier 3 evaluate: loading Tier 2 cache for imputation donor pool...")
            tier2_hdf5 = builder.TIER_PATHS[2]
            if tier2_hdf5.exists():
                df_tier2_eval = pd.read_hdf(tier2_hdf5, key="data")
                log.info("  Tier 2 donor pool: %d rows", len(df_tier2_eval))
            else:
                log.warning(
                    "Tier 2 HDF5 not found — imputation will use structural rows only. "
                    "N_Valid will be lower than training. Run tier2_finetune first."
                )
