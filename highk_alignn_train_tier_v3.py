...
==============================================================================
 ...
 FIX-MP2  Moderate: every MP entry was getting functional_code = PBE
          ...
          Original fix used run_type (invalid field → MPRestError at runtime).
          Revised fix: use is_hubbard (valid boolean field on SummaryDoc).
            is_hubbard=True  → functional_code = GGA+U
            is_hubbard=False → functional_code = PBE  (default; covers most entries)
          r2SCAN detection dropped: rare for high-k oxides; can be added in v4
          via a targeted task query after analysis of Tier 2/3 residuals.
          is_hubbard falls back to False gracefully if absent from cached docs.
 ...
 Performance fixes  (epoch time regression ~2x observed on Tier 1)
 ------------------------------------------------------------------
...
class DatasetExtractor:
    ...
    # mp-api >= 0.39: DielectricDoc exposes e_total, e_ionic, e_electronic
    # directly as top-level attributes.  The old names ("total","ionic",
    # "electronic") no longer exist and are silently ignored by the API.
    # "n" (refractive index) is valid but unused in parsing -- omitted.
    MP_ELECTRIC_FIELDS = [
    ...
    def pull_materials_project(
        ...
        with MPRester(MP_API_KEY) as mpr:
            ...
            docs_dielectric = mpr.materials.dielectric.search(
                fields=self.MP_ELECTRIC_FIELDS
            )
            log.info("  MP dielectric entries: %d", len(docs_dielectric))
            # -- Query lib: Fetch summary data for dielectric entires ---------
            # The dielectric endpoint doesn't include band_gap, formation_energy, etc.
            # we need to query the summary endpoint to get these properties
            dielectric_mp_ids = [doc.material_id for doc in docs_dielectric]
            summary_data_map = {}

            if dielectric_mp_ids:
                log.info("  Fetching summary data for dielectric entries...")
                # fetch in batches to avoid API limits
                BATCH_SIZE = 1000
                for i in range(0, len(dielectric_mp_ids), BATCH_SIZE):
                    batch_ids = dielectric_mp_ids[i:i+BATCH_SIZE]
                    docs_summary = mpr.materials.summary.search(
                        material_ids=batch_ids,
                        fields=[
                            "material_id", "formula_pretty", "structure",
                            "band_gap", "energy_above_hull",
                            "formation_energy_per_atom", "is_metal",
                            "is_hubbard",   # True when GGA+U was used (replaces run_type)
                        ],
                    )
                    for doc in docs_summary:
                        summary_data_map[doc.material_id] = doc
                    log.info("  Fetched summary data for %d/%d",
                             min(i + BATCH_SIZE, len(dielectric_mp_ids)),
                             len(dielectric_mp_ids))


            for doc in tqdm(docs_dielectric, desc="Parsing MP dielectric entries"):
                # Get summary data for this material
                summary_doc = summary_data_map.get(doc.material_id)
                row = self._parse_mp_entry(doc, has_dielectric=True, summary_doc=summary_doc)
                if row is not None:
                    rows.append(row)
    ...
    def _parse_mp_entry(self, doc, has_dielectric: bool, summary_doc=None) -> Optional[dict]:
        ...
        try:
            ...
            # FIX5 / FIX-MP2 (revised): detect DFT functional from is_hubbard.
            #
            # run_type is NOT a valid field on the mp-api summary endpoint
            # (raises MPRestError).  is_hubbard is the correct substitute:
            #   True  → Hubbard U correction applied → GGA+U
            #   False → standard GGA/PBE (covers the large majority of entries)
            #
            # r2SCAN detection is intentionally omitted: r2SCAN entries are rare
            # for the high-k oxide materials we target, and detecting them requires
            # a separate task-level query not worth the extra API calls at this tier.
            # r2SCAN support can be added in v4 via a targeted task query if the
            # analysis shows systematic r2SCAN offset in the Tier 2/3 residuals.
            #
            # is_hubbard falls back gracefully to False via getattr when absent
            # from the doc (e.g. older cached summary docs without the field).
            ...
            # Review fix 1: use the shared _compute_k_from_tensor helper instead
            # of inlining np.mean(np.diag(...)).  The helper handles malformed
            # tensors, scalars, 3-element diagonals and None inputs internally,
            # so the try/except block here is no longer needed.
            #
            # Review fix 2: e_ionic and e_electronic are already requested in
            # MP_ELECTRIC_FIELDS -- parse and store them so MP rows carry the
            # same k_ionic / k_elec breakdown that JARVIS rows do.
            # Note: e_total ≈ e_ionic + e_electronic but may differ slightly
            # due to DFPT numerical rounding; store e_total-derived k_total
            # as the authoritative value (consistent with JARVIS convention).
            k_total = None
            k_ionic = None
            k_elec  = None
            if has_dielectric:
                e_total      = getattr(doc, "e_total",      None)
                e_ionic_t    = getattr(doc, "e_ionic",      None)
                e_electronic = getattr(doc, "e_electronic", None)
                k_total = self._compute_k_from_tensor(e_total,      None)
                k_ionic = self._compute_k_from_tensor(e_ionic_t,    None)
                k_elec  = self._compute_k_from_tensor(e_electronic, None)
...
class TierDatasetBuilder:
    ...
    def deduplicate_cross_source(
        ...
        log.info("FIX4 dedup: dropped %d MP duplicates -> %d MP entries remain",
    ...
    def build_tier1(
        ...
        if skip_cross_dedup:
            ...
            df_mp_clean = (
                self.deduplicate_cross_source(df_jarvis, df_mp)
    ...
    def build_tier2(
        ...
        # Use k_total (DFT ionic+elec) for consistency with TIER2_TRAIN_CONFIG
        # target = "k_total".  k_measured holds the same value for DFT entries
        # (JARVIS/MP), but k_total is the semantically correct column here.
