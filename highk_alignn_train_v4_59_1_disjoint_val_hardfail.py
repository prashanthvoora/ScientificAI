...
==============================================================================
 ...
 Version 4.5.55 (v4.56: controlled Tier-2 k-head adaptation for Tier-3)
==============================================================================
...
TIER3_TRAIN_CONFIG = dict(
    ...
    tier3_disjoint_val_ratio = 0.20,   # fraction of eval-family verified TRAIN rows held out as VAL
...
def build_tier3_hfo2_eval_dataloader(
    ...
    if loaded_test_pos is not None and len(loaded_test_pos) > 0:
        # Reuse the exact TEST rows saved from the normal finetune run.
        test_pos = loaded_test_pos.astype(int)
        test_set = set(test_pos.tolist())
        # In persistent-split mode, all non-test rows are training candidates.
        # This removes ambiguity between tier3_finetune, tier3_evaluate and
        # --ablate_context comparisons.
        train_pos = np.asarray([p for p in all_pos.tolist() if p not in test_set], dtype=int)
        val_pos = test_pos.copy()
        eval_train_pos = np.asarray([p for p in eval_pos.tolist() if p not in test_set], dtype=int)
        log.info(
            "v4.35 PERSISTENT-SPLIT active: using saved TEST rows=%d; train rows=%d; VAL reuses TEST",
            len(test_pos), len(train_pos),
        )
    else:
        ...
        if len(val_pos) == 0 and len(test_pos) > 0:
            # Legacy trainer expects a val_loader even for epoch0-only runs. Reuse
            # test rows for VAL diagnostics but make the log explicit. Headline
            # metric should be read from TEST.
            log.warning("v4.31 train/test-only split: val set is empty; VAL loader reuses TEST rows for diagnostics only.")
            val_pos = test_pos.copy()
...
def run_tier3_finetune(
    ...
    if cfg.get("eval_hfo2_family_only", False) and TIER3_TEST_SPLIT_PATH.exists():
        ...
        val_loader = DataLoader(_canonical_subset, **_eval_loader_kwargs)
        ...
        _canonical_val_n = len(_graph_ok_test_pos)
        log.info(
    ...
    if cfg.get("tier3_disjoint_val", False) and cfg.get("eval_hfo2_family_only", False):
        ...
        if len(_val_candidate_pos) < _dv_min_pool:
        ...
        # v4.59.1: the small-pool case raised above; the carve now runs
        # unconditionally (no legacy VAL==TEST branch to fall into).
        ...
        log.info(
            ...
            "TEST reported once at end (tier3_evaluate) and never used for selection."
        )
        log.info("─" * 68)
