...
def _normalize_id(value: str) -> str:
    """
    Strip a purely-cosmetic float-formatting artifact from ID-like strings:
    pandas upcasts an all-integer ID column to float the moment it contains
    even one NaN, so '1024' silently becomes '1024.0' on one side of a join
    while the other side (which never went through that column) stays
    '1024'. This does NOT touch genuinely different values -- only strings
    that are an integer followed by '.0' (optionally more trailing zeros).
    """
    v = value.strip()
    import re
...
def merge_on_row_id(df_full: pd.DataFrame, df_pred: pd.DataFrame, id_column: str = None) -> pd.DataFrame:
    ...
    df_full["_join_key"] = df_full[key].astype(str).map(_normalize_id)
    df_pred = df_pred.copy()
    df_pred["_join_key"] = df_pred["paper_id"].astype(str).map(_normalize_id)
    ...
    if len(merged) < 0.5 * len(df_full):
        ...
        if len(pred_keys) > 1.5 * len(full_keys) or len(full_keys) > 1.5 * len(pred_keys):
            print(f"  NOTE: unique-key counts differ by a large factor "
                  f"({len(full_keys)} vs {len(pred_keys)}). If these two files describe "
                  "the same experimental dataset, that shouldn't happen -- "
                  "backbone_predictions_full_db.csv is built by filtering DOWN from "
                  "the full process DB, so it should never have MORE unique IDs than "
                  "--full_db has rows (or vice versa, beyond a handful lost to "
                  "structure-imputation misses). This usually means the file you "
                  "passed as --full_db is not the SAME file "
                  "generate_backbone_predictions.py actually read -- that script calls "
                  "extractor.load_experimental_process_db() internally, which reads "
                  "from a HARDCODED path (T.DATA_ROOT / 'processed' / "
                  "'process_db_clean.csv'), not from any --full_db argument you pass "
                  "here. Confirm both point at the literal same file (same path, "
                  "same row count, same mtime) before trusting the join.")
        if overlap:
            print(f"  {len(overlap)} keys DO match exactly even after '.0' normalization "
                  "-- if that's still far fewer than expected, some rows likely differ "
                  "in other ways (leading zeros, case, extra characters) rather than "
                  "float formatting.")
        else:
            print("  ZERO keys match even after stripping trailing '.0'. If the samples "
                  "above still look like genuinely different ID schemes, double-check "
                  "what load_experimental_process_db() renamed 'row_id' to/from in your "
                  "training script version, and whether process_db_clean.csv's id "
                  "column really is 'row_id' -- if it's a different name, pass "
                  "--id_column explicitly.")
    if len(merged) == 0:
        raise RuntimeError(
