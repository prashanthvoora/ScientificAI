...
class ALIGNNTrainer:
    ...
    @torch.no_grad()
    def collect_process_delta_stats(
        self,
        loader: DataLoader,
        split_name: str = "VAL",
        stage_name: str = "current",
    ) -> Dict[str, float]:
        """Collect bounded process-residual correction statistics.

        The residual-process model predicts:
            y_log = y_dft_base_log + delta_log

        where delta_log = process_delta_bound * tanh(raw_delta) * proc_avail.
        This diagnostic measures the actual delta added to k_total_log.

        Interpretation:
          - delta_std_proc ~= 0: process branch is effectively inactive.
          - nonzero delta with no metric gain: process branch is active but not useful.
          - large delta_abs_max near bound: process branch is saturating/overfitting.
        """
        self.model.eval()
        if not getattr(self.model_core, "use_bounded_process_residual", False):
            out = {
                "stage": stage_name, "split": split_name, "enabled": False,
                "n_total": 0, "n_proc": 0, "n_dft": 0,
                "delta_mean_all": float("nan"), "delta_std_all": float("nan"),
                "delta_abs_mean_all": float("nan"), "delta_abs_max_all": float("nan"),
                "delta_mean_proc": float("nan"), "delta_std_proc": float("nan"),
                "delta_abs_mean_proc": float("nan"), "delta_abs_max_proc": float("nan"),
                "delta_mean_dft": float("nan"), "delta_abs_max_dft": float("nan"),
                "nonzero_pct_all": float("nan"), "nonzero_pct_proc": float("nan"),
                "saturation_pct_proc": float("nan"),
                "bound": float(getattr(self.model_core, "process_delta_bound", float("nan"))),
            }
            log.info(
                "RESIDUAL-DELTA-DIAG | %-18s | %-5s | disabled (bounded process residual not active)",
        ...
        with torch.no_grad():
            for batch in loader:
                ...
                if self.ablate_context:
                    proc_ctx = None
                delta = self.model_core._process_bounded_delta(proc_ctx)
                if delta is None:
                    continue
                d = delta.detach().view(-1).cpu()
                if proc_ctx is not None and "avail" in proc_ctx:
                    a = proc_ctx["avail"].detach().view(-1).cpu()
                else:
        ...
        if not deltas:
            out = {
                "stage": stage_name, "split": split_name, "enabled": True,
                "n_total": 0, "n_proc": 0, "n_dft": 0,
                "delta_mean_all": float("nan"), "delta_std_all": float("nan"),
                "delta_abs_mean_all": float("nan"), "delta_abs_max_all": float("nan"),
                "delta_mean_proc": float("nan"), "delta_std_proc": float("nan"),
            ...
            log.warning(
                "RESIDUAL-DELTA-DIAG | %-18s | %-5s | no batches/deltas collected",
                stage_name, split_name,
            )
            return out

        delta_all = torch.cat(deltas).float()
        avail_all = torch.cat(avails).float()
        proc_mask = avail_all > 0.5
        dft_mask = ~proc_mask
        eps = 1e-8
        bound = float(getattr(self.model_core, "process_delta_bound", float("nan")))

        def _mean(x): return float(x.mean().item()) if x.numel() else float("nan")
        def _std(x): return float(x.std(unbiased=False).item()) if x.numel() else float("nan")
        def _absmean(x): return float(x.abs().mean().item()) if x.numel() else float("nan")
        def _absmax(x): return float(x.abs().max().item()) if x.numel() else float("nan")
        def _nonzero(x): return float((x.abs() > eps).float().mean().item() * 100.0) if x.numel() else float("nan")

        delta_proc = delta_all[proc_mask]
        delta_dft = delta_all[dft_mask]
        sat_threshold = 0.95 * bound if bound == bound else float("inf")
        sat_pct = (
            float((delta_proc.abs() >= sat_threshold).float().mean().item() * 100.0)
            if delta_proc.numel() and sat_threshold > 0 else float("nan")
        )
        ...
        out = {
            ...
            "split": split_name,
            "enabled": True,
            "n_total": int(delta_all.numel()),
            "n_proc": int(proc_mask.sum().item()),
            "n_dft": int(dft_mask.sum().item()),
            "proc_coverage_pct": float(proc_mask.float().mean().item() * 100.0),
            "bound": bound,
            "delta_mean_all": _mean(delta_all),
            "delta_std_all": _std(delta_all),
            "delta_abs_mean_all": _absmean(delta_all),
            "delta_abs_max_all": _absmax(delta_all),
            "delta_mean_proc": _mean(delta_proc),
            "delta_std_proc": _std(delta_proc),
            "delta_abs_mean_proc": _absmean(delta_proc),
            "delta_abs_max_proc": _absmax(delta_proc),
            "delta_mean_dft": _mean(delta_dft),
            "delta_abs_max_dft": _absmax(delta_dft),
            "nonzero_pct_all": _nonzero(delta_all),
            "nonzero_pct_proc": _nonzero(delta_proc),
            "saturation_pct_proc": sat_pct,
        }

        # Multiplicative interpretation: exp(delta_log)-1 is the relative k shift.
        rel_mean_proc = (math.exp(out["delta_mean_proc"]) - 1.0) * 100.0 if out["delta_mean_proc"] == out["delta_mean_proc"] else float("nan")
        rel_absmax_proc = (math.exp(out["delta_abs_max_proc"]) - 1.0) * 100.0 if out["delta_abs_max_proc"] == out["delta_abs_max_proc"] else float("nan")
        out["delta_mean_proc_relative_pct"] = float(rel_mean_proc)
        out["delta_abs_max_proc_relative_pct"] = float(rel_absmax_proc)

        log.info(
            "RESIDUAL-DELTA-DIAG | %-18s | %-5s | "
            "N=%d proc=%d(%.1f%%) dft=%d | "
            "delta_all mean=%+.5f std=%.5f abs_mean=%.5f abs_max=%.5f | "
            "delta_proc mean=%+.5f std=%.5f abs_mean=%.5f abs_max=%.5f "
            "(~mean %+ .2f%%, max %.2f%% k-shift) | nonzero_proc=%.1f%% sat_proc=%.1f%% bound=%.3f",
            stage_name, split_name,
            out["n_total"], out["n_proc"], out["proc_coverage_pct"], out["n_dft"],
            out["delta_mean_all"], out["delta_std_all"],
            out["delta_abs_mean_all"], out["delta_abs_max_all"],
            out["delta_mean_proc"], out["delta_std_proc"],
            out["delta_abs_mean_proc"], out["delta_abs_max_proc"],
            out["delta_mean_proc_relative_pct"], out["delta_abs_max_proc_relative_pct"],
            out["nonzero_pct_proc"], out["saturation_pct_proc"], out["bound"],
        )
        return out
...
def run_tier3_finetune(
    ...
    def _eval_exact_k_for_diag(loader, split_name: str, stage_name: str) -> dict:
        ...
        out["delta_stats"] = trainer.collect_process_delta_stats(
            loader, split_name=split_name, stage_name=stage_name
        )
        return out
