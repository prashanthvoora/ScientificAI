...
def _split_positions_grouped(rows, positions, train_frac, val_frac, seed, mode):
    """Three-way group-disjoint split; deterministic row fallback if impossible."""
    positions = np.asarray(positions, dtype=int)
    if mode == "row":
    ...
    if len(np.unique(groups)) < 3:
        log.warning("v4.59 GROUP-SPLIT mode=%s has only %d groups; falling back to row split", mode, len(np.unique(groups)))
        return _split_positions_stratified(rows, positions, TIER3_TRAIN_CONFIG["target"], train_frac, val_frac, seed)
