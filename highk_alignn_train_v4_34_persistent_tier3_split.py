...
==============================================================================
 ...
 Version 4.5.34 (v4.34: persistent Tier-3 test split IDs)
==============================================================================
...
def _tier3_split_key_from_row(row: pd.Series) -> str:
    """Stable row key used for persistent Tier-3 split matching.

    Priority:
      1. row_id when present and non-empty
      2. row_hash when present
      3. conservative composite fallback for older caches
    """
    for col in ["row_id", "row_hash"]:
        v = row.get(col, None)
...
def save_tier3_test_split_rows(
    ...
    log.info("v4.34 SPLIT-SAVE: saved Tier3 TEST split rows=%d → %s", len(rows), path)
...
def load_tier3_test_positions_from_split(
    ...
    Returns None if no usable split file exists or too few rows match.
    """
    if not path.exists():
        log.info("v4.34 SPLIT-LOAD: no saved Tier3 split file found at %s", path)
        return None
    try:
        split_df = pd.read_csv(path)
        if "split_key" in split_df.columns:
            wanted = set(split_df["split_key"].dropna().astype(str).tolist())
        else:
            wanted = set()
            for _, r in split_df.iterrows():
                wanted.add(_tier3_split_key_from_row(r))
        if not wanted:
            log.warning("v4.34 SPLIT-LOAD: saved split file has no usable keys: %s", path)
            return None
        current_keys = valid_rows.apply(_tier3_split_key_from_row, axis=1).astype(str).tolist()
        matched = [i for i, k in enumerate(current_keys) if k in wanted]
        log.info(
            "v4.34 SPLIT-LOAD: loaded saved Tier3 TEST split %s matched=%d/%d current_valid_rows=%d",
            path, len(matched), len(wanted), len(valid_rows),
        )
        if len(matched) < max(5, int(0.5 * len(wanted))):
            log.warning(
                "v4.34 SPLIT-LOAD: too few saved TEST rows matched (%d/%d). Falling back to deterministic split.",
                len(matched), len(wanted),
            )
            return None
        return np.asarray(matched, dtype=int)
    except Exception as exc:
        log.warning("v4.34 SPLIT-LOAD: failed to load saved split %s: %s", path, exc)
        return None
...
def build_tier3_hfo2_eval_dataloader(
    ...
    if loaded_test_pos is not None and len(loaded_test_pos) > 0:
        ...
        log.info(
            "v4.34 PERSISTENT-SPLIT active: using saved TEST rows=%d; train rows=%d; VAL reuses TEST",
...
def run_tier_evaluate(
    ...
    if tier == 3 and cfg.get("eval_hfo2_family_only", False):
        ...
        log.info("Test loader: %d batches  (v4.34 persistent Tier3 test split when available)", len(test_loader))
    else:
