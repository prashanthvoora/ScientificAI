...
def run_tier3_finetune(
    ...
    # If disabled, or if too few candidates exist, legacy VAL==TEST is retained.
    # ══════════════════════════════════════════════════════════════════════
    if cfg.get("tier3_disjoint_val", False) and cfg.get("eval_hfo2_family_only", False):
        ...
        if len(_val_candidate_pos) < 6:
            log.warning(
                "DISJOINT-VAL: only %d eval-family verified TRAIN rows available; "
                "cannot form a stable disjoint VAL. FALLING BACK to legacy VAL==TEST. "
                "This run is NOT a disjoint-val experiment.",
                len(_val_candidate_pos),
            )
        else:
    ...
    def _write_tier3_dataset_composition_audit():
        ...
        if cfg.get("tier3_disjoint_val", False) and _canonical_val_n != _canonical_test_n:
    ...
    if freeze_backbone:
        ...
        if len(getattr(pb_val_loader.dataset, "indices", [])) != _pb_expected_val_n:
            raise RuntimeError(
                ...
                % (
                    len(getattr(pb_val_loader.dataset, "indices", [])),
                    "VAL" if _pb_expected_val_n == _canonical_val_n else "TEST",
                    _pb_expected_val_n,
                )
            )
