...
==============================================================================
 ...
 Version 4.5.5  (Tier 3 readiness fixes -- process column rename, tier3_evaluate,
                J_g task weight correction, targeted Tier 3 rebuild flag,
                experimental row_hash collision fix, early-stopping config,
                Option-A structure imputation, target column name mismatch)
==============================================================================
...
TIER3_TRAIN_CONFIG = dict(
    ...
    # FIX-T3-6 Factor 1: LR reduced 5× (5e-5 → 1e-5).
    # With the Tier 2 backbone already near-optimal for HfO2-DFT dielectric
    # prediction, 5e-5 immediately overshoot the optimum on the first gradient
    # step.  val_MAE rose from epoch 1; patience_ctr incremented every epoch;
    # stopped at epoch 31.  1e-5 gives gentler adaptation while still allowing
    # fine-tuning signal from the k_measured distribution to propagate.
    ...
    val_ratio      = 0.15,
    test_ratio     = 0.15,
    freeze_layers  = 0,        # all layers trainable from epoch 1

    # FIX-T3-6 Factors 2 & 3: explicit early stopping config.
    # Previously all three keys were absent; ALIGNNTrainer fell back to
    # defaults (early_stopping=True, min_epochs=0, patience=30) which combined
    # with best_epoch=1 to stop training at epoch 31.
    early_stopping = True,     # keep ES enabled, but now explicitly controlled
    min_epochs     = 40,       # floor: ES cannot fire before epoch 40
    patience       = 30,       # wait 30 non-improving epochs after min_epochs
    # Net effect: guaranteed to run epochs 1-40; will continue past 40 as long
    # as val_MAE improves; stops at the first epoch ≥ 40 where patience is
    # exhausted.  Worst case: 40 + 30 = 70 epochs; best case: full 100.
)
...
class DatasetExtractor:
    ...
    def load_experimental_process_db(
        ...
        # FIX-T3-8: The previous dict held only identity entries ("k_measured" →
        # "k_measured") which never fired because the seed CSV uses different
        # column names.  All 120 experimental rows arrived with k_measured=NaN,
        # J_g_A_cm2=NaN, E_BD_MV_cm=NaN.  Expanded to cover every column name
        # variant produced by ald_process_params_seed.csv and its extensions.
        #
        ...
        target_renames = {
            # ── k_measured ────────────────────────────────────────────────────
            "dielectric_constant_k":        "k_measured",   # seed CSV schema
            ...
            "leakage_J_A_cm2_at_field":    "J_g_A_cm2",   # seed CSV schema
...
def run_tier3_finetune(
    ...
    for _tgt in ["k_measured", "band_gap", "J_g_A_cm2", "E_BD_MV_cm"]:
...
def run_tier_evaluate(
    ...
    if tier == 1:
        ...
        cfg = TIER3_TRAIN_CONFIG
        # Mirror run_tier3_finetune: operate only on structural rows so the
        # test split is reproduced from the same DataFrame as training.
        ...
        if len(df_structural) == 0:
            log.error(
                "No structural rows in Tier 3 DataFrame.  "
                "Cannot reproduce the test split.  Aborting evaluation."
            )
            return

        # Tier 3 trains k_measured in linear space (no log transform).
        df_eval = df_structural[
            pd.to_numeric(df_structural["k_measured"], errors="coerce").notna()
        ].copy()
        log.info("Tier 3 rows with k_measured (structural): %d", len(df_eval))

        target_col  = cfg["target"]            # k_measured
        task_names  = [target_col] + cfg["aux_targets"]
        ckpt_prefix = "tier3"
