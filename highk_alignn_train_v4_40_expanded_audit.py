...
def load_tier3_test_positions_from_split(
    ...
    """Load saved Tier-3 TEST positions for the current valid_rows frame.

    v4.35: match by multiple persistent keys instead of volatile dataframe
    positions.  This makes tier3_finetune, tier3_evaluate and --ablate_context
    comparable even after --rebuild_tier3 changes row ordering.
    """
    if not path.exists():
        log.info("v4.35 SPLIT-LOAD: no saved Tier3 split file found at %s", path)
        return None
    try:
        split_df = pd.read_csv(path)
        wanted_keys = set()

        # New v4.35 files: all candidate keys are stored explicitly.
        if "split_key_candidates" in split_df.columns:
            for cell in split_df["split_key_candidates"].dropna().astype(str):
                for k in cell.split("||"):
                    k = k.strip()
                    if k:
                        wanted_keys.add(k)

        # Backward compatibility with v4.34 split files.
        if "split_key" in split_df.columns:
            wanted_keys.update(split_df["split_key"].dropna().astype(str).tolist())
        for _, r in split_df.iterrows():
            wanted_keys.update(_tier3_split_keys_from_row(r))

        if not wanted_keys:
            log.warning("v4.35 SPLIT-LOAD: saved split file has no usable keys: %s", path)
            return None

        matched = []
        for i, (_, r) in enumerate(valid_rows.iterrows()):
            row_keys = set(_tier3_split_keys_from_row(r))
            if row_keys.intersection(wanted_keys):
                matched.append(i)

        n_saved_rows = len(split_df)
        log.info(
            "v4.35 SPLIT-LOAD: loaded saved Tier3 TEST split %s matched_rows=%d/%d current_valid_rows=%d candidate_keys=%d",
            path, len(matched), n_saved_rows, len(valid_rows), len(wanted_keys),
        )

        # Require most saved ROWS to match, not most candidate keys.
        if len(matched) < max(5, int(0.70 * n_saved_rows)):
            log.warning(
                "v4.35 SPLIT-LOAD: too few saved TEST rows matched (%d/%d). "
                "This usually means the split file is stale or a different DB/cache is being used. "
                "Run normal tier3_finetune once to overwrite the split, or delete %s.",
                len(matched), n_saved_rows, path,
            )
            return None
        return np.asarray(sorted(set(matched)), dtype=int)
    except Exception as exc:
        log.warning("v4.35 SPLIT-LOAD: failed to load saved split %s: %s", path, exc)
        return None
...
def run_tier3_finetune(
    ...
    def _write_tier3_dataset_composition_audit():
        ...
        log.info("v4.40 AUDIT dataset composition written: %s", comp_path)
    ...
    if freeze_backbone:
        ...
        pb_loader, pb_val_loader, _, pb_sampler, _ds = _tier3_loader_fn(
            df         = df_phase_b,
            target_col = cfg["target"],
        ...
        for audit_epoch in audit_epochs:
            ...
            _eval_exact_k_for_diag(pb_val_loader, "VAL", stage_a)
            _eval_exact_k_for_diag(test_loader, "TEST", stage_a)
            _write_tier3_prediction_audit(pb_val_loader, _main_ds_t3.df, "VAL", stage_a)
    ...
    if cfg.get("eval_hfo2_family_only", False) and TIER3_TEST_SPLIT_PATH.exists():
        try:
            ...
            log.info(
                "v4.37 FINAL-SPLIT-RELOAD: final tier3_finetune TEST loader rebuilt from saved split; batches=%d",
                len(test_loader),
            )
        except Exception as exc:
            log.warning(
