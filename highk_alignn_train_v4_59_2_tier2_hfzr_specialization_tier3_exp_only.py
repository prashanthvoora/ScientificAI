...
==============================================================================
 ...
 Version 4.59.2 (v4.59.1 + Tier-2 Hf/Zr specialization + Tier-3 experimental-only)
==============================================================================
...
TIER3_TEST_SPLIT_PATH = SPLIT_ROOT / "tier3_split_manifest_v459.csv"
...
TIER2_TRAIN_CONFIG = dict(
    ...
    # -- Tier-2 Phase C: Hf/Zr DFT specialization -------------------------
    # Ported from v4.53. Phase A/B remain unchanged. After Phase B, retain
    # its checkpoint and run a short low-LR specialization on every valid
    # Hf/Zr oxide row with k_total and a crystal structure.
    phase_c_hfzr_specialization = True,
    phase_c_epochs              = 20,
    phase_c_learning_rate       = 2e-5,
    phase_c_weight_decay        = 1e-5,
    phase_c_min_rows            = 12,
    phase_c_train_ratio         = 0.80,
    phase_c_val_ratio           = 0.10,
...
TIER3_TRAIN_CONFIG = dict(
    ...
    aux_targets    =[],
    train_ratio    = 0.70,
    val_ratio      = 0.15,  # v4.59.1: independent validation set for checkpoint selection
    test_ratio     = 0.15,  # v4.59.1: untouched held-out test set

    # v4.59.2: clean tier separation. Tier 2 consumes the valid Hf/Zr DFT
    # rows in Phase C; Tier 3 trains only on experimental rows that have a
    # native/imputed structure and at least one real process feature.
    ...
    # v4.59: explicit global DFT->experiment calibration. Branch centering is
    # disabled by default so a real population shift is not suppressed.
    tier3_use_global_bias = True,
    tier3_global_bias_init = 0.0,
    tier3_adapter_center_weight = 0.0,
    tier3_process_center_weight = 0.0,
    # v4.59: scale-invariant squared correlation, guarded by variance floor.
    tier3_adapter_process_orthogonal_weight = 1.0,
    tier3_branch_corr_eps = 1e-8,
    tier3_branch_variance_floor = 1e-6,
    # Split protocol: donor_material prevents donor/material identity overlap.
    # Alternatives: row, material, donor, paper.
    tier3_split_group_mode = "donor_material",
...
class HighKALIGNN(nn.Module):
    ...
    def __init__(
        ...
        # v4.59 global calibration is a separate identifiable component.
        self.tier3_global_bias = nn.Parameter(torch.zeros(1))
        self.use_tier3_global_bias = True
    ...
    def freeze_for_bounded_process_residual(self, train_base_head: bool = False):
        ...
        if self.use_tier3_structural_adapter:
            for p in self.tier3_structural_adapter.parameters():
                p.requires_grad = True
        if self.use_tier3_global_bias:
            self.tier3_global_bias.requires_grad = True
        object.__setattr__(self, "_tier3_base_head_reference", None)
    ...
    def configure_structural_adapter(
        ...
        log.info(
            "v4.59 STRUCTURAL-ADAPTER: enabled=%s rank=%d reset=%s rebuilt=%s",
    ...
    def forward(
        ...
        if self.use_bounded_process_residual:
            ...
            if task == "__all__":
                ...
                for t, head in self.task_heads.items():
                    ...
                    if t == "k_total_log":
                        ...
                        global_bias = self.tier3_global_bias.view(1, 1) if self.use_tier3_global_bias else 0.0
                        self._last_global_bias = float(self.tier3_global_bias.detach().item()) if self.use_tier3_global_bias else 0.0
                        out[t] = base + global_bias + structural_delta + (delta if delta is not None else 0.0)
                    else:
                        out[t] = base
                return out
            ...
            if task == "k_total_log":
                ...
                global_bias = self.tier3_global_bias.view(1, 1) if self.use_tier3_global_bias else 0.0
                self._last_global_bias = float(self.tier3_global_bias.detach().item()) if self.use_tier3_global_bias else 0.0
                return base + global_bias + structural_delta + (delta if delta is not None else 0.0)
            return base
...
class ALIGNNTrainer:
    ...
    def rebuild_optimizer_for_current_trainables(self):
        """v4.59 AdamW groups for process residual and low-rank structural adapter."""
        ...
        log.info("v4.59 OPTIMIZER-REBUILD: tensors=%d params=%d groups=%s", len(all_trainable), sum(p.numel() for p in all_trainable), [(g["group_name"],g["lr"],sum(p.numel() for p in g["params"])) for g in groups])
                    ...
                    if bool(active.any()):
                        ...
                        if structural_delta is not None:
                            ...
                            if sd_active.numel() > 1:
                                ...
                                pdc = pdlt - pdlt.mean()
                                cov = torch.mean(sdc * pdc)
                                var_s = torch.mean(sdc.square())
                                var_p = torch.mean(pdc.square())
                                var_floor = float(self.cfg.get("tier3_branch_variance_floor", 1e-6))
                                if float(var_s.detach().item()) > var_floor and float(var_p.detach().item()) > var_floor:
                                    eps = float(self.cfg.get("tier3_branch_corr_eps", 1e-8))
                                    corr = cov / torch.sqrt(var_s * var_p + eps)
                                    orthogonal_loss = corr.square()
            loss = (primary_loss
    ...
    def train(
        ...
        for epoch in range(1, n_epochs + 1):
            ...
            # v4.59 release audit: explicit adapter/process health every epoch.
            adapter_active = (
...
def save_tier3_split_rows(
    ...
    """Save exact rows for one split into a persistent three-way manifest."""
    if not is_rank0():
        return
    path.parent.mkdir(parents=True, exist_ok=True)
    ...
    if path.exists():
        try:
            ...
            existing = pd.DataFrame()
    if not overwrite and not existing.empty and "split" in existing.columns and split_name in set(existing["split"].astype(str)):
        log.info("Tier3 manifest already contains split=%s; not overwriting: %s", split_name, path)
        return
    ...
    if not existing.empty and "split" in existing.columns:
        existing = existing[existing["split"].astype(str) != str(split_name)]
        new_df = pd.concat([existing, new_df], ignore_index=True)
    new_df.to_csv(path, index=False)
    log.info("v4.59 SPLIT-MANIFEST-SAVE: split=%s rows=%d total=%d → %s", split_name, len(rows), len(new_df), path)


def load_tier3_positions_from_split(
    valid_rows: pd.DataFrame,
    path: Path = TIER3_TEST_SPLIT_PATH,
    split_name: str = "test",
) -> Optional[np.ndarray]:
    ...
    try:
        ...
        if split_df.empty:
            log.warning("v4.59 STRICT-SPLIT-LOAD: empty split file: %s", path)
            return None
        if "split" in split_df.columns:
            split_df = split_df[split_df["split"].astype(str) == str(split_name)].reset_index(drop=True)
        if split_df.empty:
            log.warning("v4.59 STRICT-SPLIT-LOAD: split=%s absent in %s", split_name, path)
            return None
...
def _tier3_group_labels(rows: pd.DataFrame, positions: np.ndarray, mode: str) -> np.ndarray:
    """Stable labels for leakage-controlled Tier-3 evaluation splitting."""
    sub = rows.iloc[np.asarray(positions, dtype=int)]
    def norm(x):
        x = str(x).strip().lower()
        return "" if x in {"", "nan", "none", "null"} else x
    labels = []
    for _, r in sub.iterrows():
        material = norm(r.get("material", r.get("material_system", r.get("formula", ""))))
        donor = norm(r.get("imputed_from", r.get("donor_id", r.get("jid", r.get("mp_id", "")))))
        paper = norm(r.get("doi", r.get("paper_id", "")))
        if mode == "material": key = material
        elif mode == "donor": key = donor or material
        elif mode == "paper": key = paper or material
        elif mode == "donor_material": key = donor or material
        else: key = _tier3_split_key_from_row(r)
        labels.append(key or _tier3_split_key_from_row(r))
    return np.asarray(labels, dtype=object)


def _split_positions_grouped(rows, positions, train_frac, val_frac, seed, mode):
    """Three-way group-disjoint split with explicit non-empty VAL/TEST guards."""
    positions = np.asarray(positions, dtype=int)
    test_frac = 1.0 - float(train_frac) - float(val_frac)
    if not (0.0 < float(train_frac) < 1.0):
        raise ValueError(f"v4.59.1 invalid train_frac={train_frac}; expected 0 < train_frac < 1")
    if not (0.0 < float(val_frac) < 1.0):
        raise ValueError(
            f"v4.59.1 requires an independent validation split, but val_frac={val_frac}. "
            "Set Tier-3 val_ratio > 0 (release default is 0.15)."
        )
    if test_frac <= 0.0:
        raise ValueError(
            f"v4.59.1 invalid fractions: train_frac={train_frac}, val_frac={val_frac}, "
            f"leaving test_frac={test_frac}; require train+val < 1."
        )
    if mode == "row":
        return _split_positions_stratified(rows, positions, TIER3_TRAIN_CONFIG["target"], train_frac, val_frac, seed)
    from sklearn.model_selection import GroupShuffleSplit
    groups = _tier3_group_labels(rows, positions, mode)
    n_groups = len(np.unique(groups))
    if n_groups < 3:
        log.warning("v4.59.1 GROUP-SPLIT mode=%s has only %d groups; falling back to row split", mode, n_groups)
        return _split_positions_stratified(rows, positions, TIER3_TRAIN_CONFIG["target"], train_frac, val_frac, seed)
    log.info(
        "v4.59.1 GROUP-SPLIT mode=%s rows=%d unique_groups=%d fractions train=%.3f val=%.3f test=%.3f",
        mode, len(positions), n_groups, train_frac, val_frac, test_frac,
    )
    g1 = GroupShuffleSplit(n_splits=1, train_size=train_frac, random_state=seed)
    tr_i, tmp_i = next(g1.split(positions, groups=groups))
    temp = positions[tmp_i]; temp_groups = groups[tmp_i]
    n_temp_groups = len(np.unique(temp_groups))
    if n_temp_groups < 2:
        log.warning(
            "v4.59.1 GROUP-SPLIT mode=%s left only %d held-out group after train split; "
            "falling back to deterministic row split", mode, n_temp_groups
        )
        return _split_positions_stratified(rows, positions, TIER3_TRAIN_CONFIG["target"], train_frac, val_frac, seed)
    val_share = val_frac / (val_frac + test_frac)
    if not (0.0 < val_share < 1.0):
        raise ValueError(f"v4.59.1 invalid held-out val_share={val_share}")
    g2 = GroupShuffleSplit(n_splits=1, train_size=val_share, random_state=seed+1)
    va_i, te_i = next(g2.split(temp, groups=temp_groups))
    train_out, val_out, test_out = positions[tr_i], temp[va_i], temp[te_i]
    if min(len(train_out), len(val_out), len(test_out)) == 0:
        raise RuntimeError(
            f"v4.59.1 produced an empty split: train={len(train_out)} val={len(val_out)} test={len(test_out)}"
        )
    return train_out, val_out, test_out


def _assert_disjoint_positions(train_pos, val_pos, test_pos, rows, group_mode):
    sets = [set(map(int, x)) for x in (train_pos, val_pos, test_pos)]
    if sets[0] & sets[1] or sets[0] & sets[2] or sets[1] & sets[2]:
        raise RuntimeError("v4.59 SPLIT-LEAK: row positions overlap across train/val/test")
    if group_mode != "row":
        gs = [set(_tier3_group_labels(rows, np.asarray(x,dtype=int), group_mode).tolist()) for x in (train_pos,val_pos,test_pos)]
        if gs[0] & gs[1] or gs[0] & gs[2] or gs[1] & gs[2]:
            raise RuntimeError(f"v4.59 GROUP-LEAK: {group_mode} groups overlap across train/val/test")
    log.info("v4.59 SPLIT-SANITY PASS: row-disjoint and %s-group-disjoint; train=%d val=%d test=%d", group_mode, len(train_pos), len(val_pos), len(test_pos))
...
def build_tier3_hfo2_eval_dataloader(
    ...
    group_mode = str(TIER3_TRAIN_CONFIG.get("tier3_split_group_mode", "donor_material"))
    loaded_test_pos = load_tier3_positions_from_split(valid_rows, split_path, "test") if use_saved_test_split else None
    loaded_val_pos = load_tier3_positions_from_split(valid_rows, split_path, "val") if use_saved_test_split else None
    loaded_train_pos = load_tier3_positions_from_split(valid_rows, split_path, "train") if use_saved_test_split else None

    if loaded_test_pos is not None and loaded_val_pos is not None and loaded_train_pos is not None:
        test_pos, val_pos, train_pos = loaded_test_pos.astype(int), loaded_val_pos.astype(int), loaded_train_pos.astype(int)
        eval_train_pos = np.asarray([p for p in train_pos if p in set(eval_pos.tolist())], dtype=int)
        log.info("v4.59 PERSISTENT-3WAY-SPLIT active: train=%d val=%d test=%d", len(train_pos), len(val_pos), len(test_pos))
    else:
        eval_train_pos, val_pos, test_pos = _split_positions_grouped(
            valid_rows, eval_pos, train_frac, val_frac, seed=42, mode=group_mode
        )
        train_pos = np.concatenate([non_eval_pos, eval_train_pos]).astype(int)
        # Remove any auxiliary/weak rows whose held-out identity appears in VAL/TEST.
        # This closes the subtle path where a non-eval row could expose the same
        # material/donor identity used by a headline evaluation row.
        if group_mode != "row":
            heldout_groups = set(_tier3_group_labels(valid_rows, np.concatenate([val_pos, test_pos]), group_mode).tolist())
            train_groups = _tier3_group_labels(valid_rows, train_pos, group_mode)
            keep = np.asarray([g not in heldout_groups for g in train_groups], dtype=bool)
            removed = int((~keep).sum())
            train_pos = train_pos[keep]
            eval_train_pos = np.asarray([p for p in eval_train_pos.tolist() if p in set(train_pos.tolist())], dtype=int)
            log.info("v4.59 GROUP-HOLDOUT-FILTER mode=%s removed_train_rows=%d", group_mode, removed)
        if len(val_pos) == 0 or len(test_pos) == 0:
            raise RuntimeError("v4.59 requires non-empty independent VAL and TEST sets")
        if save_test_split:
            for _name, _pos in [("train", train_pos), ("val", val_pos), ("test", test_pos)]:
                save_tier3_split_rows(dataset, valid_rows, _pos, split_path, split_name=_name, overwrite=True)
    _assert_disjoint_positions(train_pos, val_pos, test_pos, valid_rows, group_mode)
...
def _tier3_process_context_mask(df: pd.DataFrame) -> pd.Series:
    """True for rows carrying at least one usable Tier-3 process feature."""
    cols = (list(PROCESS_PARAMS_FEATURES.get("numerical", [])) +
            list(PROCESS_PARAMS_FEATURES.get("categorical", {}).keys()))
    ...
    for c in present:
        s = df[c]
        if c in PROCESS_PARAMS_FEATURES.get("categorical", {}):
            valid = s.notna() & s.astype(str).str.strip().str.lower().notin(["", "nan", "none", "n/a", "-"])
        else:
...
def _is_hfzr_oxide_formula(value: Any) -> bool:
    s = str(value) if value is not None else ""
    return "O" in s and ("Hf" in s or "Zr" in s)
...
def run_tier2_finetune(
    ...
    # -- Phase C: low-LR Hf/Zr DFT specialization -------------------------
    # Preserve the broad Phase-B checkpoint for direct comparison and use it
    # as the starting point for the focused Hf/Zr specialization.
    phase_b_best = CKPT_ROOT / "tier2_best.pt"
    phase_b_saved = CKPT_ROOT / "tier2_phase_b_best.pt"
    dist_barrier()
    if is_rank0() and phase_b_best.exists():
        import shutil
        shutil.copy2(phase_b_best, phase_b_saved)
        log.info("Tier 2 Phase B checkpoint preserved -> %s", phase_b_saved)
    dist_barrier()

    if cfg.get("phase_c_hfzr_specialization", False):
        _k_num_c = pd.to_numeric(df_tier2.get("k_total"), errors="coerce")
        _has_struct_c = (df_tier2.get("atoms_dict", pd.Series(index=df_tier2.index, dtype=object)).notna() &
                         df_tier2.get("has_structure", pd.Series(False, index=df_tier2.index)).fillna(False).astype(bool))
        _is_dft_c = ~df_tier2.get("source", pd.Series("", index=df_tier2.index)).astype(str).eq("Experimental")
        _is_hfzr_c = df_tier2.get("formula", pd.Series("", index=df_tier2.index)).apply(_is_hfzr_oxide_formula)
        df_t2_hfzr = df_tier2[_k_num_c.notna() & _has_struct_c & _is_dft_c & _is_hfzr_c].copy().reset_index(drop=True)
        df_t2_hfzr[target_col] = np.log(pd.to_numeric(df_t2_hfzr["k_total"], errors="coerce").clip(lower=0.1))
        log.info(
            "Tier 2 Phase C: Hf/Zr DFT specialization candidates=%d  min_required=%d",
            len(df_t2_hfzr), int(cfg.get("phase_c_min_rows", 12)),
        )
        log.info(
            "T2-PHASE-C-AUDIT rows=%d experimental=%d k_valid=%d structure_valid=%d unique_row_hash=%d",
            len(df_t2_hfzr),
            int(df_t2_hfzr.get("source", pd.Series(index=df_t2_hfzr.index, dtype=object)).astype(str).eq("Experimental").sum()),
            int(df_t2_hfzr[target_col].notna().sum()),
            int((df_t2_hfzr["atoms_dict"].notna() & df_t2_hfzr["has_structure"].fillna(False).astype(bool)).sum()) if len(df_t2_hfzr) else 0,
            int(df_t2_hfzr["row_hash"].nunique()) if "row_hash" in df_t2_hfzr.columns else len(df_t2_hfzr),
        )
        if len(df_t2_hfzr) >= int(cfg.get("phase_c_min_rows", 12)):
            pc_train, pc_val, pc_test, pc_sampler, _pc_ds = build_dataloader(
                df=df_t2_hfzr, target_col=target_col, aux_cols=cfg["aux_targets"],
                train_frac=float(cfg.get("phase_c_train_ratio", 0.80)),
                val_frac=float(cfg.get("phase_c_val_ratio", 0.10)),
                batch_size=min(int(cfg["batch_size"]), max(4, len(df_t2_hfzr))),
            )
            ckpt_pb = safe_load_checkpoint(phase_b_saved)
            if ckpt_pb is None:
                raise RuntimeError(f"Tier 2 Phase C cannot load Phase-B checkpoint: {phase_b_saved}")
            trainer.model_core.load_state_dict(_remap_state_dict(ckpt_pb["model_state_dict"]), strict=False)
            trainer.optimizer = torch.optim.AdamW(
                [p for p in trainer.model_core.parameters() if p.requires_grad],
                lr=float(cfg.get("phase_c_learning_rate", 2e-5)),
                weight_decay=float(cfg.get("phase_c_weight_decay", cfg["weight_decay"])),
            )
            cfg_pc = {**cfg, "epochs": int(cfg.get("phase_c_epochs", 20)),
                      "learning_rate": float(cfg.get("phase_c_learning_rate", 2e-5)),
                      "early_stopping": False, "min_epochs": 0}
            trainer.cfg = cfg_pc
            trainer.ckpt_prefix = "tier2_hfzr"
            trainer.train(pc_train, pc_val, target_col=target_col, train_sampler=pc_sampler)
            phase_c_best = CKPT_ROOT / "tier2_hfzr_best.pt"
            dist_barrier()
            if not phase_c_best.exists():
                raise RuntimeError(f"Tier 2 Phase C did not create checkpoint: {phase_c_best}")
            if is_rank0():
                import shutil
                shutil.copy2(phase_c_best, phase_b_best)
                log.info("Tier 2 Phase C promoted -> %s (source=%s)", phase_b_best, phase_c_best)
            dist_barrier()
            trainer.ckpt_prefix = "tier2"
            trainer.cfg = cfg
        else:
            log.warning("Tier 2 Phase C skipped: only %d Hf/Zr rows (<%d)", len(df_t2_hfzr), int(cfg.get("phase_c_min_rows", 12)))
...
def run_tier3_finetune(
    ...
    # v4.59.2: Tier 3 is experimental-only. Native DFT rows were moved to
    # Tier-2 Phase C and must not consume Tier-3 optimization steps. Keep only
    # experimental rows with a native/imputed structure and, when requested,
    # at least one real process parameter.
    if cfg.get("experimental_only", False):
        _before_exp_only = len(df_structural)
        _src_exp = df_structural.get("source", pd.Series("", index=df_structural.index)).astype(str).eq("Experimental")
        _struct_ok = (df_structural["atoms_dict"].notna() &
                      df_structural.get("has_structure", pd.Series(False, index=df_structural.index)).fillna(False).astype(bool))
        _proc_ok = _tier3_process_context_mask(df_structural)
        _keep = _src_exp & _struct_ok
        if cfg.get("require_process_context", True):
            _keep &= _proc_ok
        df_structural = df_structural[_keep].copy().reset_index(drop=True)
        log.info(
            "T3-EXPERIMENTAL-ONLY input=%d retained=%d native_dft_retained=%d "
            "with_structure=%d process_valid=%d dropped=%d",
            _before_exp_only, len(df_structural),
            int((~df_structural.get("source", pd.Series("Experimental", index=df_structural.index)).astype(str).eq("Experimental")).sum()),
            int((df_structural["atoms_dict"].notna() & df_structural["has_structure"].fillna(False).astype(bool)).sum()),
            int(_tier3_process_context_mask(df_structural).sum()),
            _before_exp_only - len(df_structural),
        )
        if len(df_structural) == 0:
            raise RuntimeError("Tier 3 experimental-only filter produced zero trainable rows")
    ...
    if cfg.get("eval_hfo2_family_only", False):
        train_loader, val_loader, test_loader, train_sampler, _main_ds_t3 = _tier3_loader_fn(
            ...
            val_frac   = cfg["val_ratio"],
            batch_size = cfg["batch_size"],
            # v4.34: normal finetune saves the split; ablation reuses it.
            use_saved_test_split = bool(ablate_context and TIER3_TEST_SPLIT_PATH.exists()),
            save_test_split = bool(not ablate_context),
            overwrite_saved_split = bool(not ablate_context),
        )
    else:
        train_loader, val_loader, test_loader, train_sampler, _main_ds_t3 = _tier3_loader_fn(
            df         = df_phase_b,
    ...
    if cfg.get("eval_hfo2_family_only", False) and TIER3_TEST_SPLIT_PATH.exists():
        ...
        _raw_val_pos = list(getattr(val_loader.dataset, "indices", []))
        _schema_by_pos = {}
        ...
        # Canonicalize TEST and VAL independently; never reuse TEST for VAL.
        _graph_ok_val_pos = []
        for _p in _raw_val_pos:
            try:
                _item = _main_ds_t3[int(_p)]
            except BaseException:
                _item = None
            if _audit_schema_key(_item) == _canonical_schema:
                _graph_ok_val_pos.append(int(_p))
        if not _graph_ok_val_pos:
            raise RuntimeError("v4.59: no schema-compatible validation rows remain")
        _eval_loader_kwargs = dict(batch_size=cfg["batch_size"], shuffle=False, num_workers=0, collate_fn=HighKGraphDataset.collate_fn, pin_memory=torch.cuda.is_available())
        val_loader = DataLoader(Subset(_main_ds_t3, _graph_ok_val_pos), **_eval_loader_kwargs)
        test_loader = DataLoader(Subset(_main_ds_t3, _graph_ok_test_pos), **_eval_loader_kwargs)

        # Only rank 0 writes the shared split file; all ranks synchronize after.
        if is_rank0():
            _valid_rows_for_split = _main_ds_t3.df.loc[_main_ds_t3.valid_idx].reset_index(drop=True)
            save_tier3_split_rows(_main_ds_t3, _valid_rows_for_split, np.asarray(_graph_ok_val_pos,dtype=int), path=TIER3_TEST_SPLIT_PATH, split_name="val", overwrite=True)
            save_tier3_split_rows(_main_ds_t3, _valid_rows_for_split, np.asarray(_graph_ok_test_pos,dtype=int), path=TIER3_TEST_SPLIT_PATH, split_name="test", overwrite=True)
        dist_barrier()

        _canonical_test_n = len(_graph_ok_test_pos)
        _canonical_val_n = len(_graph_ok_val_pos)
        log.info(
            "v4.43 CANONICAL-SPLIT locked on one DGL schema: "
            "train=%d val=%d test=%d; all audits/checkpoints must report N=%d",
            len(getattr(train_loader.dataset, "indices", [])),
            _canonical_val_n, _canonical_test_n, _canonical_test_n,
        )
        _canonical_val_n = len(_graph_ok_val_pos)
        log.info(
            "v4.42 CANONICAL-SPLIT locked on GRAPH-EVALUABLE rows: "
            "train=%d val=%d test=%d; all audits/checkpoints must report N=%d",
            len(getattr(train_loader.dataset, "indices", [])),
            _canonical_val_n, _canonical_test_n, _canonical_test_n,
        )
    ...
    def _eval_exact_k_for_diag(loader, split_name: str, stage_name: str) -> dict:
        ...
        if split_name.upper() in ("VAL", "TEST") and out["n"] != _canonical_test_n:
            raise RuntimeError(
                f"v4.43 AUDIT COUNT MISMATCH at {stage_name}/{split_name}: "
                f"observed N={out['n']} expected N={_canonical_test_n}"
            )
        log.info(
    ...
    def _write_tier3_prediction_audit(
        ...
        if safe_split in ("test", "val"):
            if len(audit_df) != _canonical_test_n:
                raise RuntimeError(
                    f"v4.41 prediction audit count mismatch {safe_stage}/{safe_split}: "
                    f"{len(audit_df)} != {_canonical_test_n}"
                )
    ...
    def _write_tier3_dataset_composition_audit():
        ...
        # VAL and TEST reuse the same rows. Because split_map stores one label per
        # row and TEST takes precedence, composition intentionally reports TEST
        # only for the held-out population.
    ...
    _write_tier3_prediction_audit(test_loader, _main_ds_t3.df, "TEST", "epoch0_pretrain")
    ...
    if freeze_backbone:
        ...
        if len(getattr(pb_val_loader.dataset, "indices", [])) != _canonical_test_n:
            raise RuntimeError(
                "v4.41 Phase-B validation split differs from canonical TEST split"
            )
        ...
        for audit_epoch in audit_epochs:
            ...
            _eval_exact_k_for_diag(test_loader, "TEST", stage_a)
            _write_tier3_prediction_audit(val_loader, _main_ds_t3.df, "VAL", stage_a)
            _write_tier3_prediction_audit(test_loader, _main_ds_t3.df, "TEST", stage_a)
    ...
    _write_tier3_prediction_audit(val_loader, _main_ds_t3.df, "VAL", "best_checkpoint")
...
def run_tier_evaluate(
    ...
    if tier == 1:
        ...
        # v4.59.2: reproduce the same experimental-only population used by
        # tier3_finetune before applying the saved identity split.
        if cfg.get("experimental_only", False):
            _eval_before = len(df_eval)
            _src_exp = df_eval.get("source", pd.Series("", index=df_eval.index)).astype(str).eq("Experimental")
            _struct_ok = (df_eval["atoms_dict"].notna() &
                          df_eval.get("has_structure", pd.Series(False, index=df_eval.index)).fillna(False).astype(bool))
            _proc_ok = _tier3_process_context_mask(df_eval)
            _keep = _src_exp & _struct_ok
            if cfg.get("require_process_context", True):
                _keep &= _proc_ok
            df_eval = df_eval[_keep].copy().reset_index(drop=True)
            log.info(
                "T3-EVAL-EXPERIMENTAL-ONLY input=%d retained=%d native_dft_retained=%d process_valid=%d",
                _eval_before, len(df_eval),
                int((~df_eval.get("source", pd.Series("Experimental", index=df_eval.index)).astype(str).eq("Experimental")).sum()),
                int(_tier3_process_context_mask(df_eval).sum()),
            )
            if len(df_eval) == 0:
                raise RuntimeError("Tier 3 evaluate experimental-only filter produced zero rows")
    ...
    if tier == 3 and cfg.get("eval_hfo2_family_only", False):
        ...
        log.info("Test loader: %d batches  (v4.35 persistent Tier3 test split when available)", len(test_loader))
    else:
    ...
    # v4.59 RELEASE: read runtime architecture metadata before constructing the
    ...
    if tier == 3:
        ...
        log.info("v4.59 EVAL-ADAPTER-CONFIG: restored enabled=%s rank=%d from checkpoint metadata",
    ...
    if tier == 3 and _adapter_enabled:
        ...
        if not _adapter_saved:
            raise RuntimeError(
                "v4.59 EVAL CHECKPOINT ERROR: structural adapter is enabled but the checkpoint "
                "contains no tier3_structural_adapter tensors. Refusing to report a process-only result."
            )
        if _adapter_missing:
            raise RuntimeError(
                "v4.59 EVAL CHECKPOINT ERROR: adapter tensors were not restored: " + str(_adapter_missing)
            )
...
def main():
    ...
    parser.add_argument("--tier3_split_group_mode", choices=["row","material","donor","donor_material","paper"], default=None)
    ...
    if args.tier3_split_group_mode is not None:
        TIER3_TRAIN_CONFIG["tier3_split_group_mode"] = args.tier3_split_group_mode
