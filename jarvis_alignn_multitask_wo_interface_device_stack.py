...
class MultiTaskALIGNN(nn.Module):
    ...
    def forward(self, batch: dict) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        h_graph = self.pool_proj(
            torch.cat([h_avg, h_sum, h_proc], dim=-1)               # [B, H*2 + 64]
        )                                                        # [B, H]

        # ── Task heads ────────────────────────────────────────────────────────
        bandgap_pred = self.bandgap_act(self.bandgap_head(h_graph))
        highk_logits = self.highk_head(h_graph)
        eps_pred     = self.eps_act(self.eps_head(h_graph))
...
def main():
    ...
    # ── 3. Load optional process conditions CSV ───────────────────────────────
    ...
    if process_df is not None:
        ...
        train_process_matrix = train_dataset.get_process_matrix()  # [N_train, PROCESS_DIM]
        process_scaler.fit(train_process_matrix)
        ...
        scaler_path = Path(CFG.results_dir) / "process_scaler.joblib"
        joblib.dump(process_scaler, scaler_path)
        logger.info(f"Process scaler scaler fitted and saved to {scaler_path}")
