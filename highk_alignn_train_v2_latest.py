...
==============================================================================
 ...
 MAD:MAE  All three tier test outputs now print full table including
          MAD:MAE ratio and paper benchmark reference (▼/▲ flags).
...
PAPER_MAD_MAE_BENCHMARK = {
    ...
    "k_measured":                 1.63,   # JARVIS-DFT ε DFPT elec+ionic
    "J_g_log":                    None,
    "E_BD":                       None,
}
...
def get_stratified_split(
    ...
    valid_mask = ~pd.isnan(target_vals)
...
class HighKALIGNN(nn.Module):
    ...
    def forward(
        ...
        embedding = self.backbone(graph, line_graph, None)   # (batch, hidden_features)
...
class ALIGNNTrainer:
    ...
    def print_multitask_results(
        ...
        ▼ = below paper reference   ▲ = beating paper reference
        ...
        ═══════════════════════════════════════════════════════════════════
          ...
          k_measured         │ 18.3214   │ 24.5621   │    1.51 │    1.63 ▼ │   12,345 │  27.7%
        ...
        for task_name, m in results.items():
            ...
            if paper_ref is not None and n > 0 and not np.isnan(m.get("mad_mae_ratio", float("nan"))):
                ...
                flag    = " ▼" if gap_pct > 5 else (" ▲" if gap_pct < -5 else "  ")
...
def run_tier2_finetune(
    ...
    test_mae, test_rmse = trainer.evaluate(test_loader, cfg["target"])
    log.info("Tier 2 TEST  MAE=%.4f  RMSE=%.4f  (target: k_measured)",
             test_mae, test_rmse)
