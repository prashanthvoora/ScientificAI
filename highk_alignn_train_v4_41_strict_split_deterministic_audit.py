...
def run_tier3_finetune(
    ...
    if cfg.get("eval_hfo2_family_only", False) and TIER3_TEST_SPLIT_PATH.exists():
        ...
        if _canonical_test_n <= 0 or _canonical_val_n != _canonical_test_n:
            raise RuntimeError(
                f"v4.41 canonical Tier3 split invalid: val={_canonical_val_n} test={_canonical_test_n}"
            )
        log.info(
            "v4.41 CANONICAL-SPLIT locked: train=%d val=%d test=%d; "
            "all audits/checkpoints must report N=%d",
    ...
    def _eval_exact_k_for_diag(loader, split_name: str, stage_name: str) -> dict:
        ...
        if split_name.upper() in ("VAL", "TEST") and out["n"] != _canonical_test_n:
            raise RuntimeError(
                f"v4.41 AUDIT COUNT MISMATCH at {stage_name}/{split_name}: "
    ...
    def _write_tier3_dataset_composition_audit():
        ...
        if observed_test != _canonical_test_n:
            raise RuntimeError(
                f"v4.41 composition TEST count {observed_test} != canonical {_canonical_test_n}"
            )
