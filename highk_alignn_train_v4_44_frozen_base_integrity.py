...
==============================================================================
 ...
 Version 4.5.37 (v4.37: final finetune reloads persistent Tier-3 split)
==============================================================================
...
TIER3_TRAIN_CONFIG = dict(
    ...
    frozen_base_state_tolerance = 1e-8,  # v4.44 hard guard for parameters + buffers
...
class HighKALIGNN(nn.Module):
    ...
    def freeze_for_bounded_process_residual(self, train_base_head: bool = False):
        ...
        v4.44: freezing parameters is not sufficient because ``model.train()``
        ...
        log.info(
            "v4.44 RESIDUAL-PROC freeze: frozen base parameters+buffers protected by eval mode; "
...
class ALIGNNTrainer:
    ...
    def rebuild_optimizer_for_current_trainables(self):
        """Rebuild AdamW after residual-only freezing (v4.44).
        ...
        if not params:
            raise RuntimeError("v4.44: no trainable parameters after residual freeze")
        self.optimizer = torch.optim.AdamW(
            params,
        ...
        self._write_optimizer_audit_manifest()
        log.info(
            "v4.44 OPTIMIZER-REBUILD: trainable_tensors=%d trainable_params=%d",
    ...
    def capture_frozen_base_reference(self):
        ...
        log.info(
            "v4.44 BASE-SNAPSHOT: captured %d frozen parameters/buffers",
    ...
    def _write_optimizer_audit_manifest(self):
        ...
        for name, p in self.model_core.named_parameters():
            if name.startswith("process_delta_head") or p.requires_grad:
                lr = float("nan")
        ...
        delta_rows = [r for r in rows if r["parameter"].startswith("process_delta_head")]
        log.info(
            "v4.40 AUDIT optimizer manifest: residual_params=%d all_in_optimizer=%s path=%s",
    ...
    def train_epoch(
        ...
        if _residual_only_mode:
            # v4.44: keep the whole frozen base in eval mode for BOTH normal
