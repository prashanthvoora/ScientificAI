...
def run_tier3_finetune(
    ...
    if cfg.get("eval_hfo2_family_only", False) and TIER3_TEST_SPLIT_PATH.exists():
        ...
        # v4.42 GRAPH-EVALUABLE-SPLIT FIX:
        # HighKGraphDataset.valid_idx only guarantees target + atoms_dict +
        # has_structure. Some rows can still fail DGL graph construction or
        # schema validation in __getitem__, and collate_fn silently drops them.
        # The old canonical count therefore used raw Subset positions (e.g. 37)
        # while evaluate() observed only graph-evaluable rows (e.g. 29).
        # Materialize the held-out positions once, retain only rows for which
        # __getitem__ returns a valid graph sample, rebuild VAL/TEST from those
        # exact positions, and overwrite the persistent split with the same
        # graph-evaluable population. All later flows then use N=29 consistently.
        ...
        for _p in _raw_test_pos:
            ...
            if _item is None:
                ...
                _graph_ok_test_pos.append(int(_p))

        if not _graph_ok_test_pos:
            raise RuntimeError(
                "v4.42: no graph-evaluable rows remain in canonical Tier3 TEST split"
            )

        if len(_graph_ok_test_pos) != len(_raw_test_pos):
            log.warning(
                "v4.42 GRAPH-EVALUABLE-SPLIT: removed %d/%d held-out rows "
                "that fail graph construction/schema validation; canonical N=%d",
                len(_graph_failed_test_pos), len(_raw_test_pos),
                len(_graph_ok_test_pos),
            )
            log.info(
                "v4.42 GRAPH-EVALUABLE-SPLIT failed subset positions: %s",
                _graph_failed_test_pos,
            )

        # VAL intentionally reuses TEST in Tier3 train/test-only diagnostics.
        ...
        # Rewrite the persistent split so tier3_evaluate and ablation load only
        # graph-evaluable held-out rows rather than the larger target-valid set.
    ...
    def _eval_exact_k_for_diag(loader, split_name: str, stage_name: str) -> dict:
        ...
        if split_name.upper() in ("VAL", "TEST") and out["n"] != _canonical_test_n:
            raise RuntimeError(
                f"v4.42 AUDIT COUNT MISMATCH at {stage_name}/{split_name}: "
