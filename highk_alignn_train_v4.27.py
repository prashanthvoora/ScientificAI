...
==============================================================================
 ...
 Version 4.5.24 (v4.27: tier3_evaluate uses df_structural as fallback donor pool)
==============================================================================
    ...
    if tier == 2 and cfg.get("log_transform", False):
        ...
        if cfg.get("log_transform", False):
            ...
            if n_v > 0:
                ...
                mae_k_lin  = (p_lin - t_lin).abs().mean().item()
                rmse_k_lin = ((p_lin - t_lin)**2).mean().sqrt().item()
                t_np       = t_lin.cpu().numpy()
                mad_k_lin  = float(np.median(np.abs(t_np - np.median(t_np))))
                mad_mae    = mad_k_lin / mae_k_lin if mae_k_lin > 0 else float("nan")
                log.info("  Diagnostic log-space  MAE  = %.4f  [log(k) units]", test_mae_log)
                log.info("  Diagnostic log-space  RMSE = %.4f", test_rmse_log)
                log.info("  PRIMARY (linear-k exact)  MAE  = %.4f  [dielectric units]", mae_k_lin)
                log.info("  PRIMARY (linear-k exact)  RMSE = %.4f  [dielectric units]", rmse_k_lin)
                log.info("  PRIMARY (linear-k exact)  MAD  = %.4f  → MAD:MAE = %.2f  (N=%d)",
                         mad_k_lin, mad_mae, n_v)
                if mad_mae >= 2.5:
                    log.info("  STATUS: ✓ PROJECT GOAL MET  (MAD:MAE %.2f ≥ 2.5)", mad_mae)
                elif mad_mae >= 1.63:
                    log.info("  STATUS: ✓ PAPER BENCHMARK MET  (MAD:MAE %.2f ≥ 1.63)", mad_mae)
                else:
                    log.info("  STATUS: ✗ BELOW PAPER BENCHMARK  (MAD:MAE %.2f < 1.63)", mad_mae)
            else:
                log.warning("  No valid k_total_log predictions (n_valid=0)")
        else:
            log.info("  k_total_log  MAE  = %.4f  [dielectric units]", test_mae_log)
            log.info("  k_total_log  RMSE = %.4f  [dielectric units]", test_rmse_log)
