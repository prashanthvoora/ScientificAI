#!/usr/bin/env python3
"""Tier-3 frozen-base integrity and metric-reconciliation summary generator (v4.46).
...
def optimizer_summary(audit_dir: Path, run_type: str) -> pd.DataFrame:
    ...
    if df is None or df.empty:
        return pd.DataFrame()
    param = df["parameter"].astype(str)
    residual = df[param.str.startswith("process_delta_head")]
    categorical = df[param.str.startswith("proc_encoder.embeddings")]

    def _all_bool(g: pd.DataFrame, col: str) -> bool:
        return bool(len(g) and g[col].astype(bool).all()) if col in g else False

    cat_enabled = bool(len(categorical) and categorical["requires_grad"].astype(bool).any())
    return pd.DataFrame([{
        "run_type": run_type,
        "residual_parameter_count": int(residual["numel"].sum()),
        "all_residual_requires_grad": _all_bool(residual, "requires_grad"),
        "all_residual_in_optimizer": _all_bool(residual, "in_optimizer"),
        "residual_lr_values": ",".join(sorted(set(residual["optimizer_lr"].astype(str)))) if len(residual) else "",
        "categorical_enabled": cat_enabled,
        "categorical_embedding_tensor_count": int(len(categorical)),
        "categorical_embedding_parameter_count": int(categorical["numel"].sum()) if len(categorical) else 0,
        "all_categorical_requires_grad": _all_bool(categorical, "requires_grad") if cat_enabled else True,
        "all_categorical_in_optimizer": _all_bool(categorical, "in_optimizer") if cat_enabled else True,
        "categorical_lr_values": ",".join(sorted(set(categorical["optimizer_lr"].astype(str)))) if len(categorical) else "",
    }])
...
def integrity_checks(base: Path, metrics: pd.DataFrame, history: pd.DataFrame, freeze_integrity: pd.DataFrame) -> pd.DataFrame:
    ...
    # v4.46 categorical embedding optimizer and learning checks.
    for run_type in ("normal", "ablation"):
        run_dir = base / run_type
        manifest_path = run_dir / "tier3_optimizer_parameter_manifest.csv"
        manifest = _safe_read_csv(manifest_path) if manifest_path.exists() else None
        cat = pd.DataFrame() if manifest is None else manifest[
            manifest["parameter"].astype(str).str.startswith("proc_encoder.embeddings")
        ]
        cat_enabled = bool(not cat.empty and cat["requires_grad"].astype(bool).any())
        checks.append({
            "check": f"categorical_embeddings_optimizer_membership:{run_type}",
            "passed": bool((not cat_enabled) or (cat["requires_grad"].astype(bool).all() and cat["in_optimizer"].astype(bool).all())),
            "detail": f"enabled={cat_enabled} tensors={len(cat)} params={int(cat['numel'].sum()) if not cat.empty else 0}",
        })
        rh = history[history["run_type"].eq(run_type)] if not history.empty and "run_type" in history else pd.DataFrame()
        if cat_enabled:
            change = pd.to_numeric(rh.get("categorical_embedding_parameter_change_from_init"), errors="coerce").dropna() if not rh.empty else pd.Series(dtype=float)
            grad = pd.to_numeric(rh.get("categorical_embedding_grad_norm"), errors="coerce").dropna() if not rh.empty else pd.Series(dtype=float)
            changed = pd.to_numeric(rh.get("categorical_embedding_changed_tensors"), errors="coerce").dropna() if not rh.empty else pd.Series(dtype=float)
            learned = bool(len(change) and change.max() > 0 and len(grad) and grad.max() > 0 and len(changed) and changed.max() > 0)
            checks.append({
                "check": f"categorical_embeddings_learned:{run_type}",
                "passed": learned,
                "detail": (f"max_grad={grad.max():.3e} max_l2_change={change.max():.3e} "
                           f"max_changed_tensors={int(changed.max())}" if len(change) and len(grad) and len(changed) else "missing trajectory columns"),
            })
...
def write_markdown(path: Path, integrity: pd.DataFrame, metrics: pd.DataFrame,
    ...
    lines: List[str] = ["# Tier-3 Frozen-Base Process-Residual Audit Summary v4.46", ""]
    ...
    if not history.empty and any(c.startswith("categorical_embedding_") for c in history.columns):
        cat_cols = [c for c in [
            "run_type", "epoch", "categorical_embedding_grad_norm",
            "categorical_embedding_parameter_count", "categorical_embedding_parameter_norm",
            "categorical_embedding_parameter_change_from_init",
            "categorical_embedding_max_abs_change_from_init",
            "categorical_embedding_changed_tensors",
        ] if c in history.columns]
        cat_hist = history[cat_cols].copy()
        lines += ["## Categorical embedding learning audit", "", cat_hist.to_markdown(index=False), ""]
    ...
    if not history.empty:
        keep = [c for c in ["run_type", "epoch", "val_mae", "val_rmse", "process_delta_grad_norm", "process_delta_parameter_norm", "process_delta_parameter_change_from_init", "process_delta_mean_abs", "process_delta_max_abs",
                           "categorical_embedding_grad_norm", "categorical_embedding_parameter_count",
                           "categorical_embedding_parameter_norm", "categorical_embedding_parameter_change_from_init",
                           "categorical_embedding_max_abs_change_from_init", "categorical_embedding_changed_tensors",
                           "frozen_base_max_abs_change", "frozen_base_l2_change", "frozen_base_changed_tensors"] if c in history.columns]
