...
def run_tier3_finetune(
    ...
    if cfg.get("eval_hfo2_family_only", False) and TIER3_TEST_SPLIT_PATH.exists():
        ...
        if len(_graph_ok_test_pos) != len(_raw_test_pos) or len(_raw_test_pos) != _saved_test_count:
            raise RuntimeError(
                "BASELINE-SPLIT-LOCK: persistent TEST is not fully graph/schema evaluable "
                f"(saved={_saved_test_count}, restored={len(_raw_test_pos)}, "
                f"canonical={len(_graph_ok_test_pos)}). Refusing to rewrite TEST identities."
            )
        dist_barrier()
