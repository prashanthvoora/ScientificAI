...
TIER3_TRAIN_CONFIG = dict(
    ...
    # v4.33: LR/scheduler tuned for pretrained task_head on small dataset.
    # 5e-6 + OneCycleLR warmup prevents overshoot of Tier2-pretrained weights.
    learning_rate  = 5e-6,
    weight_decay   = 1e-4,
    scheduler      = "onecycle",
    pct_start      = 0.3,
    ...
    # v4.33: Phase B2 config — proc_encoder-only warmup on experimental rows.
    # Backbone and task_heads are FROZEN in Phase B2; only proc_encoder,
    # stack_encoder, context_proj, alpha, beta are trained.
    # Phase B2 uses a separate dataloader (experimental rows only).
    unfreeze_backbone_lr = 1e-6,   # backbone LR if B2 unfreezes backbone (kept for reference)
    phase_b2_lr          = 1e-5,   # proc_encoder LR in Phase B2 (higher: starting from random init)
    phase_b2_epochs      = 100,    # Phase B2 epoch budget for proc_encoder warmup

    # v4.33: exp_sample_weight no longer phase-split.
    # Phase B trains on DFT rows only (no experimental rows in the loader).
    # Phase B2 trains on experimental rows only (no DFT rows in the loader).
    # Disjoint data splits make per-phase weighting unnecessary.
    exp_sample_weight = 1.0,   # applies within each phase's dataloader (uniform)
...
class HighKALIGNN(nn.Module):
    ...
    def freeze_backbone(self):
        """Freeze ALIGNN backbone for Phase B DFT-only fine-tuning.

        Phase B trains on DFT HfO2-family rows only.  All DFT rows have
        avail_flag=0 so proc_encoder and stack_encoder receive zero gradient
        from data regardless of requires_grad.  They are left trainable to
        avoid complexity — they are effectively frozen by the data.

        Frozen  : backbone (all ALIGNN message-passing layers)
        Trainable: task_heads, context_proj, proc_encoder, stack_encoder,
                   alpha, beta, func_embedding, func_proj
        """
        for param in self.backbone.parameters():
            param.requires_grad = False
        ...
        for param in self.func_proj.parameters():
            param.requires_grad = True
        log.info(" Phase B frozen  : ALIGNN backbone")
        log.info(" Phase B trainable: task_heads, context_proj, alpha, beta, func_embedding (proc/stack frozen by data)")

    def freeze_for_proc_warmup(self):
        """Freeze backbone AND task_heads for Phase B2 proc_encoder warmup.

        Phase B2 trains on experimental rows only.  The backbone and task_heads
        are frozen to preserve the DFT k representation learned in Phase B.
        Only proc_encoder, stack_encoder, context_proj, alpha, beta are trained.
        This lets proc_encoder learn a process correction on top of a fixed
        crystal embedding without corrupting the Phase B DFT learning.

        Frozen  : backbone, task_heads
        Trainable: proc_encoder, stack_encoder, context_proj, alpha, beta,
                   func_embedding, func_proj
        """
        for param in self.backbone.parameters():
            param.requires_grad = False
        for param in self.task_heads.parameters():
            param.requires_grad = False
        # Process context branch — these need to learn
        for param in self.proc_encoder.parameters():
            param.requires_grad = True
        for param in self.stack_encoder.parameters():
            param.requires_grad = True
        ...
        for param in self.func_proj.parameters():
            param.requires_grad = True
        log.info(" Phase B2 frozen  : backbone + task_heads")
        log.info(" Phase B2 trainable: proc_encoder, stack_encoder, context_proj, alpha, beta")

    def unfreeze_backbone(self, lr=None):
        """Unfreeze all parameters after Phase B2 (for future joint fine-tuning)."""
...
def run_tier3_finetune(
    ...
    if freeze_backbone:
        ...
        log.info("─" * 68)
        # ── Phase B: DFT-only HfO2-family fine-tuning ────────────────────────────
        # v4.33: Phase B uses DFT rows ONLY (source != "Experimental").
        # This cleanly reproduces the DFT ablation (MAD:MAE 3.48) and avoids
        # the loss spike / overfitting caused by mixing experimental rows with
        # a randomly-initialised proc_encoder in the same training loop.
        df_phase_b_dft = df_phase_b[
            df_phase_b["source"] != "Experimental"
        ].copy().reset_index(drop=True)
        _n_pb_dft = len(df_phase_b_dft)
        log.info(
            "Phase B dataloader: DFT-only  rows=%d  train≈%d  val≈%d  test≈%d",
            _n_pb_dft,
            int(_n_pb_dft * cfg["train_ratio"]),
            int(_n_pb_dft * cfg["val_ratio"]),
            int(_n_pb_dft * (1 - cfg["train_ratio"] - cfg["val_ratio"])),
        )
        log.info(
            "Tier 3 Phase B: DFT-only fine-tuning  "
            "rows=%d  epochs=%d  peak_lr=%.2e  scheduler=%s  batch_size=%d",
            _n_pb_dft, cfg["epochs"], cfg["learning_rate"],
            cfg.get("scheduler", "cosine"), cfg["batch_size"],
        )

        pb_loader, pb_val_loader, _, pb_sampler, _main_ds_pb = build_dataloader(
            df         = df_phase_b_dft,
            target_col = cfg["target"],
            aux_cols   = [],
            train_frac = cfg["train_ratio"],
        ...
        pb_optimizer     = torch.optim.AdamW(
            pb_params, lr=cfg["learning_rate"], weight_decay=cfg["weight_decay"]
        )
        _saved_optimizer  = trainer.optimizer
        ...
        trainer.optimizer = _saved_optimizer
        log.info(
            "Phase B complete: best val MAE=%.4f  best epoch=%d",
            _pb_best_mae, _pb_best_epoch,
        )
        log.info("─" * 68)

        # ── Phase B2: proc_encoder warmup on experimental rows only ──────────────
        # v4.33: Phase B2 uses experimental rows ONLY (source == "Experimental").
        # backbone + task_heads are FROZEN to preserve Phase B DFT learning.
        # Only proc_encoder, stack_encoder, context_proj, alpha, beta are trained.
        # This is the cleanest possible separation:
        #   proc_encoder learns a process correction on top of a fixed crystal
        #   embedding and a fixed task_head, with no DFT rows in the loop to
        #   create gradient conflicts.
        df_phase_b2_exp = df_phase_b[
            df_phase_b["source"] == "Experimental"
        ].copy().reset_index(drop=True)
        _n_pb2_exp = len(df_phase_b2_exp)

        if _n_pb2_exp < 10:
            log.warning(
                "Phase B2 SKIPPED: only %d experimental rows (need ≥ 10).",
                _n_pb2_exp,
            )
        else:
            b2_epochs = cfg.get("phase_b2_epochs", 100)
            b2_lr     = cfg.get("phase_b2_lr", 1e-5)
            log.info("─" * 68)
            log.info(
                "Tier 3 Phase B2: proc_encoder warmup on experimental rows  "
                "rows=%d  epochs=%d  lr=%.2e  batch_size=%d",
                _n_pb2_exp, b2_epochs, b2_lr, cfg["batch_size"],
            )

            # Freeze backbone + task_heads; only proc/stack context branch trains
            model.freeze_for_proc_warmup()

            pb2_loader, pb2_val_loader, _, pb2_sampler, _main_ds_pb2 = build_dataloader(
                df         = df_phase_b2_exp,
                target_col = cfg["target"],
                aux_cols   = [],
                train_frac = cfg["train_ratio"],
            ...
            pb2_params    = [p for p in model.parameters() if p.requires_grad]
            pb2_optimizer = torch.optim.AdamW(
                pb2_params, lr=b2_lr, weight_decay=cfg["weight_decay"]
            )
            _saved_optimizer  = trainer.optimizer
            ...
            trainer.cfg       = _saved_cfg
            log.info(
                "Phase B2 complete: best val MAE=%.4f  best epoch=%d",
