...
TIER3_TRAIN_CONFIG = dict(
    ...
    # v4.29 Phase B LR fix: task_head is pretrained from Tier2 and already
    # near-optimal for HfO2-family k.  The prior LR of 5e-5 with CosineAnnealingLR
    # (no warmup) applied the full LR from batch 1 across only ~4 batches/epoch
    # on 135 training rows.  This caused the task_head to overshoot from epoch 2
    # onward, making epoch 1 permanently the best checkpoint across all variants
    # (v4.26, v4.27, v4.28) regardless of proc_encoder state.
    #
    # Fix A — 10× lower peak LR (5e-6): preserves Tier2 task_head weights;
    #   allows small HfO2-specific corrections without overshooting on 135 rows.
    #
    # Fix B — OneCycleLR scheduler (replaces CosineAnnealingLR): provides a
    #   linear warmup from ~2e-8 to 5e-6 over the first 30% of epochs (~45),
    #   then cosine decay to near-zero.  The warmup prevents the full LR hitting
    #   the pretrained head in epoch 1 before the optimizer has momentum context.
    #   pct_start=0.3 matches the proven Tier1/2 pretrain schedule.
    learning_rate  = 5e-6,
    weight_decay   = 1e-4,
    scheduler      = "onecycle",
    pct_start      = 0.3,
    ...
    unfreeze_backbone_lr = 1e-6,   # v4.31: backbone at 1e-6, heads at 5e-6 (5× differential); proc/stack still frozen
    # v4.30: fixed Phase B2 budget independent of Phase B val_MAE noise
    phase_b2_epochs = 80,          # Phase B2: joint backbone+proc_encoder fine-tuning
    # v4.32: skip Phase B2 to evaluate Phase B final checkpoint directly.
    # Phase B2 causes a loss spike (1.07->2.60 in v4.31) from exp_weight
    # 0.3->1.0 jump combined with new AdamW optimizer (zero momentum) at B2 start.
    # Evaluating phase_b_final.pt isolates Phase B contribution cleanly.
    # Set skip_phase_b2=False to re-enable Phase B2 once spike is resolved.
    skip_phase_b2   = True,

    # APPROACH-1: Phase-separated experimental row weighting.
    #
    # Diagnosis: with exp_sample_weight=3.0 applied uniformly, ~80 experimental
    # rows (42% of df_phase_b) receive 3× loss weight → effectively 62% of the
    # Phase B gradient signal.  But proc_encoder starts randomly initialised,
    # so that dominant gradient is noise → epoch 1 is the best checkpoint
    # because every update from epoch 2 onward makes things worse.
    #
    # Fix: decouple the weight by phase.
    #   Phase B  (backbone frozen, proc_encoder learning from scratch):
    #     exp_sample_weight = 0.3  → DFT rows dominate (78% of gradient).
    #     Lets the well-initialised task_head + context_proj converge on the
    #     DFT k signal first.  proc_encoder gets a gentle, stable gradient
    #     rather than being the dominant force from step 1.
    #
    #   Phase B2 (backbone unfrozen, joint fine-tuning):
    #     exp_sample_weight = 1.0  → equal weighting (DFT and exp rows at parity).
    #     proc_encoder is no longer randomly initialised; equal weight lets it
    #     refine its correction on top of a stable crystal embedding.
    #
    # These two keys override the legacy exp_sample_weight per-phase.
    # The legacy key is kept at 3.0 as a fallback only for any code path that
    # does not go through the Phase B / Phase B2 override logic.
    exp_sample_weight          = 3.0,   # legacy fallback (not used by Phase B/B2)
    exp_sample_weight_phase_b  = 0.3,   # Phase B:  DFT-dominant, proc_encoder warming up
    exp_sample_weight_phase_b2 = 1.0,   # Phase B2: equal weight, proc_encoder stabilised
...
class HighKALIGNN(nn.Module):
    ...
    def freeze_backbone(self):
        """Freeze ALIGNN backbone AND proc/stack encoders for Phase B.

        Phase B goal: let task_heads converge on HfO2-family DFT k values
        with a stable crystal_emb.  proc_encoder and stack_encoder are
        randomly initialised at this point; if their weights are trainable,
        the first backward() makes context_proj produce a non-zero delta
        from random proc_emb — corrupting val_MAE for experimental rows from
        epoch 2 onward and making epoch 1 permanently the best checkpoint.

        Keeping proc_encoder/stack_encoder FROZEN during Phase B means their
        output is a constant random vector scaled by alpha=1e-3, which is
        a tiny fixed bias the task_head can absorb.  After Phase B the
        task_head has a stable, well-converged representation of DFT k.

        Phase B2 calls unfreeze_backbone() which re-enables all parameters
        (including proc/stack encoders) so they can learn meaningful
        corrections with a stable task_head underneath.

        Trainable in Phase B:
          task_heads, context_proj, alpha, beta, func_embedding, func_proj
        Frozen in Phase B (in addition to backbone):
          proc_encoder, stack_encoder
        """
        # Freeze backbone message-passing layers
        for param in self.backbone.parameters():
            param.requires_grad = False
        # APPROACH-3: also freeze proc/stack encoders — they are randomly
        # initialised and would corrupt context_proj from epoch 1 onward.
        for param in self.proc_encoder.parameters():
            param.requires_grad = False
        for param in self.stack_encoder.parameters():
            param.requires_grad = False
        # Keep these trainable: heads, context projection, gates, func conditioning
        ...
        for param in self.func_proj.parameters():
            param.requires_grad = True
        log.info(
            " Phase B frozen  : ALIGNN backbone + proc_encoder + stack_encoder"
        )
        log.info(
            " Phase B trainable: task_heads, context_proj, alpha, beta, func_embedding, func_proj"
        )

    def unfreeze_backbone(self, lr=None):
        """Unfreeze BACKBONE ONLY for Phase B2 gentle joint adaptation.

        v4.31: proc_encoder and stack_encoder remain FROZEN in Phase B2.
        Rationale: proc_encoder has random init weights (never trained).
        Unfreezing it simultaneously with the backbone causes a catastrophic
        loss spike (1.14 → 3.80 observed in v4.30) because:
          - proc_encoder gets large gradients from k_total_log error on unfreeze
          - AdamW momentum buffers are zero at Phase B2 start (new optimizer)
          - 80 experimental rows is insufficient to train a 31→128→64 MLP
            from random init without dedicated warm-up
        Phase B2 goal: gently adapt backbone to HfO2-family representation
        with proc_encoder contributing a constant (frozen) small delta.
        proc_encoder will be unlocked in a future phase when process_db
        has sufficient rows (target: 300+) for stable encoder training.
        """
        for param in self.backbone.parameters():
            param.requires_grad = True
        # proc_encoder and stack_encoder remain frozen — see docstring
        log.info(" Phase B2 unfrozen: backbone only")
        log.info(" Phase B2 frozen  : proc_encoder + stack_encoder (random init; insufficient data)")
        if lr is not None:
            log.info("  backbone lr=%.2e", lr)

    def unfreeze_all(self, lr=None):
        """Unfreeze ALL parameters including proc/stack encoders.
        Reserved for future use when process_db reaches 300+ rows and
        proc_encoder can be trained stably from its random init.
        """
        for param in self.parameters():
            param.requires_grad = True
        log.info(" All parameters unfrozen (backbone + proc_encoder + stack_encoder)")
        if lr is not None:
            log.info("  lr=%.2e", lr)
...
def run_tier3_finetune(
    ...
    if freeze_backbone:
        ...
        log.info("─" * 68)
        log.info(
            "Tier 3 Phase B: backbone-frozen, single-task k_total_log fine-tuning  "
            "rows=%d  epochs=%d  peak_lr=%.2e  scheduler=%s  batch_size=%d",
            _n_phase_b, cfg["epochs"], cfg["learning_rate"],
            cfg.get("scheduler","cosine"), cfg["batch_size"]
        )
        # (freeze_backbone() log lines now printed inside freeze_backbone() itself)
        # APPROACH-3: proc_encoder + stack_encoder also frozen in Phase B.
        ...
        # APPROACH-1 Phase B: override exp_sample_weight to 0.3 so DFT rows
        # dominate while proc_encoder is still randomly initialised.
        # trainer.cfg is read by train_epoch on every batch, so patching it
        # here takes effect immediately for the entire Phase B training loop.
        _pb_exp_w = cfg.get("exp_sample_weight_phase_b", 0.3)
        # v4.30 FIX-EARLY-STOP: disable early stopping for Phase B.
        # The val set has only ~28 rows; one outlier shifts val_MAE by ~20%.
        # This noise makes epoch 1 permanently "best" regardless of actual
        # learning, causing best checkpoint (tier3_best.pt) to be saved at
        # epoch 1 and ALL subsequent Phase B + Phase B2 training to be
        # discarded at evaluation time (which reloads tier3_best.pt).
        # Fix: run Phase B for the full epoch budget, save a dedicated
        # Phase B final checkpoint, use it as the Phase B2 starting point.
        trainer.cfg = {
            **cfg,
            "exp_sample_weight": _pb_exp_w,
            "early_stopping":    False,   # v4.30: ignore noisy val_MAE for stopping
        }
        log.info(
            "Phase B exp_sample_weight = %.2f  early_stopping=False  (full %d epochs)",
            _pb_exp_w, cfg["epochs"],
        )
        ...
        trainer.optimizer = _saved_optimizer
        trainer.cfg = cfg   # restore before Phase B2 override below

        # v4.30 FIX-EARLY-STOP: save the Phase B FINAL checkpoint explicitly.
        # With early_stopping=False, Phase B ran all cfg["epochs"] epochs.
        # The "best" checkpoint (tier3_best.pt) may be from epoch 1 due to
        # val noise.  Save the final weights separately so Phase B2 starts
        # from the fully-trained Phase B model, not the epoch-1 snapshot.
        if is_rank0():
            trainer.save_checkpoint(cfg["epochs"], _pb_best_mae, tag="phase_b_final")
        # Reset best tracking so Phase B2 competes on its own merit.
        trainer.best_val_mae = float("inf")
        trainer.best_epoch   = 0
        trainer.patience_ctr = 0
        log.info(
            "Phase B complete: final epoch=%d  best_val_MAE=%.4f at ep=%d",
            cfg["epochs"], _pb_best_mae, _pb_best_epoch,
        )
        log.info("  Phase B final checkpoint saved as tier3_phase_b_final.pt")
        log.info("─" * 68)

        # v4.32: Phase B2 guarded by skip_phase_b2 config flag.
        # With skip_phase_b2=True (default), Phase B2 is skipped entirely so
        # phase_b_final.pt is used for evaluation — isolating Phase B contribution.
        # Phase B2 caused loss spikes (1.07->2.60) from exp_weight 0.3->1.0 jump
        # combined with new AdamW optimizer with zero momentum at B2 start.
        # Set skip_phase_b2=False to re-enable once the spike is resolved.
        if not cfg.get("skip_phase_b2", True):
            unfreeze_lr = cfg.get("unfreeze_backbone_lr", 1e-5)  # FIX-1: removed dead unfreeze_after variable (was read but never used)
            # v4.30 FIX-B2-EPOCHS: use a fixed Phase B2 epoch budget from config.
            b2_epochs = cfg.get("phase_b2_epochs", 80)
            ...
            _pb2_exp_w = cfg.get("exp_sample_weight_phase_b2", 1.0)
            log.info(
                "Phase B2 exp_sample_weight = %.2f  epochs=%d  backbone-only unfreeze",
                _pb2_exp_w, b2_epochs,
            )
            _saved_cfg = trainer.cfg
            trainer.cfg = {
                **cfg,
            ...
            trainer.cfg = _saved_cfg
            log.info(
                "Phase B2 complete: Best val_MAE=%.4f  at epoch %d (Phase B2 epoch %d)",
                trainer.best_val_mae, trainer.best_epoch,
                trainer.best_epoch - _pb_best_epoch,
            )
            log.info("─" * 68)
        else:
            log.info("Phase B2 SKIPPED (skip_phase_b2=True)")
            log.info("Evaluating Phase B final checkpoint (phase_b_final.pt)")
            log.info("─" * 68)

    # CKPT-RT2+RT3 FIX: reload best checkpoint with strict=False + key remapping.
    # v4.32: when skip_phase_b2=True, load phase_b_final.pt (Phase B final epoch)
    # instead of tier3_best.pt (Phase B epoch-1, val-noise-selected).
    # This evaluates the fully-trained Phase B model (150 epochs) vs the
    # 4-gradient-step epoch-1 snapshot that all prior versions used.
    if cfg.get("skip_phase_b2", True) and (CKPT_ROOT / "tier3_phase_b_final.pt").exists():
        eval_ckpt = CKPT_ROOT / "tier3_phase_b_final.pt"
        log.info("Using phase_b_final.pt for evaluation (skip_phase_b2=True)")
    else:
        eval_ckpt = CKPT_ROOT / "tier3_best.pt"
        log.info("Using tier3_best.pt for evaluation")

    if eval_ckpt.exists():
        ckpt = safe_load_checkpoint(eval_ckpt)
        if ckpt is not None:
