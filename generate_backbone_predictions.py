...
def build_full_experimental_dataset(builder, extractor) -> pd.DataFrame:
    """
    ...
    df_exp = extractor.load_experimental_process_db()   # reads process_db_clean.csv
    if len(df_exp) == 0:
        raise RuntimeError(
            "process_db_clean.csv not found or empty at "
            f"{T.DATA_ROOT / 'processed' / 'process_db_clean.csv'}"
        )
...
def exclude_leak_adjacent_train_rows(df_final: pd.DataFrame) -> pd.DataFrame:
    ...
    if n_excluded:
        ...
        print(f"Leakage-adjacency guard: excluding {n_excluded}/{n_train_before} "
              f"nominally-train rows that share paper/material/donor identity "
              f"with a locked VAL/TEST row  (breakdown: {by_reason})")
        df_final.loc[reasons != "", "tier3_split"] = "train_excluded_leak_adjacent"
    else:
        print("Leakage-adjacency guard: no train rows share paper/material/donor "
...
def main():
    ...
    df_final = build_full_experimental_dataset(builder, extractor)
