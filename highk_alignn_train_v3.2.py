...
class DatasetExtractor:
    ...
    def pull_materials_project(
        ...
        with MPRester(MP_API_KEY) as mpr:
            ...
            # -- Diagnostic counters for k_total extraction audit ------------
            n_parsed       = 0   # rows where _parse_mp_entry returned non-None
            n_e_total_set  = 0   # docs where doc.e_total was a valid number
            n_k_total_set  = 0   # rows where k_total ended up non-None
            n_k_above_10   = 0   # k_total > 10 (Tier 2 candidates)
            _sample_etotal = []  # first 5 raw e_total values for debugging

            for doc in tqdm(docs_dielectric, desc="Parsing MP dielectric entries"):
                # Diagnostic: sample raw e_total values before parsing
...
def main():
    ...
    if args.mode == "extract_only":
        ...
        log.info("-" * 60)
        log.info("Step 5/5: Building three-tier datasets from extracted raw data")
