...
class HighKALIGNN(nn.Module):
    ...
    def configure_structural_adapter(
        ...
        log.info(
            "v4.57 STRUCTURAL-ADAPTER: enabled=%s rank=%d reset=%s rebuilt=%s",
...
class ALIGNNTrainer:
    ...
    def rebuild_optimizer_for_current_trainables(self):
        """v4.56 AdamW groups for process residual and low-rank structural adapter."""
        ...
        log.info("v4.56 OPTIMIZER-REBUILD: tensors=%d params=%d groups=%s", len(all_trainable), sum(p.numel() for p in all_trainable), [(g["group_name"],g["lr"],sum(p.numel() for p in g["params"])) for g in groups])
    ...
    def train(
        ...
        for epoch in range(1, n_epochs + 1):
            ...
            log.info(
                ...
                "best=%.4f (ep%d)  %.1fs  "
                "alpha=%.4f beta=%.4f  proc=%.1f%% stack=%.1f%%  "
                "delta_mean=%.5f delta_max=%.5f delta_active=%.1f%% delta_grad=%.3e "
                "delta_param=%.3e delta_change=%.3e  "
                "primary=%.4f drift=%.3e weighted_drift=%.3e base_grad=%.3e "
                "base_move_mean=%.3e base_move_max=%.3e base_param_l2=%.3e  "
                "frozen_max=%.3e frozen_l2=%.3e frozen_changed=%d  %s",
                ...
                epoch_stats.get("process_delta_parameter_norm", 0.0),
                epoch_stats.get("process_delta_parameter_change_from_init", 0.0),
                epoch_stats.get("primary_loss", train_loss),
                epoch_stats.get("base_drift_loss", 0.0),
                epoch_stats.get("base_drift_weighted", 0.0),
                epoch_stats.get("base_head_grad_norm", 0.0),
                epoch_stats.get("base_change_mean_abs_log", 0.0),
                epoch_stats.get("base_change_max_abs_log", 0.0),
                epoch_stats.get("base_head_parameter_change_l2", 0.0),
                epoch_stats.get("frozen_base_max_abs_change", float("nan")),
                epoch_stats.get("frozen_base_l2_change", float("nan")),
...
def run_tier_evaluate(
    ...
    # v4.57: construct the adapter at the checkpoint rank BEFORE loading so
    # shape-compatible loading can restore its learned tensors.  Do not reset.
    if tier == 3:
        model.configure_structural_adapter(
            enabled=cfg.get("tier3_use_structural_adapter", True),
            rank=cfg.get("tier3_structural_adapter_rank", 16),
