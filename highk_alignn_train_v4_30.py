...
TIER3_TRAIN_CONFIG = dict(
    ...
    unfreeze_backbone_lr = 5e-7,   # v4.29: 10× lower than Phase B peak (5e-6); backbone < heads in B2
...
class HighKALIGNN(nn.Module):
    ...
    def unfreeze_backbone(self, lr=None):
        """Unfreeze backbone AND proc/stack encoders for Phase B2 joint training.

        APPROACH-3: proc_encoder and stack_encoder were frozen in Phase B to
        prevent random-init corruption of context_proj.  Phase B2 re-enables
        ALL parameters so proc_encoder can learn meaningful corrections on top
        of the stable task_head that Phase B produced.
        """
        for param in self.backbone.parameters():
            param.requires_grad = True
        # Re-enable proc/stack encoders frozen by freeze_backbone()
        for param in self.proc_encoder.parameters():
            param.requires_grad = True
        for param in self.stack_encoder.parameters():
            param.requires_grad = True
        log.info(
            " Phase B2 unfrozen: backbone + proc_encoder + stack_encoder (all params trainable)"
        )
        if lr is not None:
...
def run_tier3_finetune(
    ...
    if freeze_backbone:
        ...
        log.info(
            "Phase B2 exp_sample_weight = %.2f  epochs=%d  (equal weighting; proc_encoder stabilised)",
        ...
        trainer.cfg = {
            **cfg,
            ...
            "early_stopping":    False,   # v4.30: val set too small; run full b2_epochs
        }
