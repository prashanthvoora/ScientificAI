...
"""
================================================================================
  JARVIS + ALIGNN Multi-Task Pipeline  —  v2  (Process Parameters)
  Tasks:
...
Install dependencies:
  pip install jarvis-tools alignn dgl torch torchvision torchaudio \
              torch-geometric scikit-learn matplotlib seaborn tqdm joblib
...
PROCESS_DIM = len(PROCESS_PARAMS)   # 12
...
class PipelineConfig:
    ...
    # ── Process parameters ────────────────────────────────────────────────────
    # process_dim must match len(PROCESS_PARAMS) = 12
    process_dim:        int   = PROCESS_DIM
    # Path to CSV with columns [jid, deposition_temp_C, ...PROCESS_PARAMS]
...
class JARVISHighKDataset(Dataset):
    ...
    Now includes a process parameter vector per material.
    """
        ...
        for idx, row in tqdm(df.iterrows(), total=len(df), desc=f"[{split}] graph build"):
            ...
            if cache_file.exists():
                with open(cache_file, "rb") as f:
                    item = pickle.load(f)
                # Always refresh process params (may have changed since cache)
...
def get_process_params(jid: str,
    ...
    If a process CSV is provided and the JID is present, use those values.
    Missing columns or JIDs fall back to 0.0 (DFT bulk / no-process baseline).

    Note: values are stored raw here; StandardScaler is applied in main()
    across the full training set before training begins.
    """
...
class MultiTaskALIGNN(nn.Module):
    ...
    def __init__(self, cfg: PipelineConfig):
        ...
        # Encodes PROCESS_DIM (12) → 64-dim embedding
        PROC_ENC_OUT = 64
        ...
        # Pool projection: concat(avg_pool, sum_pool, process_enc) → H
        # Input = H*2 (structural) + PROC_ENC_OUT (process) = 576
        self.pool_proj = nn.Sequential(
            nn.Linear(H * 2 + PROC_ENC_OUT, H),
            nn.LayerNorm(H),
            nn.SiLU(),
            nn.Dropout(0.1)
        )
    ...
    def freeze_backbone(self, freeze: bool = True):
        ...
        backbone_modules = [
            ...
            self.pool_proj
            # Note: process_encoder is NOT frozen — it always fine-tunes
        ]
    ...
    def forward(self, batch: dict) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        """
        Args:
            batch: dict with keys 'g', 'lg', 'process' (and label keys during training)
        ...
        # Authoritative device from model parameters
        ...
        # Triplet features — ensure on correct device
        if "triplet_attr" in lg.edata:
            t = self.triplet_embedding(lg.edata["triplet_attr"].to(_dev))
        else:
            t = torch.zeros(lg.num_edges(),
                            self.cfg.hidden_features // 2, device=_dev)

        # Sync bond features to line graph node features
        if lg.num_nodes() > 0:
            lg.ndata["x"] = e[:lg.num_nodes()].to(_dev)

        # ALIGNN message passing (line graph + atom graph)
        ...
        # Global pooling: [B, H*2]
        h_struct = torch.cat([self.avg_pool(g, h),
                               self.sum_pool(g, h)], dim=-1)

        # ── Process parameter encoding ────────────────────────────────────────
        # Fallback to zeros if process key absent (e.g. inference without params)
        if "process" in batch:
            proc = batch["process"].to(_dev)                    # [B, PROCESS_DIM]
        else:
            B    = h_struct.shape[0]
            proc = torch.zeros(B, self.cfg.process_dim, device=_dev)

        h_proc = self.process_encoder(proc)                     # [B, 64]

        # ── Fuse structural + process → task embedding ────────────────────────
        h_fused = self.pool_proj(
            torch.cat([h_struct, h_proc], dim=-1)               # [B, H*2 + 64]
        )                                                        # [B, H]

        # ── Task heads ────────────────────────────────────────────────────────
        bandgap_pred = self.bandgap_act(self.bandgap_head(h_fused))
        highk_logits = self.highk_head(h_fused)
        eps_pred     = self.eps_act(self.eps_head(h_fused))
...
def predict_single_material(atoms_dict: dict,
    ...
    """
    Run inference on a single material.

    Args:
        atoms_dict      : JARVIS Atoms dict (from Atoms.to_dict())
        checkpoint_path : path to best_model.pt
        cfg             : PipelineConfig instance
        process_params  : optional np.ndarray of shape [PROCESS_DIM] with raw
                          process conditions. If None, uses zero baseline.
        scaler_path     : optional path to process_scaler.pkl for normalisation
...
def main():
    ...
    # ── 2. Load optional process conditions CSV ───────────────────────────────
    process_df = load_process_df(CFG)
    if process_df is None:
        logger.info("No process CSV provided — using zero-vector baseline for all materials")
    else:
        logger.info(f"Process CSV loaded: {len(process_df)} entries")
    ...
    # ── 4. Build datasets ─────────────────────────────────────────────────────
    train_dataset = JARVISHighKDataset(df_train, CFG, split="train", process_df=process_df)
    val_dataset   = JARVISHighKDataset(df_val,   CFG, split="val",   process_df=process_df)
    test_dataset  = JARVISHighKDataset(df_test,  CFG, split="test",  process_df=process_df)

    # ── 5. Fit process parameter scaler on training set only ──────────────────
    proc_matrix = train_dataset.get_process_matrix()  # [N_train, PROCESS_DIM]

    if proc_matrix.sum() == 0:
        # All zeros (no CSV) — scaler is identity, skip fitting
        proc_scaler = StandardScaler()
        proc_scaler.fit(proc_matrix)
        logger.info("Process params are all-zero baseline — scaler is identity")
    else:
        proc_scaler = StandardScaler()
        proc_scaler.fit(proc_matrix)
        logger.info(f"Process scaler fitted on {len(proc_matrix)} training samples")

    # Apply scaler to all three splits
    train_dataset.apply_process_scaler(proc_scaler)
    val_dataset.apply_process_scaler(proc_scaler)
    test_dataset.apply_process_scaler(proc_scaler)

    # Save scaler for inference
    scaler_path = Path(CFG.checkpoint_dir) / "process_scaler.pkl"
    joblib.dump(proc_scaler, scaler_path)
    logger.info(f"Process scaler saved to {scaler_path}")
