...
==============================================================================
 ...
 Version 4.5.22 DFT-only ablation control (process/context disabled)
==============================================================================
...
TIER3_TRAIN_CONFIG = dict(
    ...
    bounded_process_residual = False,   # DFT-ONLY-ABLATION: disable bounded process residual
    process_delta_bound      = 0.10,
    process_delta_use_categorical = False,
    process_delta_train_base_head = False,

    # DFT-ONLY-ABLATION: force process/stack context off during train/eval.
    # This keeps the same Tier-3 data split/checkpoint flow but removes every
    # ALD-process path so it is a fair control for residual_proc.py.
    dft_only_ablation = True,
    dft_only_train_head = True,
)
...
class HighKALIGNN(nn.Module):
    ...
    def freeze_for_dft_only_ablation(self, train_head: bool = True):
        """Freeze all process/context paths and optionally train only k_total_log head.

        This is the clean DFT-only control: graph/backbone prediction is allowed,
        but ALD process encoder, stack encoder, context projection, alpha/beta,
        and bounded process_delta_head are all disabled. With train_head=True,
        only the k_total_log task head is adapted on Tier-3 labels; with
        train_head=False this becomes a strict zero-shot Tier-2 checkpoint eval.
        """
        for p in self.parameters():
            p.requires_grad = False
        if train_head and "k_total_log" in self.task_heads:
            for p in self.task_heads["k_total_log"].parameters():
                p.requires_grad = True
        # Ensure residual-process path is off even if a checkpoint/config enabled it.
        self.configure_bounded_process_residual(enabled=False)
        n_trainable = sum(p.numel() for p in self.parameters() if p.requires_grad)
        log.info(
            "DFT-ONLY-ABLATION freeze: process/context disabled; train_head=%s; trainable_params=%d",
            train_head, n_trainable,
        )
...
def run_tier3_finetune(
    ...
    # DFT-ONLY-ABLATION: use a separate checkpoint prefix so this control run
    # never overwrites residual/full-fusion tier3_best.pt.
    ckpt_prefix_t3 = "tier3_dftonly" if cfg.get("dft_only_ablation", False) else "tier3"
    trainer = ALIGNNTrainer(model, cfg, ckpt_prefix=ckpt_prefix_t3,
                            ablate_context=(ablate_context or cfg.get("dft_only_ablation", False)))
    ...
    dft_only_mode = bool(cfg.get("dft_only_ablation", False))
    if freeze_backbone:
        if residual_proc_mode:
            ...
            model.freeze_for_dft_only_ablation(
                train_head=cfg.get("dft_only_train_head", True)
            )
            trainer.ablate_context = True
        else:
            model.freeze_backbone()
        log.info("─" * 68)
        log.info(
            "Tier 3 Phase B: %s  rows=%d  epochs=%d  lr≈%.2e  batch_size=%d",
            "DFT-only ablation (process/context OFF)" if dft_only_mode else ("DFT-only base + bounded process residual" if residual_proc_mode else "backbone-frozen single-task k_total_log fine-tuning"),
        ...
        if residual_proc_mode:
            ...
            log.info(
                " Frozen : ALIGNN backbone and all process/context paths; "
                "training k_total_log head only. trainer.ablate_context=True."
            )
        else:
        ...
        if residual_proc_mode or dft_only_mode:
            log.info(
                "%s: skipping Phase B2 backbone unfreeze so the final model remains a clean control.",
                "RESIDUAL-PROC" if residual_proc_mode else "DFT-ONLY-ABLATION",
            )
        else:
    ...
    best_ckpt = CKPT_ROOT / f"{trainer.ckpt_prefix}_best.pt"
    if best_ckpt.exists():
        ckpt = safe_load_checkpoint(best_ckpt)
        if ckpt is not None:
            ...
            log.info(
                f"Reloaded {trainer.ckpt_prefix}_best.pt for evaluation  "
    ...
    out_path = REPORT_ROOT / ("tier3_dftonly_test_results.json" if cfg.get("dft_only_ablation", False) else "tier3_test_results.json")
    primary_metrics = {
    ...
    return CKPT_ROOT / f"{trainer.ckpt_prefix}_best.pt"
...
def run_tier_evaluate(
    ...
    if tier == 1:
        ...
        target_col  = cfg["target"]
        task_names  = [target_col] + cfg["aux_targets"]
        ckpt_prefix = "tier3_dftonly" if cfg.get("dft_only_ablation", False) else "tier3"
        # DFT-ONLY-ABLATION-EVAL: evaluation must mirror training.
        # The ablation script disables all process/context contribution by default,
        # so tier3_evaluate must also force ablate_context=True unless the caller
        # explicitly edits the config. Without this, evaluating a DFT-only checkpoint
        # can accidentally run the residual/full process forward path and reproduce
        # the residual model metric.
        if cfg.get("dft_only_ablation", False):
            ablate_context = True
            log.info("tier3_evaluate: DFT-only ablation active; forcing ablate_context=True")
    ...
    if tier == 3:
        model.configure_bounded_process_residual(
            enabled=cfg.get("bounded_process_residual", False),
            bound=cfg.get("process_residual_bound", 0.10),
            train_base_head=cfg.get("residual_train_base_head", False),
        )
        if cfg.get("dft_only_ablation", False):
            model.configure_bounded_process_residual(enabled=False)
            log.info("tier3_evaluate: bounded process residual disabled for DFT-only ablation")
    ...
    if tier == 3:
        _primary_key = cfg.get("target", target_col)
        if _primary_key in mt_results and mt_results[_primary_key]["n"] > 0:
            km = mt_results[_primary_key]
            log.info(
                "%s  MAD=%.2f  MAE=%.4f  MAD:MAE=%.2f  "
                "(paper: 1.63 @ 44K no transfer | goal: ≥ 2.5 with three-tier transfer)",
                _primary_key, km["mad"], km["mae"], km["mad_mae_ratio"],
            )

    # ── Save to JSON ──────────────────────────────────────────────────────────
    out_path = REPORT_ROOT / ("tier3_dftonly_evaluate_results.json" if (tier == 3 and cfg.get("dft_only_ablation", False)) else f"tier{tier}_evaluate_results.json")
...
def main():
    ...
    if args.mode in ["tier1_evaluate", "tier2_evaluate", "tier3_evaluate"]:
        ...
        if tier_num == 3 and TIER3_TRAIN_CONFIG.get("dft_only_ablation", False):
            ckpt_default = CKPT_ROOT / "tier3_dftonly_best.pt"
        else:
