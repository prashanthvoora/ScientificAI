...
# ==============================================================================
# TIER-1 EXTERNAL BENCHMARK EVALUATION (v4.60.3-ext1)
# ==============================================================================
...
def run_tier1_external_evaluate(
    ...
    df = _external_overlap_audit(
        df_loaded, TierDatasetBuilder.TIER_PATHS[1], overlap_policy, output_dir
    )
    flow["rows_after_overlap_policy"] = int(len(df))
    flow["rows_removed_by_overlap_policy"] = int(len(df_loaded) - len(df))
    flow["strict_external_rows_after_policy"] = int(
        df.get("is_strict_external", pd.Series(True, index=df.index)).fillna(False).sum()
    )
    ...
    for head, target_col in head_to_target.items():
        ...
        for ri in dataset.valid_idx:
            ...
            if pd.isna(y) or not np.isfinite(pval):
                continue
            true_vals.append(float(y)); pred_vals.append(float(pval))
        ...
        yarr, parr = np.asarray(true_vals), np.asarray(pred_vals)
        stats = _external_metric_arrays(yarr, parr)
        boot = _external_bootstrap(yarr, parr, n_boot=n_boot, seed=20260729 + len(metric_rows))
        row = {"task": head, "scale": "log" if head == "k_total_log" else "native", **stats, **boot}
        metric_rows.append(row)
        flow[f"final_valid_pairs_{head}"] = int(stats.get("n", 0))
        if head == "k_total_log" and len(yarr):
            ylin, plin = np.exp(yarr), np.exp(parr)
            lstats = _external_metric_arrays(ylin, plin)
            lboot = _external_bootstrap(ylin, plin, n_boot=n_boot, seed=20260829)
            metric_rows.append({"task": "k_total", "scale": "linear", **lstats, **lboot})
    ...
    for _, r in metrics_df.iterrows():
        lines.append(
            f"| {r['task']} | {r['scale']} | {int(r.get('n',0))} | {r.get('mae',np.nan):.6f} | "
