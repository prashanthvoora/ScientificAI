    ...
    def train_epoch(
        ...
        for batch in loader:
            ...
            if has_trainable_graph:
                ...
                log.info(
                    "v4.39 ABLATION-NOGRAD: context is disabled and the DFT base "
    ...
    def train(
        ...
        for epoch in range(1, n_epochs + 1):
            ...
            log.info(
                ...
                "delta_mean=%.5f delta_max=%.5f delta_active=%.1f%% delta_grad=%.3e  %s",
...
def run_tier3_finetune(
    ...
    def _write_tier3_prediction_audit(
        ...
        audit_dir = REPORT_ROOT / "tier3_prediction_audit"
        ...
        with torch.no_grad():
            for batch in loader:
                ...
                if getattr(model_core, "use_bounded_process_residual", False):
                    delta = model_core._process_bounded_delta(proc_ctx)
                    if delta is not None:
                        delta_log = delta.detach().cpu().view(-1)
                ...
                if proc_ctx is not None and "avail" in proc_ctx:
                    proc_avail = proc_ctx["avail"].detach().cpu().view(-1).tolist()
