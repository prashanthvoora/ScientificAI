...
==============================================================================
 ...
 Version 4.5.55 (v4.55: controlled Tier-2 k-head adaptation for Tier-3)
==============================================================================

 v4.55 changes (controlled base-head adaptation)
 ---------------------------------------------------------------------------
...
TIER3_TRAIN_CONFIG = dict(
    ...
    # v4.55: controlled adaptation of the Tier-2 k_total_log output head.
    ...
    process_delta_train_base_head = True,
    tier3_base_head_learning_rate = 1e-5,
    tier3_process_learning_rate   = 1e-4,
    tier3_base_drift_weight       = 1.0,
    tier3_base_head_weight_decay  = 1e-4,
...
class HighKALIGNN(nn.Module):
    ...
    def __init__(
        ...
        # v4.55 controlled base-head adaptation diagnostics.  The frozen
    ...
    def freeze_for_bounded_process_residual(self, train_base_head: bool = False):
        ...
        v4.46: freezing parameters is not sufficient because ``model.train()``
        can still activate dropout or update normalization buffers in the base
        path. The trainer therefore uses :meth:`set_residual_only_train_mode`
        during every residual epoch so the complete frozen base stays in eval
        mode while only ``process_delta_head`` is placed in train mode.
        """
        for p in self.parameters():
            p.requires_grad = False
        for p in self.process_delta_head.parameters():
            p.requires_grad = True
        # v4.46: categorical residual mode must train the embedding tables that
        # feed process_delta_head. v4.46 used their values but left them frozen.
        ...
        if train_base_head and "k_total_log" in self.task_heads:
            for p in self.task_heads["k_total_log"].parameters():
                p.requires_grad = True
            # Snapshot the exact loaded Tier-2 head before any Tier-3 update.
            ref_head = copy.deepcopy(self.task_heads["k_total_log"])
            ref_head.to(next(self.parameters()).device)
            ref_head.eval()
            for p in ref_head.parameters():
                p.requires_grad = False
            object.__setattr__(self, "_tier3_base_head_reference", ref_head)
            log.info("v4.55 BASE-HEAD-REFERENCE: frozen Tier-2 k_total_log head captured")
        else:
            object.__setattr__(self, "_tier3_base_head_reference", None)
        self._residual_train_base_head = bool(train_base_head)
        n_trainable = sum(p.numel() for p in self.parameters() if p.requires_grad)
        log.info(
            "v4.46 RESIDUAL-PROC freeze: frozen base parameters+buffers protected by eval mode; "
            "train_base_head=%s; trainable_params=%d",
    ...
    def set_residual_only_train_mode(self, ablate_context: bool = False):
        ...
        if getattr(self, "_residual_train_base_head", False):
            # Keep head dropout disabled while still allowing gradients through
            # its Linear layers. This makes base adaptation deterministic.
        ...
        if self.use_bounded_process_residual:
            ...
            if task == "__all__":
                ...
                for t, head in self.task_heads.items():
                    base = head(embedding)
                    if t == "k_total_log":
                        self._record_base_adaptation(embedding, base)
...
class ALIGNNTrainer:
    ...
    def rebuild_optimizer_for_current_trainables(self):
        """Rebuild AdamW after residual-only freezing (v4.46).

        The trainer is instantiated before Phase-B freezing in the legacy flow,
        so its initial optimizer may still contain the entire model. Rebuilding
        here makes optimizer membership match the scientific experiment exactly.
        """
        base_params = []
        if self.cfg.get("process_delta_train_base_head", False):
            base_params = [p for p in self.model_core.task_heads["k_total_log"].parameters() if p.requires_grad]
        base_ids = {id(p) for p in base_params}
        process_params = [
            p for p in self.model_core.parameters()
            if p.requires_grad and id(p) not in base_ids
        ]
        if not process_params and not base_params:
            raise RuntimeError("v4.55: no trainable parameters after residual freeze")
        groups = []
        if process_params:
            groups.append({
        ...
        if base_params:
            groups.append({
                "params": base_params,
                "lr": float(self.cfg.get("tier3_base_head_learning_rate", 1e-5)),
                "weight_decay": float(self.cfg.get("tier3_base_head_weight_decay", self.cfg["weight_decay"])),
                "group_name": "base_k_head",
            })
        ...
        self._write_optimizer_audit_manifest()
        all_trainable = process_params + base_params
        self._base_head_initial_state = {
            name: p.detach().cpu().clone()
            for name, p in self.model_core.task_heads["k_total_log"].named_parameters()
        } if base_params else {}
        log.info(
            "v4.55 OPTIMIZER-REBUILD: trainable_tensors=%d trainable_params=%d groups=%s",
            len(all_trainable), sum(p.numel() for p in all_trainable),
            [(g.get("group_name"), g.get("lr"), sum(p.numel() for p in g["params"])) for g in groups],
        )
    ...
    def capture_frozen_base_reference(self):
        ...
        self._frozen_base_reference = {
            ...
            and not (self.cfg.get("process_delta_train_base_head", False)
                     and name.startswith("task_heads.k_total_log."))
        }
        log.info(
    ...
    def train_epoch(
        ...
        for batch in loader:
            ...
            loss = primary_loss
            base_drift_loss = torch.zeros((), device=self.device)
            base_drift_weight = float(self.cfg.get("tier3_base_drift_weight", 0.0))
            if (self.cfg.get("bounded_process_residual", False)
                    and self.cfg.get("process_delta_train_base_head", False)
                    and base_drift_weight > 0.0):
                adapted_base = getattr(self.model_core, "_last_adapted_base_log", None)
                reference_base = getattr(self.model_core, "_last_reference_base_log", None)
                if adapted_base is None or reference_base is None:
                    raise RuntimeError("v4.55 base-drift tensors were not populated by model.forward")
                valid_base = ~torch.isnan(target.squeeze(-1))
                if bool(valid_base.any()):
                    base_drift_loss = F.mse_loss(
                        adapted_base.squeeze(-1)[valid_base],
                        reference_base.squeeze(-1)[valid_base],
                    )
                    loss = primary_loss + base_drift_weight * base_drift_loss
        ...
        for epoch in range(1, n_epochs + 1):
            ...
            if self.cfg.get("process_delta_train_base_head", False):
                ...
                if (epoch_stats.get("base_change_mean_abs_log", 0.0) > mean_lim or
                    ...
                    log.warning(
                        "v4.55 BASE-CHANGE LIMIT: mean=%.4f (limit %.4f), max=%.4f (limit %.4f). "
...
def run_tier3_finetune(
    ...
    if freeze_backbone:
        ...
        # v4.55 optimizer was rebuilt immediately after residual freezing with
...
def main():
    ...
    parser.add_argument("--tier3_base_head_lr", type=float, default=None,
                        help="v4.55 LR for controlled Tier-2 k_total_log head adaptation")
    parser.add_argument("--tier3_process_lr", type=float, default=None,
                        help="v4.55 LR for process residual branch")
    parser.add_argument("--tier3_base_drift_weight", type=float, default=None,
                        help="v4.55 MSE anchor weight to original frozen Tier-2 base head")
    parser.add_argument("--disable_tier3_base_head_adaptation", action="store_true",
                        help="Restore frozen-base residual-only behavior")
    ...
    if args.tier3_base_head_lr is not None:
        TIER3_TRAIN_CONFIG["tier3_base_head_learning_rate"] = float(args.tier3_base_head_lr)
    ...
    if args.tier3_base_drift_weight is not None:
        TIER3_TRAIN_CONFIG["tier3_base_drift_weight"] = float(args.tier3_base_drift_weight)
    if args.disable_tier3_base_head_adaptation:
        TIER3_TRAIN_CONFIG["process_delta_train_base_head"] = False
    ...
    if args.mode == "tier3_finetune":
        log.info(
            "v4.55 BASE-HEAD-ADAPT: enabled=%s base_lr=%.2e process_lr=%.2e drift_weight=%.3g",
            TIER3_TRAIN_CONFIG.get("process_delta_train_base_head", False),
            TIER3_TRAIN_CONFIG.get("tier3_base_head_learning_rate", 1e-5),
            TIER3_TRAIN_CONFIG.get("tier3_process_learning_rate", TIER3_TRAIN_CONFIG["learning_rate"]),
            TIER3_TRAIN_CONFIG.get("tier3_base_drift_weight", 1.0),
        )
