...
==============================================================================
 ...
 Version 4.5.29 (v4.29: perovskite-filter for bounded process residual)
==============================================================================
 v4.5.29 fixes  (v4.29: PEROVSKITE-FILTER -- MAD:MAE collapse 6.14 -> 1.09)
 ---------------------------------------------------------------------------
 PEROVSKITE-FILTER  Diagnosis: compact DB v4 (631 rows, 410 new_db_added_only
           + 221 trusted_v3_original) added SrTiO3/BaTiO3 rows with
           k_experimental = 100-215.  In RESIDUAL-PROC mode, final_log_k =
           frozen_dft_base_log_k + process_delta_bound(0.35) * tanh(delta_raw).
           HfO2-family DFT-experimental gaps are small (~0.15 log units,
           k~20-25) and fit inside the 0.35 bound. Perovskite gaps are
           1.0-1.4 log units -- physically unreachable within the bound.
           tanh saturates at +1 for every perovskite row, producing ~87x the
           per-row loss of a well-fit HfO2 row. The saturated perovskite
           gradient dominates process_delta_head training and pushes it to
           emit near-maximal +0.35 correction for ALL materials, including
           HfO2 -- collapsing MAD:MAE from 6.14 to 1.09 (model regresses
           toward the training-set mean).
           Fix: exclude experimental rows with k_total > bounded_residual_
           k_max (default 80.0) from Phase B training whenever
           bounded_process_residual=True. See PEROVSKITE-FILTER block in
           run_tier3_finetune() and bounded_residual_k_max in
           TIER3_TRAIN_CONFIG.  DFT-only rows are never excluded.
           Also added --trusted_only_tier3 ablation flag (restricts
           experimental DB to source_db=='trusted_v3_original', i.e. the
           original ~221-row verified set) to establish a clean baseline
           before evaluating the compact-DB expansion + filter together.
           NOTE: the compact DB's six '~20'/'~22'/etc HfO2 k values are
           already correctly recovered by the existing _clean_numeric_series
           approx-prefix handling (FIX-T3-11) -- no fix needed there.
 ---------------------------------------------------------------------------
...
TIER3_TRAIN_CONFIG = dict(
    ...
    # PEROVSKITE-FILTER (v4.29): experimental rows with k_total above this
    # threshold are excluded from bounded-residual Phase B training. HfO2-
    # family + adjacent binary oxides (Al2O3, ZrO2, La2O3, Y2O3, HfSiO, etc.)
    # all have k < 60. Perovskite/titanate families (SrTiO3, BaTiO3) have
    # k = 100-300+, a DFT-experimental gap of 1.0-1.4 log units that exceeds
    # process_delta_bound=0.35 and saturates tanh, hijacking the process
    # correction gradient for ALL materials including the HfO2 target family.
    # See v4.29 changelog / PEROVSKITE-FILTER block in run_tier3_finetune.
    bounded_residual_k_max   = 80.0,
...
# ABLATION (v4.29): set True via --trusted_only_tier3 CLI flag in main().
# Consumed by load_experimental_process_db() to restrict the experimental
# process DB to source_db=='trusted_v3_original' only.
_ABLATE_TRUSTED_ONLY = False
...
class DatasetExtractor:
    ...
    def load_experimental_process_db(
        ...
        # ABLATION (v4.29): --trusted_only_tier3 restricts to the original
        # verified rows only, dropping the entire new_db_added_only expansion.
        # Use this to establish a clean before/after baseline.
        if _ABLATE_TRUSTED_ONLY:
            if "source_db" in df.columns:
                n_before_ablate = len(df)
                df = df[df["source_db"] == "trusted_v3_original"].copy()
                log.warning(
                    "ABLATION --trusted_only_tier3: source_db filter %d -> %d rows "
                    "(dropped new_db_added_only expansion)",
                    n_before_ablate, len(df),
                )
            else:
                log.warning(
                    "ABLATION --trusted_only_tier3 requested but 'source_db' "
                    "column not present in %s -- flag is a no-op for this file.",
                    path,
                )
...
def run_tier3_finetune(
    ...
    # ── PEROVSKITE-FILTER (v4.29) ────────────────────────────────────────────
    # RESIDUAL-PROC mode learns final_log_k = frozen_dft_base_log_k +
    # process_delta_bound * tanh(delta_raw).  For HfO2-family materials the
    # DFT-experimental gap is small (~0.15 log units for k~20-25) and fits
    # comfortably inside process_delta_bound=0.35.  For perovskite/ferro-
    # electric families (SrTiO3, BaTiO3) the compact-DB experimental k values
    # (100-215) sit 1.0-1.4 log units away from any plausible DFT baseline --
    # far outside the bound.  tanh saturates at +1 for these rows, so the
    # process_delta_head is driven to always output the maximum positive
    # correction.  Because gradient magnitude scales with squared error, a
    # handful of saturated perovskite rows (~87x larger per-row loss than a
    # well-fit HfO2 row) dominates the head's training signal and the same
    # near-maximal correction gets applied to every material -- including
    # HfO2 -- corrupting the metric that matters for this project.
    #
    # Fix: when bounded_process_residual is active, exclude experimental rows
    # whose measured k exceeds bounded_residual_k_max from the bounded-
    # residual training pool.  HfO2-family + adjacent binary oxides
    # (Al2O3, ZrO2, La2O3, Y2O3, HfSiO, etc.) all have k < 60; only
    # perovskite/titanate families (SrTiO3, BaTiO3, and similar) exceed 80.
    # DFT-only rows are never excluded (they come from JARVIS DFPT and do not
    # participate in the process-residual mismatch this filter targets).
    if cfg.get("bounded_process_residual", False):
        BOUNDED_K_MAX = float(cfg.get("bounded_residual_k_max", 80.0))
        k_phase_b = pd.to_numeric(df_phase_b.get("k_total", pd.Series(dtype=float)), errors="coerce")
        is_experimental = (
            df_phase_b.get("source", pd.Series("", index=df_phase_b.index)) == "Experimental"
        )
        high_k_exp_mask = is_experimental & (k_phase_b > BOUNDED_K_MAX)
        n_excluded = int(high_k_exp_mask.sum())
        if n_excluded > 0:
            excluded_materials = (
                df_phase_b.loc[high_k_exp_mask, "material"]
                .value_counts()
                .to_dict()
                if "material" in df_phase_b.columns else {}
            )
            log.warning(
                "PEROVSKITE-FILTER: excluding %d/%d experimental rows with "
                "k_total > %.1f from bounded-residual Phase B training "
                "(process_delta_bound=%.2f cannot span the DFT-experimental "
                "gap for these materials -> gradient saturation/hijacking).  "
                "Excluded materials: %s",
                n_excluded, int(is_experimental.sum()), BOUNDED_K_MAX,
                cfg.get("process_delta_bound", 0.35), excluded_materials,
            )
            df_phase_b = df_phase_b[~high_k_exp_mask].reset_index(drop=True)
        else:
            log.info(
                "PEROVSKITE-FILTER: no experimental rows exceed k_total=%.1f "
                "threshold -- filter is a no-op for this dataset.",
                BOUNDED_K_MAX,
            )
    # ──────────────────────────────────────────────────────────────────────
...
def main():
    ...
    parser.add_argument(
        "--trusted_only_tier3", action="store_true",
        help=(
            "ABLATION (v4.29): restrict the experimental process DB to "
            "source_db=='trusted_v3_original' only, dropping the 410-row "
            "new_db_added_only expansion entirely (including the weak_expanded "
            "rows). Use this to establish a clean baseline MAD:MAE on the "
            "original ~221-row verified set before evaluating whether the "
            "compact-DB expansion + PEROVSKITE-FILTER together improve on it. "
            "Requires --rebuild_tier3 to take effect (changes upstream data)."
        ),
    )
    ...
    if args.trusted_only_tier3:
        # ABLATION (v4.29): consumed inside load_experimental_process_db via
        # the module-level _ABLATE_TRUSTED_ONLY flag (set immediately below).
        global _ABLATE_TRUSTED_ONLY
        _ABLATE_TRUSTED_ONLY = True
        log.warning(
            "ABLATION MODE: --trusted_only_tier3 active -- experimental DB "
            "restricted to source_db=='trusted_v3_original' only. "
            "The new_db_added_only expansion (410 rows) is excluded."
        )
