...
class ALIGNNTrainer:
    ...
    def capture_frozen_base_reference(self):
        ...
        # v4.59.5: derive the exclusion set from requires_grad rather than
        # maintaining a fragile hard-coded list.  The new trainable
        # tier3_global_bias was previously captured as part of the frozen base,
        # so its legitimate optimizer update triggered a false FROZEN-BASE
        # DRIFT error (typically changed_tensors=1).
        trainable_param_names = {
            name for name, param in self.model_core.named_parameters()
        ...
        self._frozen_base_reference = {
            ...
            and not name.startswith("process_delta_head.")
        ...
        # Fail before epoch 1 if a future trainable parameter is accidentally
        # included in the frozen reference. This makes the audit invariant
        # self-checking as the architecture evolves.
        overlap = sorted(set(self._frozen_base_reference).intersection(trainable_param_names))
        if overlap:
            raise RuntimeError(
                "v4.59.5 FROZEN-SNAPSHOT INVALID: trainable parameters were "
                f"captured as frozen: {overlap[:10]}"
            )
        log.info(
            "v4.59.5 BASE-SNAPSHOT: captured %d frozen parameters/buffers; "
            "excluded_trainable=%d global_bias_trainable=%s",
            len(self._frozen_base_reference),
            len(trainable_param_names),
            bool(getattr(self.model_core, "use_tier3_global_bias", False)
                 and self.model_core.tier3_global_bias.requires_grad),
        )
...
def load_tier3_positions_from_split(
    ...
    try:
        ...
        # v4.59.5: accept complete small splits. The legacy threshold
        # max(5, 0.95*n_saved) rejected every valid manifest with fewer than
        # five rows even when matching was exact (for example 3/3 VAL rows).
        # Require exact identity for small evaluation populations; for larger
        # legacy manifests retain the 95% stale-file tolerance.
        if n_saved <= 5:
            required_matches = n_saved
        else:
            required_matches = int(np.ceil(0.95 * n_saved))
        if len(matched) < required_matches:
            log.warning(
                "v4.59.5 STRICT-SPLIT-LOAD rejected split=%s: only %d/%d rows "
                "matched (required=%d). Rebuild %s if the underlying Tier3 "
                "dataset has intentionally changed.",
                split_name, len(matched), n_saved, required_matches, path,
            )
            return None
        log.info(
            "v4.59.5 STRICT-SPLIT-LOAD ACCEPT: split=%s matched=%d/%d "
            "required=%d",
            split_name, len(matched), n_saved, required_matches,
        )
...
def run_tier3_finetune(
    ...
    def _eval_exact_k_for_diag(loader, split_name: str, stage_name: str) -> dict:
        ...
        _split_upper = split_name.upper()
        _expected_n = (
            _canonical_val_n if _split_upper == "VAL" else
            _canonical_test_n if _split_upper == "TEST" else None
        )
        if _expected_n is not None and out["n"] != _expected_n:
            raise RuntimeError(
                f"v4.59.2 AUDIT COUNT MISMATCH at {stage_name}/{split_name}: "
                f"observed N={out['n']} expected N={_expected_n}"
            )
        log.info(
    ...
    def _write_tier3_prediction_audit(
        ...
        if safe_split in ("test", "val"):
            ...
            if len(audit_df) != _expected_audit_n:
                raise RuntimeError(
                    f"v4.59.3 prediction audit count mismatch {safe_stage}/{safe_split}: "
    ...
    if freeze_backbone:
        ...
        # v4.59.3: Phase B must use the exact schema-canonical evaluation
        # populations established above. Rebuilding a second validation loader
        # from df_phase_b can change valid-row filtering, index space, or schema
        # membership even when the persistent manifest is correct. Reuse the
        # locked canonical loaders directly so checkpoint selection and all
        # stage-to-stage audits refer to identical VAL/TEST identities.
        _pb_rebuilt_val_n = len(getattr(pb_val_loader.dataset, "indices", []))
        _pb_rebuilt_test_n = len(getattr(pb_test_loader.dataset, "indices", []))
        if _pb_rebuilt_val_n != _canonical_val_n or _pb_rebuilt_test_n != _canonical_test_n:
            log.warning(
                "v4.59.3 PHASE-B-EVAL-REUSE: rebuilt val/test counts differ "
                "from canonical (%d/%d vs %d/%d); reusing canonical loaders",
                _pb_rebuilt_val_n, _pb_rebuilt_test_n,
                _canonical_val_n, _canonical_test_n,
            )
        pb_val_loader = val_loader
        pb_test_loader = test_loader
        if len(getattr(pb_val_loader.dataset, "indices", [])) != _canonical_val_n:
            raise RuntimeError("v4.59.3 canonical Phase-B VAL loader count mismatch")
        if len(getattr(pb_test_loader.dataset, "indices", [])) != _canonical_test_n:
            raise RuntimeError("v4.59.3 canonical Phase-B TEST loader count mismatch")
