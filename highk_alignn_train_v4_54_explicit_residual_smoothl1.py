...
==============================================================================
 ...
 Version 4.54 (explicit residual SmoothL1 supervision)
==============================================================================
...
TIER3_TRAIN_CONFIG = dict(
    ...
    process_delta_train_base_head = False,  # v4.49 LOCKED: frozen base head
    process_delta_init_seed = 42,  # deterministic Tier-3-only residual initialization

    # v4.54 EXPLICIT-RESIDUAL-SMOOTHL1:
    # Add an auxiliary robust loss directly on the learned process correction.
    # Only rows with proc_context["avail"] > 0 participate; DFT-only rows are
    # excluded because they have no process correction to supervise.
    explicit_residual_loss_enabled = True,
    explicit_residual_loss_weight = 0.25,
    explicit_residual_smooth_l1_beta = 0.05,
...
class HighKALIGNN(nn.Module):
    ...
    def set_residual_only_train_mode(self, ablate_context: bool = False):
        ...
        if getattr(self, "_residual_train_base_head", False):
            self.task_heads["k_total_log"].train()
    ...
    def _process_bounded_delta(self, proc_context: Optional[dict]) -> Optional[torch.Tensor]:
        ...
        if not bool(avail.any()):
            ...
            self._last_process_delta_active_pct = 0.0
            self._last_process_delta_tensor = total_delta
            return total_delta
        ...
        # Retain the graph-connected correction for the explicit residual loss.
        # It is overwritten on every forward pass and is never checkpointed.
        self._last_process_delta_tensor = total_delta
        return total_delta
...
class ALIGNNTrainer:
    ...
    def rebuild_optimizer_for_current_trainables(self):
        ...
        """
        params = [p for p in self.model_core.parameters() if p.requires_grad]
        if not params:
            raise RuntimeError("v4.46: no trainable parameters after residual freeze")
        self.optimizer = torch.optim.AdamW(
            params,
        ...
        self._write_optimizer_audit_manifest()
        log.info(
            "v4.46 OPTIMIZER-REBUILD: trainable_tensors=%d trainable_params=%d",
    ...
    def train_epoch(
        ...
        """
        _residual_only_mode = bool(
            self.cfg.get("bounded_process_residual", False)
        ...
        explicit_residual_loss_sum = 0.0
        explicit_residual_batches = 0
        explicit_residual_rows = 0
        ...
        for batch in loader:
            ...
            primary_loss = self.criterion(
                preds, targets_dict, functional_weights=func_loss_weights
            )
            loss = primary_loss

            # v4.54 EXPLICIT-RESIDUAL-SMOOTHL1
            # For a frozen base, the exact row target for the process residual is
            #     ideal_delta = y_true_log - frozen_base_log.
            # The base prediction is recovered from final_pred - learned_delta.
            # Supervise only process-available rows; including DFT-only rows would
            # incorrectly ask an unavailable process branch to absorb base error.
            residual_loss_value = primary_loss.new_zeros(())
            if (
                self.cfg.get("explicit_residual_loss_enabled", False)
                and _residual_only_mode
                and not self.ablate_context
                and proc_ctx is not None
                and target_col in preds
            ):
                delta_pred = getattr(
                    self.model_core, "_last_process_delta_tensor", None
                )
                if delta_pred is not None:
                    final_pred = preds[target_col]
                    target_shaped = target.view_as(final_pred)
                    avail_mask = proc_ctx["avail"].view(-1) > 0.5
                    finite_mask = (
                        torch.isfinite(target_shaped.view(-1))
                        & torch.isfinite(final_pred.view(-1))
                        & torch.isfinite(delta_pred.view(-1))
                    )
                    residual_mask = avail_mask & finite_mask
                    if bool(residual_mask.any()):
                        # Frozen base is detached explicitly to prevent accidental
                        # gradient flow if the configuration changes later.
                        frozen_base = (final_pred - delta_pred).detach()
                        ideal_delta = (target_shaped - frozen_base).detach()
                        beta = float(
                            self.cfg.get("explicit_residual_smooth_l1_beta", 0.05)
                        )
                        per_row_residual = F.smooth_l1_loss(
                            delta_pred.view(-1)[residual_mask],
                            ideal_delta.view(-1)[residual_mask],
                            beta=beta,
                            reduction="none",
                        )
                        # Reuse existing per-sample weights when configured.
                        if func_loss_weights is not None:
                            row_w = func_loss_weights.view(-1)[residual_mask]
                            residual_loss_value = (
                                per_row_residual * row_w
                            ).sum() / row_w.sum().clamp_min(1e-12)
                        else:
                            residual_loss_value = per_row_residual.mean()
                        residual_weight = float(
                            self.cfg.get("explicit_residual_loss_weight", 0.25)
                        )
                        loss = primary_loss + residual_weight * residual_loss_value
                        explicit_residual_loss_sum += float(
                            residual_loss_value.detach().item()
                        )
                        explicit_residual_batches += 1
                        explicit_residual_rows += int(residual_mask.sum().item())
        ...
        return {
            ...
            "explicit_residual_loss": explicit_residual_loss_sum / max(explicit_residual_batches, 1),
            "explicit_residual_batches": explicit_residual_batches,
            "explicit_residual_rows": explicit_residual_rows,
    ...
    def train(
        ...
        for epoch in range(1, n_epochs + 1):
            ...
            log.info(
                ...
                "base_drift_max=%.3e base_drift_l2=%.3e base_changed=%d  %s",
            ...
            history.append({
                ...
                "explicit_residual_loss": epoch_stats.get("explicit_residual_loss", 0.0),
                "explicit_residual_batches": epoch_stats.get("explicit_residual_batches", 0),
                "explicit_residual_rows": epoch_stats.get("explicit_residual_rows", 0),
                "val_mae":         val_mae,
                "val_rmse":        val_rmse,
...
def run_tier3_finetune(
    ...
    def _write_tier3_prediction_audit(
        ...
        with torch.no_grad():
            for batch in loader:
                ...
                for j, row_idx in enumerate(row_indices):
                    ...
                    ideal_delta_log = (true_log - base_l) if np.isfinite(true_log) and np.isfinite(base_l) else float("nan")
                    ideal_delta_k = (k_true - k_dft) if np.isfinite(k_true) and np.isfinite(k_dft) else float("nan")
                    delta_fit_abs_error_log = abs(delta_l - ideal_delta_log) if np.isfinite(delta_l) and np.isfinite(ideal_delta_log) else float("nan")
                    correct_delta_direction = bool(np.sign(delta_l) == np.sign(ideal_delta_log)) if np.isfinite(delta_l) and np.isfinite(ideal_delta_log) else False
                    sat_threshold = 0.95 * float(cfg.get("process_delta_bound", 0.15))
                    saturated_at_configured_bound = bool(abs(delta_l) >= sat_threshold) if np.isfinite(delta_l) else False
                    base_abs_error = abs(k_dft-k_true) if np.isfinite(k_dft) and np.isfinite(k_true) else float("nan")
                    residual_help_k = (base_abs_error-abs_error) if np.isfinite(base_abs_error) and np.isfinite(abs_error) else float("nan")
                    ...
                    rows.append({
                        ...
                        "ideal_delta_log": ideal_delta_log,
                        "ideal_delta_k": ideal_delta_k,
                        "delta_fit_abs_error_log": delta_fit_abs_error_log,
                        "correct_delta_direction": correct_delta_direction,
                        "configured_delta_bound": float(cfg.get("process_delta_bound", 0.15)),
                        "saturation_threshold_95pct": sat_threshold,
                        "saturated_at_configured_bound": saturated_at_configured_bound,
                        "base_abs_error": base_abs_error,
                        "residual_help_k": residual_help_k,
                        "residual_helped": bool(residual_help_k > 0) if np.isfinite(residual_help_k) else False,
                        "abs_error": abs_error,
                        "log_error": log_error,
                        "k_true_log": true_log,
                        "k_pred_log": pred_l,
        ...
        # v4.53 STAGES1-4: ideal-residual learnability diagnostics.  This does
        # not alter training; it measures how well the learned process delta
        # reproduces the checkpoint-specific ideal residual on each split.
        _ideal = pd.to_numeric(audit_df.get("ideal_delta_log"), errors="coerce")
        _learned = pd.to_numeric(audit_df.get("delta_log_pred"), errors="coerce")
        _valid_lr = _ideal.notna() & _learned.notna()
        _corr = float(_ideal[_valid_lr].corr(_learned[_valid_lr])) if int(_valid_lr.sum()) >= 2 and _ideal[_valid_lr].nunique() > 1 and _learned[_valid_lr].nunique() > 1 else float("nan")
        learnability_summary = pd.DataFrame([{
            "stage": stage_name,
            "split": split_name,
            "n": int(_valid_lr.sum()),
            "ideal_delta_log_mean": float(_ideal[_valid_lr].mean()) if _valid_lr.any() else float("nan"),
            "ideal_delta_log_std": float(_ideal[_valid_lr].std()) if _valid_lr.any() else float("nan"),
            "learned_delta_log_mean": float(_learned[_valid_lr].mean()) if _valid_lr.any() else float("nan"),
            "learned_delta_log_std": float(_learned[_valid_lr].std()) if _valid_lr.any() else float("nan"),
            "delta_fit_mae_log": float(pd.to_numeric(audit_df.get("delta_fit_abs_error_log"), errors="coerce").mean()),
            "delta_direction_accuracy_pct": 100.0 * float(pd.to_numeric(audit_df.get("correct_delta_direction"), errors="coerce").mean()),
            "ideal_vs_learned_delta_corr": _corr,
            "saturation_pct_configured_bound": 100.0 * float(pd.to_numeric(audit_df.get("saturated_at_configured_bound"), errors="coerce").mean()),
            "residual_help_rate_pct": 100.0 * float(pd.to_numeric(audit_df.get("residual_helped"), errors="coerce").mean()),
            "mean_residual_help_k": float(pd.to_numeric(audit_df.get("residual_help_k"), errors="coerce").mean()),
            "configured_delta_bound": float(cfg.get("process_delta_bound", 0.15)),
            "saturation_threshold_95pct": 0.95 * float(cfg.get("process_delta_bound", 0.15)),
        }])
        learnability_path = audit_dir / f"tier3_{safe_stage}_{safe_split}_ideal_residual_learnability.csv"
        learnability_summary.to_csv(learnability_path, index=False)
        ...
        log.info(
            "Tier3 audit written [%s/%s]: rows=%d  pred=%s  top=%s  material=%s  donor=%s  residual=%s learnability=%s",
            stage_name, split_name, len(audit_df), pred_path, top_path, mat_path, donor_path, resid_path, learnability_path,
        )
        return pred_path
...
def main():
    ...
    parser.add_argument(
        "--residual_loss_weight", type=float, default=None,
        help=(
            "Override explicit residual SmoothL1 auxiliary-loss weight. "
            "Recommended controlled sweep: 0.0, 0.25, 0.5, 1.0."
        ),
    )
    parser.add_argument(
        "--residual_smooth_l1_beta", type=float, default=None,
        help=(
            "Override SmoothL1 beta for explicit residual loss "
            "(default 0.05 log-k units)."
        ),
    )
    parser.add_argument(
        "--disable_explicit_residual_loss", action="store_true",
        help="Disable v4.54 explicit residual SmoothL1 loss for baseline reproduction.",
    )
    args = parser.parse_args()

    if args.residual_loss_weight is not None:
        if args.residual_loss_weight < 0:
            parser.error("--residual_loss_weight must be >= 0")
        TIER3_TRAIN_CONFIG["explicit_residual_loss_weight"] = args.residual_loss_weight
    if args.residual_smooth_l1_beta is not None:
        if args.residual_smooth_l1_beta <= 0:
            parser.error("--residual_smooth_l1_beta must be > 0")
        TIER3_TRAIN_CONFIG["explicit_residual_smooth_l1_beta"] = args.residual_smooth_l1_beta
    if args.disable_explicit_residual_loss:
        TIER3_TRAIN_CONFIG["explicit_residual_loss_enabled"] = False
    ...
    if args.mode == "tier3_finetune":
        log.info(
            "v4.54 explicit residual SmoothL1: enabled=%s weight=%.4f beta=%.4f",
            TIER3_TRAIN_CONFIG.get("explicit_residual_loss_enabled", False),
            float(TIER3_TRAIN_CONFIG.get("explicit_residual_loss_weight", 0.0)),
            float(TIER3_TRAIN_CONFIG.get("explicit_residual_smooth_l1_beta", 0.05)),
        )
    if torch.cuda.is_available():
