...
==============================================================================
 ...
 Version 4.5.10 (v4.13: k-valid subset dataloader + Phase A step cap)
==============================================================================
...
TIER3_TRAIN_CONFIG = dict(
    ...
    target            = "k_measured_log",   # was "k_measured" (linear)
    log_transform     = True,
    log_original_col  = "k_measured",
...
TASK_TO_COLUMN = {
    ...
    "k_measured_log":           "k_measured_log",   # FIX-T3-9A: log-space Tier 3 target
...
class MaskedMultiTaskLoss(nn.Module):
    ...
    HIGH_K_THRESHOLDS covers all three k-column variants:
      "k_measured"  / "k_total"  → linear scale: threshold = 35.0
      "k_total_log"              → log scale:    threshold = log(35) ≈ 3.555
    ...
    def __init__(
        ...
        self.task_weights = task_weights or {
            "k_measured":                2.0,   # Tier 3 primary (linear, legacy)
            # FIX-T3-10A: k_measured_log was absent → defaulted to 1.0 =
            # same as band_gap → equal gradient competition with only 5 valid
            # samples → backbone shifted and damaged band_gap representations.
            "k_measured_log":            2.0,   # Tier 3 primary (log-space, FIX-T3-9A)
            "k_total":                   2.0,   # Tier 1 aux (linear scale)
            # Review fix #3: k_total_log was missing → Tier 2 primary was
            # weighted 1.0 (default fallback) instead of 2.0.
            "k_total_log":               2.0,   # Tier 2 primary (log scale)
            "band_gap":                  1.0,
...
def _impute_structures(
    ...
    def _find_donor(material: str) -> Optional[pd.Series]:
        ...
        # 2. Reduced formula fallback
        m_red = _reduced(m)
        if m_red in reduced_map:
            return formula_to_donor[reduced_map[m_red]]
        # 3. Substring match for common aliases (e.g. "HfO2 (monoclinic)" → "HfO2")
    ...
    if len(df_unmatched) > 0:
        unmatched_formulas = (
            df_unmatched.get("material", pd.Series(dtype=str))
...
def run_tier3_finetune(
    ...
    df_imputed, df_unmatched = _impute_structures(df_process_only, df_structural)
    ...
    _kmeas_col = cfg["target"]  # "k_measured_log" after FIX-T3-9A
    ...
    # FIX-T3-KSUBSET: build Phase B dataloader on k_measured_log non-null subset.
    # Problem: df_structural has ~952 rows but only 139 (14.6%) have k_measured_log.
    # With batch_size=8: expected k signal per batch = 8 × 0.146 = 1.17 rows.
    # 85% of Phase B gradient steps carried ZERO k_measured_log signal — the model
    # optimised band_gap+formation_energy and effectively ignored k_measured_log.
    # Fix: filter to k-valid rows so every batch step carries 100% k signal.
    # Aux targets (band_gap, formation_energy) remain in the 139-row subset
    # because DFT rows in df_structural carry all three targets from JARVIS/MP.
    df_phase_b = df_structural[df_structural[cfg["target"]].notna()].copy()
    ...
    if phase_a_steps > 0 and df_tier2 is not None:
        ...
        if _phase_a_mode == "steps":
            # FIX-T3-PHASEACAP: Step-budget mode — exit after phase_a_steps total.
            # Use itertools.islice to consume exactly phase_a_steps batches across
            # repeated passes over pa_loader (in case budget > 1 epoch).
            ...
            # We need a scheduler with the right total step count for LR warmup.
            sched_pa = trainer.build_scheduler(phase_a_steps, 1)

            # Cycle the loader indefinitely; islice terminates after the budget.
            _cycled = _itools.cycle(pa_loader)
            _budget_loader = _itools.islice(_cycled, phase_a_steps)

            # Wrap in a one-element list so train_epoch sees a standard iterable.
            # Instead, call the inner forward/backward directly via a mini wrapper:
            _model    = trainer.model
            _opt      = trainer.optimizer
            _loss_fn  = trainer.loss_fn
            _scaler   = getattr(trainer, "scaler", None)
            _device   = trainer.device

            _model.train()
            _running_loss = 0.0
            for _batch in _budget_loader:
                if _batch is None:
                    continue
                _graph      = _batch["graph"].to(_device)
                _lg         = _batch["line_graph"].to(_device)
                _tgt        = _batch["target"].to(_device)
                _proc_ctx   = _batch.get("proc_context")
                _stack_ctx  = _batch.get("stack_context")
                if _proc_ctx is not None:
                    _proc_ctx  = {k: v.to(_device) for k, v in _proc_ctx.items()}
                    _stack_ctx = {k: v.to(_device) for k, v in _stack_ctx.items()}
                _func_code  = _batch.get("functional_code")
                if _func_code is not None:
                    _func_code = _func_code.to(_device)
                _exp_flag   = _batch.get("is_experimental")
                if _exp_flag is not None:
                    _exp_flag = _exp_flag.to(_device)
                ...
                if _scaler is not None:
                    ...
                    with autocast(dtype=torch.bfloat16):
                        _preds = _model(
                            _graph, _lg,
                            proc_context=_proc_ctx,
                            stack_context=_stack_ctx,
                            functional_code=_func_code,
                        )
                        _loss = _loss_fn(_preds, _tgt,
                                         functional_code=_func_code,
                                         is_experimental=_exp_flag)
                ...
                if sched_pa is not None:
                    sched_pa.step()

                _running_loss  = 0.9 * _running_loss + 0.1 * _loss.item()
                _pa_steps_done += 1

                if _pa_steps_done % 50 == 0 or _pa_steps_done == phase_a_steps:
                    log.info(
                        "Tier 3 Phase A  step %4d/%d  loss(ema)=%.4f",
                        _pa_steps_done, phase_a_steps, _running_loss,
                    )

            _pa_total_steps = _pa_steps_done
        else:
