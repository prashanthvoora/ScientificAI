...
TIER1_TRAIN_CONFIG = dict(
    ...
    # k_total_log: log-transformed aux head for k_total prediction consistency.
    # Training on log(k_total) in Tier 1 ensures the head weights are in log
    # space when transferred to Tier 2, which also uses k_total_log as target.
    # Without this, Tier 2 must unlearn the Tier 1 linear-scale head before
    # learning log-scale, wasting the first 20-30 fine-tuning epochs.
    aux_targets    = ["band_gap", "k_total_log"],   # k_total in log space
    log_transform_aux = {"k_total_log": "k_total"}, # source col for log transform
    train_ratio    = 0.80,
    val_ratio      = 0.10,
    test_ratio     = 0.10,
    patience       = 50,   # increased from 30: allows more exploration on large dataset
)
...
TIER2_TRAIN_CONFIG = dict(
    ...
    freeze_layers     = 2,
    unfreeze_after    = 20,            # epoch at which all layers are unfrozen
    patience          = 50,            # increased from 30: more exploration on 5K dataset
)
...
def run_tier1_pretrain(df_tier1: pd.DataFrame, ablate_context: bool = False):
    ...
    for log_col, src_col in cfg.get("log_transform_aux", {}).items():
        if src_col in df_tier1.columns:
            ...
            mask = df_tier1[src_col].notna()
            df_tier1[log_col] = np.nan
            df_tier1.loc[mask, log_col] = np.log(
                df_tier1.loc[mask, src_col].clip(lower=0.1)
            )
...
def run_tier2_finetune(
    ...
    if cfg.get("log_transform", False):
        ...
        df_t2_k[log_col] = np.log(df_t2_k[orig_col].clip(lower=0.1))
        log.info(
            "Log transform: %s → %s  range=[%.3f, %.3f]  "
            "(original k range=[%.1f, %.1f])",
            orig_col, log_col,
            df_t2_k[log_col].min(), df_t2_k[log_col].max(),
            df_t2_k[orig_col].min(), df_t2_k[orig_col].max(),
        )
    ...
    # Phase 1: freeze first N layers for stability (first unfreeze_after epochs)
    model.freeze_alignn_layers(cfg["freeze_layers"])

    trainer = ALIGNNTrainer(model, cfg, ckpt_prefix="tier2",
                            ablate_context=ablate_context)

    # Phase 1 (epochs 1 → unfreeze_after): lower layers frozen, warmup on k_total_log
    unfreeze_at = cfg.get("unfreeze_after", 20)
    log.info("Tier 2 Phase 1: lower %d layers frozen for first %d epochs",
             cfg["freeze_layers"], unfreeze_at)
    cfg_phase1         = {**cfg, "epochs": unfreeze_at}
    trainer.cfg        = cfg_phase1
    scheduler1         = trainer.build_scheduler(len(train_loader), unfreeze_at)
    for epoch in range(1, unfreeze_at + 1):
        trainer.train_epoch(train_loader, scheduler1, target_col)

    # Phase 2 (epochs unfreeze_after+1 → 150): all layers active, lower LR
    log.info("Tier 2 Phase 2: all layers unfrozen, lr=%.2e",
    ...
    remaining_epochs   = cfg["epochs"] - unfreeze_at
    cfg_phase2         = {**cfg, "epochs": remaining_epochs}
    trainer.cfg        = cfg_phase2
    history = trainer.train(train_loader, val_loader, target_col=target_col)
    ...
    test_mae, test_rmse = trainer.evaluate(test_loader, target_col)

    # If log-transformed, also report MAE in original k units for interpretability.
    # log-space MAE 0.1 ≈ predictions within exp(0.1)=1.11× of true value.
    if cfg.get("log_transform", False):
        log.info(
            "Tier 2 TEST  MAE(log)=%.4f  RMSE(log)=%.4f  "
            "[log scale; exp(MAE)=%.3f× = predict within ×%.2f of truth]",
            test_mae, test_rmse, np.exp(test_mae), np.exp(test_mae),
        )
        # Approximate original-unit MAE from log-space MAE for reporting.
        # (Exact value requires storing predictions; this is a distribution estimate.)
        k_mean = df_t2_k[cfg["log_original_col"]].mean()
        approx_k_mae = k_mean * (np.exp(test_mae) - 1)
        log.info(
            "Tier 2 TEST  MAE(k, approx)=%.2f  [original dielectric units, "
            "based on mean k=%.1f; run inference on test set for exact value]",
            approx_k_mae, k_mean,
        )
    else:
