...
==============================================================================
 ...
 Version 4.5.50 (v4.37: final finetune reloads persistent Tier-3 split)
==============================================================================
...
# v4.53: experimental-only Tier-3 uses its own persistent split file so it
# cannot accidentally reuse the older mixed DFT+experimental TEST population.
TIER3_TEST_SPLIT_PATH = SPLIT_ROOT / "tier3_experimental_only_test_row_ids.csv"
...
TIER2_TRAIN_CONFIG = dict(
    ...
    # v4.53 TIER2-PHASE-C: project-family DFT specialization.
    # Phase A learns broad oxide representations; Phase B learns broad DFT k;
    # Phase C gently specializes the resulting checkpoint on Hf/Zr oxide DFT
    # rows that previously appeared only as zero-context anchors in Tier 3.
...
TIER3_TRAIN_CONFIG = dict(
    ...
    # v4.53 TIER3-EXP-ONLY: Tier 3 learns process residuals only from
    # experimental rows carrying at least one ALD process feature. DFT rows are
    # moved conceptually into Tier-2 Phase C and no longer enter Tier-3 splits.
...
def run_tier2_finetune(
    ...
    # ── Phase C (v4.53): low-LR Hf/Zr DFT specialization ─────────────────
    # Start from the best broad-k Phase-B checkpoint, then specialize only on
    # project-family DFT rows. The original Tier-2 TEST loader remains untouched
    # and is used below to measure whether specialization improved or degraded
    # broad Tier-2 generalization.
    if cfg.get("phase_c_hfzr_specialization", False):
        dist_barrier()
        phase_b_best = CKPT_ROOT / "tier2_best.pt"
        phase_b_ckpt = safe_load_checkpoint(phase_b_best)
        if phase_b_ckpt is not None:
            trainer.model_core.load_state_dict(phase_b_ckpt["model_state_dict"], strict=False)

        def _is_hfzr_oxide_formula(formula) -> bool:
            if not isinstance(formula, str):
                return False
            return "O" in formula and ("Hf" in formula or "Zr" in formula)

        df_t2_hfzr = df_t2_k[df_t2_k["formula"].apply(_is_hfzr_oxide_formula)].copy()
        min_rows = int(cfg.get("phase_c_min_rows", 12))
        log.info(
            "Tier 2 Phase C: Hf/Zr DFT specialization candidates=%d  min_required=%d",
            len(df_t2_hfzr), min_rows,
        )
        if len(df_t2_hfzr) >= min_rows:
            pc_train, pc_val, _pc_test, pc_sampler, _pc_ds = build_dataloader(
                df=df_t2_hfzr,
                target_col=target_col,
                aux_cols=cfg["aux_targets"],
                train_frac=float(cfg.get("phase_c_train_ratio", 0.80)),
                val_frac=float(cfg.get("phase_c_val_ratio", 0.10)),
                batch_size=min(cfg["batch_size"], max(4, len(df_t2_hfzr) // 4)),
            )
            pc_cfg = {
                **cfg,
                "epochs": int(cfg.get("phase_c_epochs", 20)),
                "learning_rate": float(cfg.get("phase_c_learning_rate", 2e-5)),
                "learning_rate_unfreeze": float(cfg.get("phase_c_learning_rate", 2e-5)),
                "scheduler": "cosine",
                "early_stopping": False,
                "min_epochs": 0,
                "patience": int(cfg.get("phase_c_epochs", 20)) + 1,
            }
            pc_trainer = ALIGNNTrainer(
                model, pc_cfg, ckpt_prefix="tier2_hfzr",
                ablate_context=ablate_context,
            )
            pc_trainer.optimizer = torch.optim.AdamW(
                model.parameters(),
                lr=pc_cfg["learning_rate"],
                weight_decay=cfg["weight_decay"],
            )
            pc_trainer.train(pc_train, pc_val, target_col=target_col, train_sampler=pc_sampler)

            dist_barrier()
            pc_best = CKPT_ROOT / "tier2_hfzr_best.pt"
            pc_ckpt = safe_load_checkpoint(pc_best)
            if pc_ckpt is not None:
                trainer.model_core.load_state_dict(pc_ckpt["model_state_dict"], strict=False)
                # Promote the specialized candidate to the canonical Tier-2
                # checkpoint consumed by Tier-3, while retaining the Phase-B
                # file for controlled rollback/comparison.
                if is_rank0():
                    backup = CKPT_ROOT / "tier2_phase_b_best.pt"
                    if phase_b_best.exists():
                        shutil.copy2(phase_b_best, backup)
                    tmp_promote = CKPT_ROOT / "tier2_best.pt.promote.tmp"
                    shutil.copy2(pc_best, tmp_promote)
                    tmp_promote.replace(phase_b_best)
                    log.info(
                        "v4.53 PHASE-C PROMOTE: %s -> %s  (Phase-B backup: %s)",
                        pc_best, phase_b_best, backup,
                    )
                dist_barrier()
            else:
                log.warning("Tier 2 Phase C produced no checkpoint; retaining Phase-B tier2_best.pt")
        else:
            log.warning(
                "Tier 2 Phase C skipped: only %d Hf/Zr k-valid DFT rows (<%d)",
                len(df_t2_hfzr), min_rows,
            )
...
def run_tier3_finetune(
    ...
    """
    log.info("=" * 70)
    log.info(" TIER 3 -- Experimental Process-Residual Fine-tune")
    ...
    # v4.53 TIER3-EXP-ONLY: retain only experimental rows for residual
    # learning. Optionally require at least one process feature so every row
    # can activate the residual branch. This creates a strict boundary:
    # Tier 2 = structural DFT learning; Tier 3 = process correction.
    if cfg.get("experimental_only", False):
        n_before_exp_only = len(df_structural)
        if "source" not in df_structural.columns:
            raise RuntimeError("TIER3-EXP-ONLY requires a 'source' column")
        df_structural = df_structural[
            df_structural["source"].astype(str).eq("Experimental")
        ].copy()

        if cfg.get("require_process_context", True):
            proc_cols = (
                list(PROCESS_PARAMS_FEATURES["numerical"]) +
                list(PROCESS_PARAMS_FEATURES["categorical"].keys())
            )
            present_cols = [c for c in proc_cols if c in df_structural.columns]
            if not present_cols:
                raise RuntimeError(
                    "TIER3-EXP-ONLY found no process-feature columns after Tier-3 build"
                )
            has_proc = df_structural[present_cols].notna().any(axis=1)
            n_without_proc = int((~has_proc).sum())
            df_structural = df_structural[has_proc].copy()
        else:
            n_without_proc = 0

        df_structural = df_structural.reset_index(drop=True)
        log.info(
            "v4.53 TIER3-EXP-ONLY: %d -> %d rows  dropped_nonexperimental=%d  "
            "dropped_no_process_context=%d",
            n_before_exp_only, len(df_structural),
            n_before_exp_only - len(df_structural) - n_without_proc,
            n_without_proc,
        )
        if len(df_structural) == 0:
            raise RuntimeError(
                "TIER3-EXP-ONLY produced zero usable rows; check structure imputation and process columns"
            )
...
def run_tier_evaluate(
    ...
    if tier == 1:
        ...
        # v4.53: reproduce the same experimental-only/process-present universe
        # used by tier3_finetune before loading the dedicated persistent split.
        if cfg.get("experimental_only", False):
            n_before_eval_exp = len(df_eval)
            if "source" not in df_eval.columns:
                raise RuntimeError("tier3_evaluate experimental-only mode requires 'source'")
            df_eval = df_eval[df_eval["source"].astype(str).eq("Experimental")].copy()
            if cfg.get("require_process_context", True):
                proc_cols = (list(PROCESS_PARAMS_FEATURES["numerical"]) +
                             list(PROCESS_PARAMS_FEATURES["categorical"].keys()))
                present_cols = [c for c in proc_cols if c in df_eval.columns]
                if not present_cols:
                    raise RuntimeError("tier3_evaluate found no process-feature columns")
                has_proc = df_eval[present_cols].notna().any(axis=1)
                n_no_proc_eval = int((~has_proc).sum())
                df_eval = df_eval[has_proc].copy()
            else:
                n_no_proc_eval = 0
            df_eval = df_eval.reset_index(drop=True)
            log.info(
                "v4.53 T3-EVAL-EXP-ONLY: %d -> %d rows  no_process=%d",
                n_before_eval_exp, len(df_eval), n_no_proc_eval,
            )
