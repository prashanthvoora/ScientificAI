...
==============================================================================
 ...
 Version 4.60 (locked disjoint VAL/TEST baseline)
==============================================================================

 v4.56 changes (controlled base-head adaptation)
 ---------------------------------------------------------------------------
 BASE-HEAD-ADAPT  During bounded Tier-3 residual training, optionally train only
           the Tier-2 k_total_log task head together with the process residual.
           The ALIGNN backbone and functional-conditioning path remain frozen.

 BASE-HEAD-REFERENCE  A frozen, dropout-disabled copy of the loaded Tier-2
           k_total_log head supplies the original base prediction for every row.

 BASE-DRIFT-LOSS  Adds lambda * MSE(adapted_base_log, original_base_log),
           preventing the small Tier-3 dataset from overwriting Tier-2 knowledge.

 TWO-LR-OPTIMIZER  Process branch and base k head use independent AdamW groups.
           Default process LR=1e-4; base-head LR=1e-5.

 BASE-ADAPT-DIAG  Training history records primary loss, drift loss, weighted
           drift contribution, base-head gradient norm, parameter movement,
           and mean/max per-batch base prediction movement.
==============================================================================

 v4.51 changes (dual-scale metric reporting)
 ---------------------------------------------------------------------------
 DUAL-SCALE-METRICS  Tier-2 and Tier-3 k_total test tables now report
           MAE(log), RMSE(log), exact MAE(linear), exact RMSE(linear), and
           MAD:MAE(linear) together. All five primary metrics are computed
           from the same valid TEST prediction/target tensors. Non-log task
           heads retain native-scale MAE/RMSE in the linear columns.
...
def init_dist() -> bool:
    ...
    if "RANK" not in os.environ:
        ...
        return False
    # v4.43 DDP-GPU-BIND: torchrun exposes LOCAL_RANK for the GPU local to
    # this node. Bind CUDA before creating the NCCL process group and pass an
    # explicit device_id so NCCL never has to guess rank-to-GPU mapping.
    ...
    log.info("[rank %d/%d] DDP active on cuda:%d", rank, world, local_rank)
    return True
...
def _read_hdf_with_retry(path: Path, key: str = "data", retries: int = 8, delay_s: float = 0.5) -> pd.DataFrame:
    """Read an HDF5 dataframe with bounded retry for transient filesystem locks.

    Concurrent readers are safe, but a reader can briefly observe an HDF5 lock
    while rank 0 is atomically replacing a freshly rebuilt cache.  The DDP
    barrier normally prevents this; retries make the path robust on NFS/Lustre.
    """
    last_exc = None
...
def _atomic_write_hdf(df: pd.DataFrame, path: Path, key: str = "data") -> None:
    """Write HDF5 through a temporary file and atomically replace the cache.

    This function must be called by rank 0 only in distributed runs.  Atomic
    replacement prevents other ranks from seeing a partially written HDF5 file.
    """
    path.parent.mkdir(parents=True, exist_ok=True)
    tmp = path.with_name(f".{path.name}.rank{int(_DIST.get('rank', 0))}.tmp")
    try:
        if tmp.exists():
            tmp.unlink()
        df.to_hdf(tmp, key=key, mode="w", complevel=6, complib="blosc")
        os.replace(tmp, path)
    finally:
        if tmp.exists():
            try:
...
# v4.34: persistent Tier-3 split files.
# These make tier3_finetune, tier3_evaluate, and --ablate_context runs
# evaluate the exact same test row IDs, so MAE/RMSE/MAD:MAE comparisons are
# apples-to-apples.
SPLIT_ROOT = REPORT_ROOT / "tier3_splits"
SPLIT_ROOT.mkdir(parents=True, exist_ok=True)
TIER3_TEST_SPLIT_PATH = SPLIT_ROOT / "tier3_test_row_ids.csv"
# DISJOINT-VAL (v4.59): persistent VAL split, carved from TRAIN and kept disjoint
# from TEST. Saved for provenance/auditing only; tier3_evaluate reports TEST.
TIER3_VAL_SPLIT_PATH = SPLIT_ROOT / "tier3_val_row_ids.csv"
...
TIER3_TRAIN_CONFIG = dict(
    # v4.49 LOCKED-T3-CONFIG: validated baseline used for all subsequent changes.
    # Best observed result: linear MAE=3.2173, MAD:MAE=1.44, best epoch=23.
    epochs         = 100,
    ...
    learning_rate  = 1e-4,   # v4.38: process-delta head only; backbone/base head remain frozen
    weight_decay   = 1e-4,
    scheduler      = "cosine",
    loss           = "mse",
    max_grad_norm  = 1.0,
    ...
    aux_targets    =[],
    train_ratio    = 0.70,
    val_ratio      = 0.00,  # v4.31: train/test-only split; former val rows join test
    test_ratio     = 0.30,
    ...
    exp_sample_weight = 1.0,  # v4.49 LOCKED

    # FIX-T3-9C: explicit early stopping tuned for small noisy dataset.
    early_stopping = True,
    min_epochs     = 20,    # v4.38: allow convergence while avoiding unnecessary overfit
    patience       = 20,    # v4.38: stop after 20 epochs without improvement
    ...
    process_delta_bound      = 0.15,  # v4.49 LOCKED: validated bounded residual
    process_delta_use_categorical = True,  # v4.49 LOCKED: categorical process embeddings enabled
    # v4.56: controlled adaptation of the Tier-2 k_total_log output head.
    # The ALIGNN backbone remains frozen.  A frozen reference copy of the loaded
    # Tier-2 head anchors the adapted base through base_drift_weight.
    process_delta_train_base_head = False,  # v4.56: original Tier-2 head remains frozen
    tier3_use_structural_adapter = True,
    tier3_structural_adapter_rank = 16,
    tier3_structural_adapter_learning_rate = 5e-5,
    tier3_process_learning_rate   = 1e-4,
    tier3_structural_adapter_weight_decay = 1e-4,
    tier3_adapter_magnitude_weight = 1.0,
    tier3_adapter_center_weight = 5.0,
    tier3_process_center_weight = 5.0,
    tier3_adapter_process_orthogonal_weight = 1.0,
    tier3_process_weight_decay    = 1e-4,
    tier3_base_change_warn_mean   = 0.03,
    tier3_base_change_warn_max    = 0.10,
    process_delta_init_seed = 42,  # deterministic Tier-3-only residual initialization
    frozen_base_state_tolerance = 1e-8,  # v4.46 hard guard for parameters + buffers
    ...
    # v4.40 AUDIT: retain intermediate residual checkpoints for trajectory audit.
    audit_checkpoint_epochs = [1, 10, 20, 40, 80, 100],  # v4.49 LOCKED audit trajectory
    audit_write_dataset_composition = True,
    audit_write_optimizer_manifest = True,

    # EVAL-FAMILY-SPLIT: train on the full expanded high-k process database,
    # but reserve validation/test exclusively for the project target family
    # (HfO2/HZO/Hf-Zr-O/doped-HfO2). This makes the Tier-3 metric comparable
    # to the old v3 database, which was mostly HfO2-family, while still using
    # the wider 410-row expansion as auxiliary training signal.
    # Non-HfO2-family rows are included only in TRAIN. HfO2-family rows are
    # split into train/val/test, so validation/test rows are not leaked into train.
    eval_hfo2_family_only = True,

    # QUALITY-AWARE-EVAL: compact v4 DB labels rows as
    #   verified      -> allowed in train/val/test
    #   weak_expanded -> train-only auxiliary signal
    #   reject        -> dropped during CSV load
    # Validation/test are restricted to verified rows by default so synthetic
    # or weakly grounded expansion rows cannot depress or inflate headline metrics.
    eval_verified_only = True,

    # ── DISJOINT-VAL (v4.59) ────────────────────────────────────────────────
    # Checkpoint selection / early stopping must use a validation set that is
    # DISJOINT from TEST. When enabled, a held-out VAL set is carved from the
    # eval-family verified TRAIN pool BEFORE any gradient step (including the
    # epoch-0 baseline seeding). TEST is left completely untouched and remains
    # the once-only holdout reported by tier3_evaluate — so the headline metric
    # is no longer a min-over-epochs on the test rows.
    # When disabled, legacy v4.58 behaviour (VAL == TEST rows) is reproduced
    # exactly, so old numbers remain comparable.
    tier3_disjoint_val       = True,
    tier3_disjoint_val_ratio = 0.30,   # v4.60.1: raised 0.20->0.30 so carved VAL clears tier3_disjoint_val_min_rows (15) on the ~65-row eval-family pool (0.20 gave ~13 -> abort)
    tier3_disjoint_val_seed  = 42,     # deterministic hold-out seed
    # v4.59.1: minimum eval-family verified candidate pool required to form a
    # disjoint VAL. Below this the run ABORTS (hard error) rather than silently
    # reverting to VAL==TEST. Lower this only as a deliberate, reviewed choice.
    tier3_disjoint_val_min_pool = 15,
    tier3_disjoint_val_min_rows = 15,
    tier3_lock_persistent_splits = True,
    tier3_allow_split_creation = True,  # set False after first accepted baseline run

    # v4.31 donor/material-family calibration. These deterministic log(k)
    # offsets are applied on top of the frozen DFT baseline before any learned
    # residual. They are intentionally conservative and interpretable.
    enable_material_family_offset = False,  # v4.38: isolate learned process value from manual calibration
    material_family_offsets = {
        "pure_hfo2": 0.00,
        "hzo_like": 0.20,
        "doped_hfo2_al": 0.15,
        "doped_hfo2_la": 0.15,
        "doped_hfo2_other": 0.10,
        "zro2_like": 0.05,
        "other_highk": 0.00,
    },
    phase_aware_donor_selection = True,
    donor_log_topn = 5,
)
...
class DatasetExtractor:
    ...
    def load_experimental_process_db(
        ...
        # QUALITY-AWARE-DB v4.28:
        # Optional compact DB column. Rows marked reject are excluded at source.
        # Rows marked weak_expanded are retained for TRAIN only by the custom
        # Tier-3 split; they are never used in validation/test by default.
        if "data_quality" in df.columns:
            df["data_quality"] = df["data_quality"].fillna("verified").astype(str).str.strip().str.lower()
            q_counts = df["data_quality"].value_counts(dropna=False).to_dict()
            log.info("QUALITY-AWARE-DB: input data_quality counts: %s", q_counts)
            n_before_q = len(df)
            df = df[df["data_quality"] != "reject"].copy()
            if len(df) != n_before_q:
                log.warning("QUALITY-AWARE-DB: dropped %d reject rows before Tier-3 build", n_before_q - len(df))
        else:
            df["data_quality"] = "verified"

        if "data_split_policy" not in df.columns:
            df["data_split_policy"] = np.where(
                df["data_quality"].eq("weak_expanded"),
                "train_only_no_val_test",
                "train_val_test_allowed",
            )
...
class TierDatasetBuilder:
    ...
    def build_tier3(
        ...
        """
        tier3_path = self.TIER_PATHS[3]

        # v4.46 HDF5-DDP-LOCK: only rank 0 may rebuild/write the shared HDF5
        # cache.  All other ranks wait, then read the completed atomic file.
        if _DIST["active"] and force_rebuild and not is_rank0():
            log.info("[rank %d] Waiting for rank 0 to rebuild Tier 3 cache: %s",
                     _DIST["rank"], tier3_path)
            dist_barrier()
            if not tier3_path.exists():
                raise FileNotFoundError(
                    f"Rank 0 did not produce Tier 3 cache at {tier3_path}"
                )
            return _read_hdf_with_retry(tier3_path, key="data")

        if tier3_path.exists() and not force_rebuild:
            # Synchronize before shared reads in case rank 0 has just completed
            # another cache-producing stage on a slow/network filesystem.
            dist_barrier()
            log.info("Loading existing Tier 3 from %s", tier3_path)
            return _read_hdf_with_retry(tier3_path, key="data")

        log.info("Deriving Tier 3 project dataset%s...",
                 " on rank 0" if _DIST["active"] else "")
        ...
        # v4.46 HDF5-DDP-LOCK: rank 0 is the sole writer.  Use atomic
        # replacement so waiting ranks never observe a partially written file.
        if not _DIST["active"] or is_rank0():
            _atomic_write_hdf(df_tier3, tier3_path, key="data")
        ...
        if _DIST["active"]:
            # Release non-zero ranks blocked at the entry barrier only after
            # the atomic cache replacement and manifest update are complete.
            dist_barrier()
        return df_tier3
...
def infer_material_family_from_row(row) -> str:
    """Return a compact material-family label used for donor rules and offsets.

    The function prefers the curated material_family column from the compact DB,
    then falls back to material/formula string parsing. It must never inspect the
    target k value, so it is safe for train/val/test usage.
    """
    fam = str(row.get("material_family", "")).strip().lower()
    if fam and fam not in ("nan", "none", "", "unknown"):
        return fam
    s = (str(row.get("material", row.get("material_system", ""))) + " " +
         str(row.get("formula", ""))).lower().replace(" ", "")
    if "hfo2" in s and not any(x in s for x in ["zr", "al", "la", "y", "gd", "si", "sr"]):
        return "pure_hfo2"
    if "hzo" in s or ("hf" in s and "zr" in s and "o" in s) or "hf0." in s:
        return "hzo_like"
    if ("alhfo" in s or "hfalo" in s or ("hf" in s and "al" in s and "o" in s)):
        return "doped_hfo2_al"
    if ("lahfo" in s or "hfl ao" in s or ("hf" in s and "la" in s and "o" in s)):
        return "doped_hfo2_la"
    if "hf" in s and "o" in s:
        return "doped_hfo2_other"
    if "zro2" in s or ("zr" in s and "o" in s):
        return "zro2_like"
    return "other_highk"

def material_family_log_offset_from_row(row) -> float:
    """Deterministic, conservative log(k) correction by material family."""
    if not TIER3_TRAIN_CONFIG.get("enable_material_family_offset", True):
        return 0.0
    fam = infer_material_family_from_row(row)
    offsets = TIER3_TRAIN_CONFIG.get("material_family_offsets", {})
    return float(offsets.get(fam, 0.0))
...
class HighKGraphDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        proc_context = {
            ...
            # v4.31: deterministic material-family calibration offset in log(k).
            # This remains active even when learned process_delta_bound=0.
            "family_offset": torch.tensor([material_family_log_offset_from_row(row)], dtype=torch.float32),
        }
        stack_context = {
...
def get_stratified_split(
    ...
    try:
        ...
        elif test_ratio >= 1.0 - 1e-9:
            # EVAL-SPLIT-FIX v4.33:
            # Train/test-only Tier3 evaluation uses val_frac=0.0. In that case
            # all temp rows must go to TEST, not through sklearn with
            # test_size=1.0, which raises:
            #   test_size=1.0 should be either positive and smaller than...
            val_idx  = np.array([], dtype=int)
            test_idx = temp_idx
            log.info(
                "Train/test-only split: val_frac=0 → assigning all %d temp rows to TEST",
                len(temp_idx),
            )
        else:
...
class HighKALIGNN(nn.Module):
    ...
    def __init__(
        ...
        #   y_exp_log = y_DFT_log + process_delta_bound * tanh(delta_raw)
        ...
        # v4.38 diagnostics populated by _process_bounded_delta().
        self._last_process_delta_mean_abs = 0.0
        self._last_process_delta_max_abs  = 0.0
        self._last_process_delta_active_pct = 0.0
        self._last_process_delta_tensor = None

        # v4.56 LOW-RANK STRUCTURAL ADAPTER. The original Tier-2 k head stays
        # frozen. This zero-initialized branch learns only a small structure-
        # dependent correction in log(k): frozen_base + adapter_delta + process_delta.
        self.tier3_structural_adapter_rank = 16
        self.tier3_structural_adapter = nn.Sequential(
            nn.Linear(backbone_out_dim, self.tier3_structural_adapter_rank, bias=False),
            nn.SiLU(),
            nn.Linear(self.tier3_structural_adapter_rank, 1, bias=True),
        )
        nn.init.zeros_(self.tier3_structural_adapter[-1].weight)
        nn.init.zeros_(self.tier3_structural_adapter[-1].bias)
        self.use_tier3_structural_adapter = False
        self._last_structural_delta = None
        self._last_structural_delta_mean = 0.0
        self._last_structural_delta_std = 0.0
        self._last_structural_delta_mean_abs = 0.0
        self._last_structural_delta_max_abs = 0.0
        self._last_adapter_process_correlation = 0.0

        # Legacy v4.56 diagnostics retained for backward-compatible audit files.  The frozen
        # reference head is attached with object.__setattr__ so it is not
        # registered in state_dict/DDP and can never be optimized accidentally.
        object.__setattr__(self, "_tier3_base_head_reference", None)
        self._last_adapted_base_log = None
        self._last_reference_base_log = None
        self._last_base_change_mean_abs = 0.0
        self._last_base_change_max_abs = 0.0
    ...
    def _categorical_embedding_named_parameters(self):
        """Yield process categorical embedding parameters used by residual path."""
        if not bool(getattr(self, "process_delta_use_categorical", False)):
            return []
        return [
            (f"proc_encoder.embeddings.{name}", param)
            for name, module in self.proc_encoder.embeddings.items()
            for param_name, param in module.named_parameters()
            for name in [f"{name}.{param_name}"]
        ]

    def freeze_for_bounded_process_residual(self, train_base_head: bool = False):
        """v4.56 freeze complete Tier-2 base; train process branch + structural adapter."""
        for p in self.parameters():
            p.requires_grad = False
        for p in self.process_delta_head.parameters():
            p.requires_grad = True
        if self.process_delta_use_categorical:
            for p in self.proc_encoder.embeddings.parameters():
                p.requires_grad = True
        if self.use_tier3_structural_adapter:
            for p in self.tier3_structural_adapter.parameters():
                p.requires_grad = True
        object.__setattr__(self, "_tier3_base_head_reference", None)
        self._residual_train_base_head = False
        n_trainable = sum(p.numel() for p in self.parameters() if p.requires_grad)
        log.info(
            "v4.56 RESIDUAL+ADAPTER freeze: Tier-2 head/backbone frozen; "
            "structural_adapter=%s rank=%d trainable_params=%d",
            self.use_tier3_structural_adapter, self.tier3_structural_adapter_rank, n_trainable,
        )

    def set_residual_only_train_mode(self, ablate_context: bool = False):
        """Keep the frozen base deterministic and train only the residual head.

        ``self.eval()`` disables dropout and prevents normalization-buffer drift
        throughout ALIGNN, functional conditioning, task heads, and context
        encoders. The residual head alone is returned to train mode for the
        normal process-aware experiment. In ablation mode every module remains
        in eval mode because no trainable path is intentionally used.
        """
        self.eval()
        if not ablate_context:
            self.process_delta_head.train()
            if self.process_delta_use_categorical:
                self.proc_encoder.embeddings.train()
        if self.use_tier3_structural_adapter and not ablate_context:
            self.tier3_structural_adapter.train()
        # Frozen Tier-2 head always stays in eval mode.
        if "k_total_log" in self.task_heads:
            self.task_heads["k_total_log"].eval()
    ...
    def reset_bounded_process_residual(self, seed: int = 42):
        """Deterministically reset Tier-3-only residual parameters.

        Tier 2 contains no process context, so ``process_delta_head`` and the
        process categorical embeddings do not learn meaningful Tier-2 state.
        Resetting them after loading ``tier2_best.pt`` prevents an otherwise
        irrelevant checkpoint-specific random initialization from changing the
        Tier-3 optimization trajectory.  The final residual layer is restored
        to exact zeros so epoch-0 predictions remain identical to the frozen
        Tier-2 base prediction.
        """
        seed = int(seed)
        cuda_devices = []
        ...
        reset_names = []
        # fork_rng preserves the caller's RNG stream while making this reset
        # identical across runs and across Tier-2 checkpoints.
        with torch.random.fork_rng(devices=cuda_devices, enabled=True):
            torch.manual_seed(seed)
            if torch.cuda.is_available():
                torch.cuda.manual_seed_all(seed)

            for name, module in self.process_delta_head.named_modules():
                if name and isinstance(module, nn.Linear):
                    module.reset_parameters()
                    reset_names.append(f"process_delta_head.{name}")

            if bool(getattr(self, "process_delta_use_categorical", False)):
                for name, module in self.proc_encoder.embeddings.items():
                    module.reset_parameters()
                    reset_names.append(f"proc_encoder.embeddings.{name}")

            # Preserve the residual architecture's exact frozen-base start.
            nn.init.zeros_(self.process_delta_head[-1].weight)
            nn.init.zeros_(self.process_delta_head[-1].bias)

        log.info(
            "T3-PROC-RESET: deterministic Tier-3 residual reset seed=%d; "
            "modules=%s; final_layer_zero=True",
            seed,
            reset_names,
        )

    def configure_structural_adapter(
        self,
        enabled: bool = True,
        rank: int = 16,
        reset: bool = False,
    ):
        """Configure the low-rank structural correction.

        ``reset=True`` is used only when beginning a fresh Tier-3 finetune from a
        Tier-2 checkpoint.  Evaluation/checkpoint restore must use reset=False so
        learned adapter weights are never overwritten after state_dict loading.
        """
        rank = int(rank)
        in_dim = ALIGNN_BASE_CONFIG["hidden_features"]
        rebuilt = False
        if rank != self.tier3_structural_adapter_rank:
            dev = next(self.parameters()).device
            self.tier3_structural_adapter_rank = rank
            self.tier3_structural_adapter = nn.Sequential(
                nn.Linear(in_dim, rank, bias=False),
                nn.SiLU(),
                nn.Linear(rank, 1, bias=True),
            ).to(dev)
            rebuilt = True
        if reset or rebuilt:
            nn.init.zeros_(self.tier3_structural_adapter[-1].weight)
            nn.init.zeros_(self.tier3_structural_adapter[-1].bias)
        self.use_tier3_structural_adapter = bool(enabled)
        log.info(
            "v4.58 STRUCTURAL-ADAPTER: enabled=%s rank=%d reset=%s rebuilt=%s",
            enabled, rank, reset, rebuilt,
        )

    def _structural_adapter_delta(self, embedding: torch.Tensor) -> torch.Tensor:
        if not self.use_tier3_structural_adapter:
            delta = torch.zeros(embedding.shape[0], 1, device=embedding.device)
        else:
            # detach is an explicit scientific guard: no gradient can enter the
            # ALIGNN backbone or functional-conditioning path through this branch.
            delta = self.tier3_structural_adapter(embedding.detach())
        self._last_structural_delta = delta
        d = delta.detach().reshape(-1)
        self._last_structural_delta_mean = float(d.mean().item()) if d.numel() else 0.0
        self._last_structural_delta_std = float(d.std(unbiased=False).item()) if d.numel() else 0.0
        self._last_structural_delta_mean_abs = float(d.abs().mean().item()) if d.numel() else 0.0
        self._last_structural_delta_max_abs = float(d.abs().max().item()) if d.numel() else 0.0
        return delta
    ...
    def _process_bounded_delta(self, proc_context: Optional[dict]) -> Optional[torch.Tensor]:
        ...
        if proc_context is None:
            return None
        dev = next(self.parameters()).device
        family_offset = proc_context.get("family_offset", None)
        if family_offset is not None:
            family_offset = family_offset.to(dev)
        else:
            # Backward-compatible path for old batches/checkpoints.
            num0 = proc_context["num"].to(dev)
            family_offset = torch.zeros(num0.shape[0], 1, device=dev)
        ...
        if not bool(avail.any()):
            ...
            learned_zero = 0.0 * self.process_delta_head(dummy_in)
            total_delta = family_offset + learned_zero
            self._last_process_delta_mean_abs = float(total_delta.detach().abs().mean().item())
            self._last_process_delta_max_abs = float(total_delta.detach().abs().max().item())
            self._last_process_delta_active_pct = 0.0
            self._last_process_delta_tensor = learned_zero
            return total_delta
        ...
        learned_delta = self.process_delta_bound * torch.tanh(raw) * avail
        # v4.31: add deterministic material-family offset separately from the
        # learned residual. v4.38 disables this offset by default so diagnostics
        # isolate the learned process correction.
        total_delta = family_offset + learned_delta
        self._last_process_delta_mean_abs = float(total_delta.detach().abs().mean().item())
        self._last_process_delta_max_abs = float(total_delta.detach().abs().max().item())
        self._last_process_delta_active_pct = float(100.0 * avail.detach().mean().item())
        self._last_process_delta_tensor = learned_delta
        return total_delta

    def _record_base_adaptation(self, embedding: torch.Tensor, adapted_base: torch.Tensor):
        """Record adapted and frozen-reference base predictions for drift loss."""
        ref_head = getattr(self, "_tier3_base_head_reference", None)
        if ref_head is None:
            self._last_adapted_base_log = adapted_base
            self._last_reference_base_log = adapted_base.detach()
            self._last_base_change_mean_abs = 0.0
            self._last_base_change_max_abs = 0.0
            return
        with torch.no_grad():
            reference_base = ref_head(embedding.detach())
        self._last_adapted_base_log = adapted_base
        self._last_reference_base_log = reference_base.detach()
        diff = (adapted_base.detach() - reference_base.detach()).abs()
        self._last_base_change_mean_abs = float(diff.mean().item()) if diff.numel() else 0.0
        self._last_base_change_max_abs = float(diff.max().item()) if diff.numel() else 0.0
        ...
        if self.use_bounded_process_residual:
            ...
            if task == "__all__":
                ...
                for t, head in self.task_heads.items():
                    base = head(embedding)
                    if t == "k_total_log":
                        structural_delta = self._structural_adapter_delta(embedding)
                        out[t] = base + structural_delta + (delta if delta is not None else 0.0)
                    else:
                        out[t] = base
                return out
            ...
            if task == "k_total_log":
                structural_delta = self._structural_adapter_delta(embedding)
                return base + structural_delta + (delta if delta is not None else 0.0)
            return base
...
def _load_state_dict_shape_compatible(
    model: nn.Module, state: dict, checkpoint_label: str = "checkpoint"
):
    """Load exact tensors and migrate expanded categorical embedding tables.

    ``strict=False`` accepts missing/unexpected keys but not size mismatches.
    For a resized process embedding, copy the shared checkpoint rows into the
    current table and retain initialization only for newly added vocabulary rows.
    """
    key_remap = {
        "task_heads.k_measured_log.": "task_heads.k_total_log.",
        "task_heads.k_measured.":     "task_heads.k_total.",
    ...
    for key, value in renamed.items():
        ...
        if tuple(value.shape) == tuple(expected.shape):
            compatible[key] = value
        elif (key.startswith("proc_encoder.embeddings.")
              and key.endswith(".weight")
              and value.ndim == 2 and expected.ndim == 2):
            migrated = expected.detach().clone()
            rows = min(value.shape[0], expected.shape[0])
            cols = min(value.shape[1], expected.shape[1])
            migrated[:rows, :cols] = value[:rows, :cols].to(
                dtype=migrated.dtype, device=migrated.device
            )
            compatible[key] = migrated
            resized.append((key, tuple(value.shape), tuple(expected.shape), rows, cols))
        else:
            skipped.append((key, tuple(value.shape), tuple(expected.shape)))

    missing, unexpected = model.load_state_dict(compatible, strict=False)
    for key, old_shape, new_shape, rows, cols in resized:
        log.warning(
            "CKPT-EVAL-COMPAT: migrated %s %s -> %s; copied overlap [%d,%d] from %s",
            key, old_shape, new_shape, rows, cols, checkpoint_label,
        )
    for key, old_shape, new_shape in skipped:
        log.warning(
            "CKPT-EVAL-COMPAT: skipped incompatible tensor %s %s -> %s from %s",
            key, old_shape, new_shape, checkpoint_label,
        )
...
class ALIGNNTrainer:
    ...
    def __init__(
        ...
        self.patience_ctr  = 0
        self._warned_no_grad_ablation = False

        # v4.40 AUDIT: preserve the initial residual-head state so each epoch can
        # report real parameter movement, not just gradient magnitude.
        self._process_delta_initial_state = {
            name: param.detach().cpu().clone()
            for name, param in self.model_core.process_delta_head.named_parameters()
        }
        self._categorical_embedding_initial_state = {
            name: param.detach().cpu().clone()
            for name, param in self.model_core._categorical_embedding_named_parameters()
        }
        self._write_optimizer_audit_manifest()
        log.info("Trainer patience: %d epochs", self.patience)

    def _process_delta_parameter_diagnostics(self) -> dict:
        """Return residual-head norm and L2 movement from trainer initialization."""
        sq_norm = 0.0
        sq_change = 0.0
        for name, param in self.model_core.process_delta_head.named_parameters():
            cur = param.detach().float().cpu()
            sq_norm += float(torch.sum(cur * cur).item())
            init = self._process_delta_initial_state.get(name)
            if init is not None:
                diff = cur - init.float()
                sq_change += float(torch.sum(diff * diff).item())
        return {
            "process_delta_parameter_norm": math.sqrt(max(sq_norm, 0.0)),
            "process_delta_parameter_change_from_init": math.sqrt(max(sq_change, 0.0)),
        }

    def _categorical_embedding_parameter_diagnostics(self) -> dict:
        """Return categorical embedding norm and movement from initialization."""
        sq_norm = 0.0
        sq_change = 0.0
        max_abs_change = 0.0
        changed_tensors = 0
        count = 0
        initial = getattr(self, "_categorical_embedding_initial_state", {})
        for name, param in self.model_core._categorical_embedding_named_parameters():
            count += int(param.numel())
            cur = param.detach().float().cpu()
            sq_norm += float(torch.sum(cur * cur).item())
            init = initial.get(name)
            if init is not None:
                diff = cur - init.float()
                sq_change += float(torch.sum(diff * diff).item())
                ...
                if local_max > 0.0:
                    changed_tensors += 1
        return {
            "categorical_embedding_parameter_count": count,
            "categorical_embedding_parameter_norm": math.sqrt(max(sq_norm, 0.0)),
            "categorical_embedding_parameter_change_from_init": math.sqrt(max(sq_change, 0.0)),
            "categorical_embedding_max_abs_change_from_init": max_abs_change,
            "categorical_embedding_changed_tensors": changed_tensors,
        }

    def _adapter_parameter_diagnostics(self) -> dict:
        """Return structural-adapter norm and movement from Tier-3 initialization."""
        sq_norm = 0.0
        sq_change = 0.0
        max_abs_change = 0.0
        grad_sq = 0.0
        initial = getattr(self, "_adapter_initial_state", {})
        for name, param in self.model_core.tier3_structural_adapter.named_parameters():
            cur = param.detach().float().cpu()
            sq_norm += float(torch.sum(cur * cur).item())
            init = initial.get(name)
            if init is not None:
                ...
                sq_change += float(torch.sum(diff * diff).item())
                if diff.numel():
                    max_abs_change = max(max_abs_change, float(diff.abs().max().item()))
            if param.grad is not None:
                grad_sq += float(param.grad.detach().float().pow(2).sum().item())
        return {
            "adapter_parameter_norm": math.sqrt(max(sq_norm, 0.0)),
            "adapter_parameter_change_l2": math.sqrt(max(sq_change, 0.0)),
            "adapter_parameter_change_max_abs": max_abs_change,
            "adapter_grad_norm_snapshot": math.sqrt(max(grad_sq, 0.0)),
        }

    def rebuild_optimizer_for_current_trainables(self):
        """v4.58 AdamW groups for process residual and low-rank structural adapter."""
        adapter_params = [p for p in self.model_core.tier3_structural_adapter.parameters() if p.requires_grad]
        adapter_ids = {id(p) for p in adapter_params}
        process_params = [p for p in self.model_core.parameters() if p.requires_grad and id(p) not in adapter_ids]
        groups = []
        if process_params:
            groups.append({
                "params": process_params,
                "lr": float(self.cfg.get("tier3_process_learning_rate", self.cfg["learning_rate"])),
                "weight_decay": float(self.cfg.get("tier3_process_weight_decay", self.cfg["weight_decay"])),
                "group_name": "process_residual",
            })
        if adapter_params:
            groups.append({
                "params": adapter_params,
                "lr": float(self.cfg.get("tier3_structural_adapter_learning_rate", 5e-5)),
                "weight_decay": float(self.cfg.get("tier3_structural_adapter_weight_decay", self.cfg["weight_decay"])),
                "group_name": "structural_adapter",
            })
        if not groups:
            raise RuntimeError("v4.56: no trainable parameters after residual+adapter freeze")
        self.optimizer = torch.optim.AdamW(groups)
        self._process_delta_initial_state = {name: p.detach().cpu().clone() for name,p in self.model_core.process_delta_head.named_parameters()}
        self._categorical_embedding_initial_state = {name: p.detach().cpu().clone() for name,p in self.model_core._categorical_embedding_named_parameters()}
        self._adapter_initial_state = {name: p.detach().cpu().clone() for name,p in self.model_core.tier3_structural_adapter.named_parameters()}
        self._base_head_initial_state = {}
        self._write_optimizer_audit_manifest()
        all_trainable = process_params + adapter_params
        log.info("v4.58 OPTIMIZER-REBUILD: tensors=%d params=%d groups=%s", len(all_trainable), sum(p.numel() for p in all_trainable), [(g["group_name"],g["lr"],sum(p.numel() for p in g["params"])) for g in groups])

    def capture_frozen_base_reference(self):
        """Snapshot every frozen parameter and buffer outside residual head."""
        self._frozen_base_reference = {
            name: tensor.detach().cpu().clone()
            for name, tensor in self.model_core.state_dict().items()
            if not name.startswith("process_delta_head.")
            and not name.startswith("tier3_structural_adapter.")
            and not (self.model_core.process_delta_use_categorical and
                     name.startswith("proc_encoder.embeddings."))
        }
        log.info(
            "v4.46 BASE-SNAPSHOT: captured %d frozen parameters/buffers",
            len(self._frozen_base_reference),
        )

    def _frozen_base_state_diagnostics(self) -> dict:
        ref = getattr(self, "_frozen_base_reference", None)
        if not ref:
            return {
                "frozen_base_max_abs_change": float("nan"),
                "frozen_base_l2_change": float("nan"),
                "frozen_base_changed_tensors": -1,
            }
        max_abs = 0.0
        sq = 0.0
        changed = 0
        current = self.model_core.state_dict()
        for name, old in ref.items():
            cur = current[name].detach().cpu()
            if cur.dtype.is_floating_point:
                diff = cur.float() - old.float()
                local_max = float(diff.abs().max().item()) if diff.numel() else 0.0
                local_sq = float((diff * diff).sum().item()) if diff.numel() else 0.0
            else:
                neq = cur.ne(old)
                local_max = 1.0 if bool(neq.any()) else 0.0
                local_sq = float(neq.sum().item())
            max_abs = max(max_abs, local_max)
            sq += local_sq
            if local_max > 0.0:
                changed += 1
        return {
            "frozen_base_max_abs_change": max_abs,
            "frozen_base_l2_change": math.sqrt(max(sq, 0.0)),
            "frozen_base_changed_tensors": changed,
        }

    def _write_optimizer_audit_manifest(self):
        """Write trainable-parameter and optimizer-membership evidence for Tier-3 audit."""
        if not is_rank0() or not self.cfg.get("audit_write_optimizer_manifest", False):
            return
        audit_dir = REPORT_ROOT / "tier3_prediction_audit" / ("ablation" if self.ablate_context else "normal")
        audit_dir.mkdir(parents=True, exist_ok=True)
        opt_ids = {id(p) for g in self.optimizer.param_groups for p in g["params"]}
        rows = []
        for name, p in self.model_core.named_parameters():
            if (name.startswith("process_delta_head") or
                    name.startswith("proc_encoder.embeddings") or p.requires_grad):
                lr = float("nan")
                for g in self.optimizer.param_groups:
                    if any(id(p) == id(q) for q in g["params"]):
                        lr = float(g.get("lr", float("nan")))
                        break
                group = ("categorical_embedding" if name.startswith("proc_encoder.embeddings")
                         else "process_delta_head" if name.startswith("process_delta_head")
                         else "other_trainable")
                rows.append({
        ...
        out = audit_dir / f"{self.ckpt_prefix}_optimizer_parameter_manifest.csv"
        pd.DataFrame(rows).to_csv(out, index=False)
        delta_rows = [r for r in rows if r["parameter_group"] == "process_delta_head"]
        cat_rows = [r for r in rows if r["parameter_group"] == "categorical_embedding"]
        log.info(
            "v4.46 AUDIT optimizer manifest: residual_params=%d residual_all_in_optimizer=%s "
            "categorical_enabled=%s categorical_params=%d categorical_all_in_optimizer=%s path=%s",
            sum(r["numel"] for r in delta_rows),
            bool(delta_rows) and all(r["requires_grad"] and r["in_optimizer"] for r in delta_rows),
            bool(self.model_core.process_delta_use_categorical),
            sum(r["numel"] for r in cat_rows),
            (not self.model_core.process_delta_use_categorical) or
                (bool(cat_rows) and all(r["requires_grad"] and r["in_optimizer"] for r in cat_rows)),
            out,
        )
    ...
    def build_scheduler(self, n_steps_per_epoch: int, n_epochs: int):
        ...
        if sched_type == "onecycle":
            ...
            if len(self.optimizer.param_groups) > 1:
                # Preserve the relative LR of process and base-head groups.
                return torch.optim.lr_scheduler.LambdaLR(
                    self.optimizer,
                    lr_lambda=[lambda ep, T=max(n_epochs, 1): 0.01 + 0.99 * 0.5 * (1.0 + math.cos(math.pi * ep / T))
    ...
    def train_epoch(
        ...
        """
        _residual_only_mode = bool(self.cfg.get("bounded_process_residual", False))
        _frozen_ablation = bool(self.ablate_context and _residual_only_mode)
        if _residual_only_mode:
            # v4.46: keep the whole frozen base in eval mode for BOTH normal
            # and ablation residual experiments. Only process_delta_head is put
            # in train mode during the normal run.
            self.model_core.set_residual_only_train_mode(
                ablate_context=self.ablate_context
            )
        else:
            self.model.train()
        total_loss        = 0.0
        ...
        delta_mean_sum    = 0.0
        delta_max_seen    = 0.0
        delta_active_sum  = 0.0
        delta_diag_batches = 0
        proc_delta_grad_sum = 0.0
        categorical_embedding_grad_sum = 0.0
        primary_loss_sum = 0.0
        base_drift_loss_sum = 0.0
        base_drift_weighted_sum = 0.0
        base_head_grad_sum = 0.0
        base_change_mean_sum = 0.0
        base_change_max_seen = 0.0
        adapter_mean_sum = 0.0
        adapter_std_sum = 0.0
        adapter_mean_abs_sum = 0.0
        adapter_max_abs_seen = 0.0
        adapter_grad_sum = 0.0
        adapter_mag_loss_sum = 0.0
        adapter_center_loss_sum = 0.0
        process_center_loss_sum = 0.0
        orthogonal_loss_sum = 0.0
        branch_corr_sum = 0.0
        branch_corr_batches = 0
        ...
        for batch in loader:
            ...
            primary_loss = self.criterion(preds, targets_dict, functional_weights=func_loss_weights)
            loss = primary_loss
            # v4.56 correction-branch regularization.
            structural_delta = getattr(self.model_core, "_last_structural_delta", None)
            process_delta_tensor = getattr(self.model_core, "_last_process_delta_tensor", None)
            adapter_mag_loss = torch.zeros((), device=self.device)
            adapter_center_loss = torch.zeros((), device=self.device)
            process_center_loss = torch.zeros((), device=self.device)
            orthogonal_loss = torch.zeros((), device=self.device)
            if structural_delta is not None:
                sd = structural_delta.squeeze(-1)
                adapter_mag_loss = torch.mean(sd * sd)
                adapter_center_loss = torch.mean(sd) ** 2
            if process_delta_tensor is not None:
                pdlt = process_delta_tensor.squeeze(-1)
                if proc_ctx is not None and "avail" in proc_ctx:
                    active = proc_ctx["avail"].squeeze(-1) > 0
                    if bool(active.any()):
                        pdlt = pdlt[active]
                        process_center_loss = torch.mean(pdlt) ** 2
                        if structural_delta is not None:
                            sd_active = structural_delta.squeeze(-1)[active]
                            if sd_active.numel() > 1:
                                sdc = sd_active - sd_active.mean()
                                pdc = pdlt - pdlt.mean()
                                orthogonal_loss = torch.mean(sdc * pdc) ** 2
            loss = (primary_loss
                    + float(self.cfg.get("tier3_adapter_magnitude_weight", 1.0)) * adapter_mag_loss
                    + float(self.cfg.get("tier3_adapter_center_weight", 5.0)) * adapter_center_loss
                    + float(self.cfg.get("tier3_process_center_weight", 5.0)) * process_center_loss
                    + float(self.cfg.get("tier3_adapter_process_orthogonal_weight", 1.0)) * orthogonal_loss)
            base_drift_loss = adapter_mag_loss
            base_drift_weight = float(self.cfg.get("tier3_adapter_magnitude_weight", 1.0))
            primary_loss_sum += float(primary_loss.detach().item())
            base_drift_loss_sum += float(base_drift_loss.detach().item())
            base_drift_weighted_sum += float(base_drift_weight * base_drift_loss.detach().item())
            adapter_mag_loss_sum += float(adapter_mag_loss.detach().item())
            adapter_center_loss_sum += float(adapter_center_loss.detach().item())
            process_center_loss_sum += float(process_center_loss.detach().item())
            orthogonal_loss_sum += float(orthogonal_loss.detach().item())
            if structural_delta is not None:
                _sd = structural_delta.detach().reshape(-1)
                adapter_mean_sum += float(_sd.mean().item()) if _sd.numel() else 0.0
                adapter_std_sum += float(_sd.std(unbiased=False).item()) if _sd.numel() else 0.0
                adapter_mean_abs_sum += float(_sd.abs().mean().item()) if _sd.numel() else 0.0
                adapter_max_abs_seen = max(adapter_max_abs_seen, float(_sd.abs().max().item()) if _sd.numel() else 0.0)
            if structural_delta is not None and process_delta_tensor is not None:
                _s = structural_delta.detach().reshape(-1)
                _p = process_delta_tensor.detach().reshape(-1)
                if _s.numel() == _p.numel() and _s.numel() > 1:
                    _sc = _s - _s.mean(); _pc = _p - _p.mean()
                    _den = torch.sqrt(torch.sum(_sc * _sc) * torch.sum(_pc * _pc))
                    if float(_den.item()) > 0.0:
                        branch_corr_sum += float(torch.sum(_sc * _pc).item() / _den.item())
                        branch_corr_batches += 1
            base_change_mean_sum += float(getattr(self.model_core, "_last_base_change_mean_abs", 0.0))
            base_change_max_seen = max(base_change_max_seen, float(getattr(self.model_core, "_last_base_change_max_abs", 0.0)))

            # v4.40 AUDIT+ABLATION-NOGRAD FIX:
            # In the frozen-backbone residual experiment the only trainable
            # parameters are in process_delta_head. With --ablate_context,
            # proc_context is deliberately set to None, so predictions are
            # independent of every trainable parameter. The resulting loss is
            # therefore a valid constant baseline but has requires_grad=False.
            # Calling backward() on it raises:
            #   RuntimeError: element 0 of tensors does not require grad ...
            # Treat such batches as evaluation-only ablation batches: record
            # their loss/metrics, but do not call backward(), optimizer.step(),
            # gradient clipping, or the per-step OneCycle scheduler.
            has_trainable_graph = bool(loss.requires_grad and loss.grad_fn is not None)
            if has_trainable_graph:
                loss.backward()
            elif self.ablate_context and not self._warned_no_grad_ablation:
                log.info(
                    "v4.40 AUDIT+ABLATION-NOGRAD: context is disabled and the DFT base "
                    "is frozen; running loss/metric-only epochs with no optimizer "
                    "updates. This is the intended ablation baseline."
                )
                self._warned_no_grad_ablation = True

            # v4.38/v4.39: process-residual learning diagnostics.
            grad_sq = 0.0
            if has_trainable_graph:
                for p in self.model_core.process_delta_head.parameters():
                    if p.grad is not None:
                        grad_sq += float(p.grad.detach().pow(2).sum().item())
            proc_delta_grad_sum += math.sqrt(grad_sq)
            adapter_grad_sq = 0.0
            if has_trainable_graph:
                for p in self.model_core.tier3_structural_adapter.parameters():
                    if p.grad is not None:
                        adapter_grad_sq += float(p.grad.detach().pow(2).sum().item())
            adapter_grad_sum += math.sqrt(adapter_grad_sq)
            cat_grad_sq = 0.0
            if has_trainable_graph:
                for _, p in self.model_core._categorical_embedding_named_parameters():
                    if p.grad is not None:
                        cat_grad_sq += float(p.grad.detach().pow(2).sum().item())
            categorical_embedding_grad_sum += math.sqrt(cat_grad_sq)
            base_grad_sq = 0.0
            if has_trainable_graph and self.cfg.get("process_delta_train_base_head", False):
                for p in self.model_core.task_heads["k_total_log"].parameters():
                    if p.grad is not None:
                        base_grad_sq += float(p.grad.detach().pow(2).sum().item())
            base_head_grad_sum += math.sqrt(base_grad_sq)
            delta_mean_sum += float(getattr(self.model_core, "_last_process_delta_mean_abs", 0.0))
            delta_max_seen = max(delta_max_seen, float(getattr(self.model_core, "_last_process_delta_max_abs", 0.0)))
            delta_active_sum += float(getattr(self.model_core, "_last_process_delta_active_pct", 0.0))
            delta_diag_batches += 1

            # Gradient clipping / optimizer update only when the current
            # forward pass reaches at least one trainable parameter.
            if has_trainable_graph:
        ...
        _param_diag = self._process_delta_parameter_diagnostics()
        _cat_diag = self._categorical_embedding_parameter_diagnostics()
        _adapter_diag = self._adapter_parameter_diagnostics()
        _base_diag = self._frozen_base_state_diagnostics()
        base_param_sq = 0.0
        base_param_max = 0.0
        for name, p in self.model_core.task_heads["k_total_log"].named_parameters():
            old = getattr(self, "_base_head_initial_state", {}).get(name)
            if old is not None:
                d = p.detach().cpu().float() - old.float()
                base_param_sq += float((d * d).sum().item())
                base_param_max = max(base_param_max, float(d.abs().max().item()) if d.numel() else 0.0)
        if _residual_only_mode and not self.cfg.get("process_delta_train_base_head", False):
            tol = float(self.cfg.get("frozen_base_state_tolerance", 1e-8))
            if (_base_diag["frozen_base_changed_tensors"] > 0 and
                    _base_diag["frozen_base_max_abs_change"] > tol):
                raise RuntimeError(
                    "v4.46 FROZEN-BASE DRIFT: "
                    f"changed_tensors={_base_diag['frozen_base_changed_tensors']} "
                    f"max_abs={_base_diag['frozen_base_max_abs_change']:.3e} "
                    f"l2={_base_diag['frozen_base_l2_change']:.3e} tol={tol:.3e}"
                )
        return {
            "loss":            total_loss / max(n_batches, 1),
            "proc_avail_pct":  100.0 * proc_avail_total  / max(n_samples, 1),
            "stack_avail_pct": 100.0 * stack_avail_total / max(n_samples, 1),
            "process_delta_mean_abs": delta_mean_sum / max(delta_diag_batches, 1),
            "process_delta_max_abs": delta_max_seen,
            "process_delta_active_pct": delta_active_sum / max(delta_diag_batches, 1),
            "process_delta_grad_norm": proc_delta_grad_sum / max(n_batches, 1),
            "categorical_embedding_grad_norm": categorical_embedding_grad_sum / max(n_batches, 1),
            "primary_loss": primary_loss_sum / max(n_batches, 1),
            "adapter_output_mean": adapter_mean_sum / max(delta_diag_batches, 1),
            "adapter_output_std": adapter_std_sum / max(delta_diag_batches, 1),
            "adapter_output_mean_abs": adapter_mean_abs_sum / max(delta_diag_batches, 1),
            "adapter_output_max_abs": adapter_max_abs_seen,
            "adapter_grad_norm": adapter_grad_sum / max(n_batches, 1),
            "adapter_magnitude_loss": adapter_mag_loss_sum / max(n_batches, 1),
            "adapter_center_loss": adapter_center_loss_sum / max(n_batches, 1),
            "process_center_loss": process_center_loss_sum / max(n_batches, 1),
            "adapter_process_orthogonal_loss": orthogonal_loss_sum / max(n_batches, 1),
            "adapter_process_correlation": branch_corr_sum / max(branch_corr_batches, 1),
            "base_drift_loss": base_drift_loss_sum / max(n_batches, 1),
            "base_drift_weighted": base_drift_weighted_sum / max(n_batches, 1),
            "base_head_grad_norm": base_head_grad_sum / max(n_batches, 1),
            "base_change_mean_abs_log": base_change_mean_sum / max(n_batches, 1),
            "base_change_max_abs_log": base_change_max_seen,
            "base_head_parameter_change_l2": math.sqrt(max(base_param_sq, 0.0)),
            "base_head_parameter_change_max_abs": base_param_max,
            **_param_diag,
            **_cat_diag,
            **_adapter_diag,
            **_base_diag,
    ...
    def evaluate_multitask(
        ...
        for head_name in self.model_core.task_heads:
            ...
            mae_native  = (pred_valid - tgt_valid).abs().mean().item()
            rmse_native = ((pred_valid - tgt_valid) ** 2).mean().sqrt().item()

            _is_log_head = str(head_name).endswith("_log")
            if _is_log_head:
                # v4.51: exact dual-scale metrics from the SAME valid rows.
                # Never combine test MAE with full-dataset MAD for a log head.
                pred_linear = torch.exp(pred_valid)
                tgt_linear  = torch.exp(tgt_valid)
                mae_linear  = (pred_linear - tgt_linear).abs().mean().item()
                rmse_linear = ((pred_linear - tgt_linear) ** 2).mean().sqrt().item()
                mad_linear  = float(
                    (tgt_linear - tgt_linear.mean()).abs().mean().item()
                )
                mad_mae_linear = mad_linear / max(mae_linear, 1e-9)
                mad_source = "same TEST rows, exp-space"
                # Backward-compatible aliases use publication/physical scale.
                mae_alias, rmse_alias, mad_alias, ratio_alias = (
                    mae_linear, rmse_linear, mad_linear, mad_mae_linear
                )
            else:
                # Native non-log targets are already in their physical scale.
                if head_name in full_mads:
                    mad_linear  = full_mads[head_name]
                    mad_source = "full dataset"
                else:
                    mad_linear  = float(
                        (tgt_valid - tgt_valid.mean()).abs().mean().item()
                    )
                    mad_source = "test split"
                mae_linear = mae_native
                rmse_linear = rmse_native
                mad_mae_linear = mad_linear / max(mae_linear, 1e-9)
                mae_alias, rmse_alias, mad_alias, ratio_alias = (
                    mae_linear, rmse_linear, mad_linear, mad_mae_linear
                )
            ...
            results[head_name] = {
                "mae":             mae_alias,
                "rmse":            rmse_alias,
                "mae_log":         mae_native if _is_log_head else float("nan"),
                "rmse_log":        rmse_native if _is_log_head else float("nan"),
                "mae_linear":      mae_linear,
                "rmse_linear":     rmse_linear,
                "mad":             mad_alias,
                "mad_linear":      mad_linear,
                "mad_mae_ratio":   ratio_alias,
                "mad_mae_linear":  mad_mae_linear,
    ...
    @torch.no_grad()
    def evaluate_adapter_diagnostics(self, loader: DataLoader, target_col: str, split_name: str = "TEST", gather_distributed: bool = False) -> dict:
        """Measure adapter and process contributions on the exact evaluation rows.

        v4.60.3: ``gather_distributed`` must be True ONLY when ``loader`` is
        sharded across ranks (a DistributedSampler). The final TEST loader
        (FINAL-SPLIT-RELOAD) and the tier3_evaluate loader are plain, full-on-
        every-rank DataLoaders, so each rank already holds every row. Calling
        ``all_gather_object`` on them would (a) DEADLOCK if a rank diverged or
        exited (this collective is the last op after TEST-UNSEAL, so any rank
        imbalance hangs the run) and (b) double-count deltas across ranks. With
        the default False, every rank computes the complete diagnostics locally
        ...
        for batch in loader:
            ...
            if func_code is not None:
                func_code = func_code.to(self.device)
            _ = self.model(graph, line_graph, task=target_col, proc_context=proc_ctx,
                           stack_context=stack_ctx, functional_code=func_code)
            sd = getattr(self.model_core, "_last_structural_delta", None)
            pd = getattr(self.model_core, "_last_process_delta_tensor", None)
            if sd is not None:
                s_all.append(sd.detach().reshape(-1).cpu())
            if pd is not None:
                p_all.append(pd.detach().reshape(-1).cpu())
        svec = torch.cat(s_all) if s_all else torch.empty(0)
        pvec = torch.cat(p_all) if p_all else torch.empty(0)
        # Gather rank-local vectors so rank-0 TEST diagnostics cover all DDP rows.
        # v4.60.3: only when the loader is genuinely sharded (gather_distributed=True).
        if gather_distributed and dist.is_available() and dist.is_initialized():
            gathered_s = [None for _ in range(dist.get_world_size())]
            gathered_p = [None for _ in range(dist.get_world_size())]
            dist.all_gather_object(gathered_s, svec)
            dist.all_gather_object(gathered_p, pvec)
            svec = torch.cat([x for x in gathered_s if x is not None and x.numel()]) if any(x is not None and x.numel() for x in gathered_s) else torch.empty(0)
            pvec = torch.cat([x for x in gathered_p if x is not None and x.numel()]) if any(x is not None and x.numel() for x in gathered_p) else torch.empty(0)
        def stats(x):
            if not x.numel():
                return dict(mean=0.0, std=0.0, mean_abs=0.0, max_abs=0.0, n=0)
            return dict(mean=float(x.mean()), std=float(x.std(unbiased=False)),
                        mean_abs=float(x.abs().mean()), max_abs=float(x.abs().max()), n=int(x.numel()))
        ss, ps = stats(svec), stats(pvec)
        corr = 0.0
        n = min(svec.numel(), pvec.numel())
        if n > 1:
            sc=svec[:n]-svec[:n].mean(); pc=pvec[:n]-pvec[:n].mean()
            den=torch.sqrt(torch.sum(sc*sc)*torch.sum(pc*pc))
            if float(den)>0: corr=float(torch.sum(sc*pc)/den)
        result={"adapter":ss,"process":ps,"correlation":corr,
                "enabled":bool(self.model_core.use_tier3_structural_adapter),
                "rank":int(self.model_core.tier3_structural_adapter_rank)}
        if is_rank0():
            log.info(
                "V4.58 ADAPTER-%s enabled=%s rank=%d n=%d mean=%+.6f std=%.6f "
                "mean_abs=%.6f max_abs=%.6f process_mean=%+.6f process_std=%.6f "
                "process_mean_abs=%.6f process_max_abs=%.6f corr=%+.4f",
                split_name.upper(), result["enabled"], result["rank"], ss["n"],
                ss["mean"], ss["std"], ss["mean_abs"], ss["max_abs"],
                ps["mean"], ps["std"], ps["mean_abs"], ps["max_abs"], corr)
        return result
    ...
    def print_multitask_results(
        ...
        """Print all task heads with explicit log-space and linear-space metrics.

        For log-transformed dielectric heads (for example ``k_total_log``),
        ``mae_log``/``rmse_log`` describe the optimized training space, while
        ``mae_linear``/``rmse_linear`` and ``mad_mae_linear`` are computed after
        exponentiating the exact same prediction/target tensors.  For ordinary
        non-log targets, the linear columns contain the native task metrics and
        the log columns are intentionally blank.
        """
        hdr  = f" {tier_name} -- {split_name} SET -- ALL TASK HEADS"
        line = "=" * max(132, len(hdr) + 2)
        log.info("\n%s", line)
        log.info(hdr)
        log.info("%s", line)
        log.info(
            "  %-24s | %10s | %10s | %11s | %12s | %15s | %9s | %9s | %8s",
            "Task", "MAE(log)", "RMSE(log)", "MAE(linear)", "RMSE(linear)",
            "MAD:MAE(linear)", "Paper ref", "N valid", "Coverage",
        )
        log.info("  %s", "-" * 128)
        ...
        for task_name, m in results.items():
            ...
            cov    = float(m.get("coverage_pct", 0.0) or 0.0)
            is_pri = "<- primary" if task_name == primary_col else ""
            note   = "(no data this tier)" if n == 0 else is_pri

            def _fmt(key: str, width: int = 10) -> str:
                val = m.get(key, float("nan"))
                try:
                    ok = np.isfinite(float(val))
                except (TypeError, ValueError):
                    ok = False
                return f"{float(val):{width}.4f}" if ok else "--".rjust(width)

            mae_log_s  = _fmt("mae_log", 10)
            rmse_log_s = _fmt("rmse_log", 10)
            mae_lin_s  = _fmt("mae_linear", 11)
            rmse_lin_s = _fmt("rmse_linear", 12)

            ratio = m.get("mad_mae_linear", m.get("mad_mae_ratio", float("nan")))
            try:
                ratio_ok = np.isfinite(float(ratio))
            except (TypeError, ValueError):
                ratio_ok = False
            ratio_s = f"{float(ratio):15.2f}" if ratio_ok else "--".rjust(15)

            paper_ref = PAPER_MAD_MAE_BENCHMARK.get(task_name)
            if paper_ref is not None and ratio_ok:
                gap_pct = 100.0 * (paper_ref - float(ratio)) / paper_ref
            ...
            display_name = str(m.get("display_name", task_name))
            log.info(
                "  %-24s | %s | %s | %s | %s | %s | %s | %9s | %7.1f%%  %s",
                display_name, mae_log_s, rmse_log_s, mae_lin_s, rmse_lin_s,
                ratio_s, paper_s, f"{n:,}", cov, note,
            )

        log.info("%s", line)
        log.info(
            "  k log-heads: log columns = optimization space; linear columns and "
            "MAD:MAE(linear) = exact exp-space metrics on the same rows.  "
    ...
    def train(
        ...
        for epoch in range(1, n_epochs + 1):
            ...
            # v4.58 release audit: explicit adapter/process health every epoch.
            adapter_active = (
                self.model_core.use_tier3_structural_adapter
                and epoch_stats.get("adapter_parameter_change_l2", 0.0) > 0.0
                and epoch_stats.get("adapter_output_mean_abs", 0.0) > 0.0
                and epoch_stats.get("adapter_grad_norm", 0.0) > 0.0
            )
            log.info(
            ...
            log.info(
                "V4.58 ADAPTER-EPOCH ep=%d enabled=%s active=%s rank=%d "
                "out_mean=%+.6f out_std=%.6f out_mean_abs=%.6f out_max_abs=%.6f "
                "grad=%.3e param_norm=%.3e param_change_l2=%.3e param_change_max=%.3e",
                epoch, bool(self.model_core.use_tier3_structural_adapter), adapter_active,
                int(self.model_core.tier3_structural_adapter_rank),
                epoch_stats.get("adapter_output_mean", 0.0),
                epoch_stats.get("adapter_output_std", 0.0),
                epoch_stats.get("adapter_output_mean_abs", 0.0),
                epoch_stats.get("adapter_output_max_abs", 0.0),
                epoch_stats.get("adapter_grad_norm", 0.0),
                epoch_stats.get("adapter_parameter_norm", 0.0),
                epoch_stats.get("adapter_parameter_change_l2", 0.0),
                epoch_stats.get("adapter_parameter_change_max_abs", 0.0),
            )
            log.info(
                "V4.58 BRANCH-EPOCH ep=%d process_mean_abs=%.6f process_max_abs=%.6f "
                "process_active=%.1f%% process_grad=%.3e adapter_process_corr=%+.4f "
                "primary=%.6f adapter_mag=%.3e adapter_center=%.3e "
                "process_center=%.3e orthogonal=%.3e frozen_max=%.3e frozen_changed=%d",
                epoch,
                epoch_stats.get("process_delta_mean_abs", 0.0),
                epoch_stats.get("process_delta_max_abs", 0.0),
                epoch_stats.get("process_delta_active_pct", 0.0),
                epoch_stats.get("process_delta_grad_norm", 0.0),
                epoch_stats.get("adapter_process_correlation", 0.0),
                epoch_stats.get("primary_loss", train_loss),
                epoch_stats.get("adapter_magnitude_loss", 0.0),
                epoch_stats.get("adapter_center_loss", 0.0),
                epoch_stats.get("process_center_loss", 0.0),
                epoch_stats.get("adapter_process_orthogonal_loss", 0.0),
                epoch_stats.get("frozen_base_max_abs_change", float("nan")),
                int(epoch_stats.get("frozen_base_changed_tensors", -1)),
            )
            ...
            history.append({
                ...
                "process_delta_mean_abs": epoch_stats.get("process_delta_mean_abs", 0.0),
                "process_delta_max_abs": epoch_stats.get("process_delta_max_abs", 0.0),
                "process_delta_active_pct": epoch_stats.get("process_delta_active_pct", 0.0),
                "process_delta_grad_norm": epoch_stats.get("process_delta_grad_norm", 0.0),
                "process_delta_parameter_norm": epoch_stats.get("process_delta_parameter_norm", 0.0),
                "process_delta_parameter_change_from_init": epoch_stats.get("process_delta_parameter_change_from_init", 0.0),
                "adapter_output_mean": epoch_stats.get("adapter_output_mean", 0.0),
                "adapter_output_std": epoch_stats.get("adapter_output_std", 0.0),
                "adapter_output_mean_abs": epoch_stats.get("adapter_output_mean_abs", 0.0),
                "adapter_output_max_abs": epoch_stats.get("adapter_output_max_abs", 0.0),
                "adapter_grad_norm": epoch_stats.get("adapter_grad_norm", 0.0),
                "adapter_parameter_norm": epoch_stats.get("adapter_parameter_norm", 0.0),
                "adapter_parameter_change_l2": epoch_stats.get("adapter_parameter_change_l2", 0.0),
                "adapter_parameter_change_max_abs": epoch_stats.get("adapter_parameter_change_max_abs", 0.0),
                "adapter_process_correlation": epoch_stats.get("adapter_process_correlation", 0.0),
                "adapter_magnitude_loss": epoch_stats.get("adapter_magnitude_loss", 0.0),
                "adapter_center_loss": epoch_stats.get("adapter_center_loss", 0.0),
                "process_center_loss": epoch_stats.get("process_center_loss", 0.0),
                "adapter_process_orthogonal_loss": epoch_stats.get("adapter_process_orthogonal_loss", 0.0),
                "categorical_embedding_grad_norm": epoch_stats.get("categorical_embedding_grad_norm", 0.0),
                "categorical_embedding_parameter_count": epoch_stats.get("categorical_embedding_parameter_count", 0),
                "categorical_embedding_parameter_norm": epoch_stats.get("categorical_embedding_parameter_norm", 0.0),
                "categorical_embedding_parameter_change_from_init": epoch_stats.get("categorical_embedding_parameter_change_from_init", 0.0),
                "categorical_embedding_max_abs_change_from_init": epoch_stats.get("categorical_embedding_max_abs_change_from_init", 0.0),
                "categorical_embedding_changed_tensors": epoch_stats.get("categorical_embedding_changed_tensors", 0),
                "frozen_base_max_abs_change": epoch_stats.get("frozen_base_max_abs_change", float("nan")),
                "frozen_base_l2_change": epoch_stats.get("frozen_base_l2_change", float("nan")),
                "frozen_base_changed_tensors": epoch_stats.get("frozen_base_changed_tensors", -1),
                "primary_loss": epoch_stats.get("primary_loss", train_loss),
                "base_drift_loss": epoch_stats.get("base_drift_loss", 0.0),
                "base_drift_weighted": epoch_stats.get("base_drift_weighted", 0.0),
                "base_head_grad_norm": epoch_stats.get("base_head_grad_norm", 0.0),
                "base_change_mean_abs_log": epoch_stats.get("base_change_mean_abs_log", 0.0),
                "base_change_max_abs_log": epoch_stats.get("base_change_max_abs_log", 0.0),
                "base_head_parameter_change_l2": epoch_stats.get("base_head_parameter_change_l2", 0.0),
                "base_head_parameter_change_max_abs": epoch_stats.get("base_head_parameter_change_max_abs", 0.0),
            })

            if self.cfg.get("process_delta_train_base_head", False):
                mean_lim = float(self.cfg.get("tier3_base_change_warn_mean", 0.03))
                max_lim = float(self.cfg.get("tier3_base_change_warn_max", 0.10))
                if (epoch_stats.get("base_change_mean_abs_log", 0.0) > mean_lim or
                        epoch_stats.get("base_change_max_abs_log", 0.0) > max_lim):
                    log.warning(
                        "v4.56 BASE-CHANGE LIMIT: mean=%.4f (limit %.4f), max=%.4f (limit %.4f). "
                        "Increase --tier3_base_drift_weight or reduce --tier3_base_head_lr.",
                        epoch_stats.get("base_change_mean_abs_log", 0.0), mean_lim,
                        epoch_stats.get("base_change_max_abs_log", 0.0), max_lim,
                    )
            ...
            # v4.40 AUDIT: save explicit trajectory checkpoints (default: 1,10).
            if epoch in set(self.cfg.get("audit_checkpoint_epochs", [])) and is_rank0():
                self.save_checkpoint(epoch, val_mae, tag=f"audit_ep{epoch}")
        ...
        hist_path = REPORT_ROOT / f"{self.ckpt_prefix}_training_history_{'ablation' if self.ablate_context else 'normal'}.json"
...
def is_hfo2_eval_family_row(row: pd.Series) -> bool:
    """Return True for HfO2/HZO/Hf-Zr-O/doped-HfO2 target-family rows.

    This is intentionally inclusive for common process-DB material notations:
    HfO2, HZO, Hf0.5Zr0.5O2, HfAlO, HfLaO, HfSiO, HfSiON, nanolaminates
    containing HfO2, and other Hf-containing oxides. It excludes broad
    non-Hf families such as SrTiO3, BaTiO3, La2O3, Nb2O5, Ta2O5, Al2O3-only.
    """
    text = " ".join(
        str(row.get(c, "")) for c in [
            "material", "material_system", "formula", "composition",
            "film_material", "material_name"
        ]
    ).lower().replace(" ", "")
    if not text or text in {"nan", "none"}:
        return False

    # Direct HfO2 / HZO / Hf-Zr-O notations.
    if any(tok in text for tok in ["hfo2", "hzo", "hfzro", "hf-zr-o", "hf0.", "hf/o"]):
        return True

    # Hf-containing oxide or oxynitride/doped-HfO families.
    has_hf = "hf" in text
    has_oxide_marker = any(tok in text for tok in ["o", "oxide", "oxynitride", "sio", "sion"])
    if has_hf and has_oxide_marker:
        return True
...
def _norm_split_val(v) -> str:
    """Normalise values used in persistent split keys."""
    if v is None:
        return ""
    s = str(v).strip()
...
def _tier3_split_keys_from_row(row: pd.Series) -> List[str]:
    """Return multiple stable keys for one Tier-3 row.

    v4.35 fix: dataframe integer positions and even row_hash can change after
    --rebuild_tier3 or after quality/family columns are added.  Therefore the
    persistent split file stores and loads a SET of matching keys.  Any one of
    these keys is sufficient to recover the saved TEST row.
    """
    keys: List[str] = []

    # Strong single-column identifiers when present.
    for col in ["row_id", "paper_id", "doi", "row_hash"]:
        v = _norm_split_val(row.get(col, None))
        if v:
            keys.append(f"{col}:{v}")

    # Material + source + k signature is useful when row_id/row_hash changed.
    material = _norm_split_val(row.get("material", row.get("material_system", ""))).lower().replace(" ", "")
    formula  = _norm_split_val(row.get("formula", "")).lower().replace(" ", "")
    source   = _norm_split_val(row.get("source", "")).lower()
    kval_raw = row.get("k_total", row.get("k_measured", ""))
    try:
        kval = f"{float(kval_raw):.6g}"
    except Exception:
        kval = _norm_split_val(kval_raw)
    if material and kval:
        keys.append(f"matk:{material}|{source}|{kval}")
    if formula and kval:
        keys.append(f"formk:{formula}|{source}|{kval}")

    # Conservative composite fallback for older caches.
    parts = []
    for col in ["paper_id", "doi", "material", "material_system", "formula", "source", "k_total"]:
        parts.append(_norm_split_val(row.get(col, "")))
    fallback = "fallback:" + "|".join(parts)
...
def _tier3_split_key_from_row(row: pd.Series) -> str:
    """Primary stable row key used for persistent Tier-3 split matching."""
    keys = _tier3_split_keys_from_row(row)
    return keys[0] if keys else "fallback:"


def save_tier3_test_split_rows(
    dataset: "HighKGraphDataset",
    valid_rows: pd.DataFrame,
    test_pos: np.ndarray,
    path: Path = TIER3_TEST_SPLIT_PATH,
    split_name: str = "test",
    overwrite: bool = True,
) -> None:
    """Save exact Tier-3 TEST row IDs/keys used by finetune.

    The saved file is later consumed by tier3_evaluate and --ablate_context
    runs to guarantee the same TEST rows are used.
    """
    if not is_rank0():
        return
    path.parent.mkdir(parents=True, exist_ok=True)
    if path.exists() and not overwrite:
        log.info("Tier3 split file already exists; not overwriting: %s", path)
        return
    test_pos = np.asarray(test_pos, dtype=int)
    rows = []
    for p in test_pos.tolist():
        if p < 0 or p >= len(valid_rows):
            continue
        r = valid_rows.iloc[p]
        rows.append({
            "split": split_name,
            "position": int(p),
            "split_key": _tier3_split_key_from_row(r),
            "split_key_candidates": "||".join(_tier3_split_keys_from_row(r)),
            "row_id": r.get("row_id", ""),
            "row_hash": r.get("row_hash", ""),
            "paper_id": r.get("paper_id", ""),
            "doi": r.get("doi", ""),
            "material": r.get("material", r.get("material_system", "")),
            "formula": r.get("formula", ""),
            "source": r.get("source", ""),
            "data_quality": r.get("data_quality", ""),
            "material_family": r.get("material_family", ""),
            "k_total": r.get("k_total", ""),
            "k_total_log": r.get("k_total_log", ""),
        })
    pd.DataFrame(rows).to_csv(path, index=False)
    log.info("v4.35 SPLIT-SAVE: saved Tier3 TEST split rows=%d → %s", len(rows), path)


def load_tier3_test_positions_from_split(
    valid_rows: pd.DataFrame,
    path: Path = TIER3_TEST_SPLIT_PATH,
) -> Optional[np.ndarray]:
    """Load saved Tier-3 TEST positions using strict one-to-one matching.

    v4.41 root-cause fix:
    v4.35 built one global set from *all* candidate keys and accepted any
    intersection. Non-unique keys such as DOI, paper_id, material+k, or formula+k
    could therefore make one saved row match several current rows. A 29-row
    split could expand to 66 rows during reload.

    This implementation processes each saved row independently and consumes at
    most one current row. It prefers stable unique identifiers (row_hash,
    row_id), then exact composites. Ambiguous keys are never allowed to fan out.
    """
    if not path.exists():
        log.info("v4.41 STRICT-SPLIT-LOAD: no saved Tier3 split file at %s", path)
        return None
    try:
        split_df = pd.read_csv(path)
        if split_df.empty:
            log.warning("v4.41 STRICT-SPLIT-LOAD: empty split file: %s", path)
            return None

        current_key_map: Dict[str, List[int]] = {}
        current_rows = valid_rows.reset_index(drop=True)
        for pos, (_, row) in enumerate(current_rows.iterrows()):
            for key in _tier3_split_keys_from_row(row):
                current_key_map.setdefault(key, []).append(pos)
        ...
        def _ordered_saved_keys(r: pd.Series) -> List[str]:
            keys: List[str] = []
            # Prefer exact values stored in the split CSV.
            for col in ["row_hash", "row_id"]:
                v = _norm_split_val(r.get(col, None))
                if v:
                    keys.append(f"{col}:{v}")
            # Then the primary key and saved candidates in their stored order.
            primary = _norm_split_val(r.get("split_key", None))
            if primary:
                keys.append(primary)
            cand = _norm_split_val(r.get("split_key_candidates", None))
            if cand:
                keys.extend([k.strip() for k in cand.split("||") if k.strip()])
            # Reconstruct composites for backward compatibility.
            keys.extend(_tier3_split_keys_from_row(r))
            seen = set()
            return [k for k in keys if k and not (k in seen or seen.add(k))]

        for _, saved_row in split_df.iterrows():
            selected = None
            had_ambiguous = False
            for key in _ordered_saved_keys(saved_row):
                candidates = [p for p in current_key_map.get(key, []) if p not in used]
                if len(candidates) == 1:
                    selected = candidates[0]
                    break
                if len(candidates) > 1:
                    had_ambiguous = True
                    # Disambiguate using exact metadata equality, but still select
                    # at most one row.
                    exact = []
                    for p in candidates:
                        cur = current_rows.iloc[p]
                        checks = []
                        for col in ["row_hash", "row_id", "paper_id", "doi", "material", "formula", "source"]:
                            sv = _norm_split_val(saved_row.get(col, None))
                            if sv:
                                checks.append(_norm_split_val(cur.get(col, None)) == sv)
                        try:
                            sk = float(saved_row.get("k_total", float("nan")))
                            ck = float(cur.get("k_total", float("nan")))
                            if np.isfinite(sk) and np.isfinite(ck):
                                checks.append(abs(sk - ck) <= 1e-8)
                        except Exception:
                            pass
        ...
        n_saved = len(split_df)
        log.info(
            "v4.41 STRICT-SPLIT-LOAD: matched exactly %d/%d saved rows; "
            "unmatched=%d ambiguous_unresolved=%d current_valid=%d",
            len(matched), n_saved, unmatched_saved, ambiguous_saved, len(current_rows),
        )
        # Exact test identity is required for controlled comparisons. Permit a
        # tiny tolerance only for genuinely stale legacy files.
        if len(matched) < max(5, int(0.95 * n_saved)):
            log.warning(
                "v4.41 STRICT-SPLIT-LOAD rejected split: only %d/%d rows matched. "
                "Delete %s and run normal tier3_finetune once.",
                len(matched), n_saved, path,
            )
            return None
        if len(set(matched)) != len(matched):
            raise RuntimeError("v4.41 strict split produced duplicate positions")
        return np.asarray(matched, dtype=int)
    except Exception as exc:
        log.warning("v4.41 STRICT-SPLIT-LOAD failed for %s: %s", path, exc)
        return None



def _tier3_split_fingerprint(path: Path) -> str:
    """Stable SHA256 fingerprint for a persistent split CSV."""
    if not path.exists():
        return "MISSING"
    return hashlib.sha256(path.read_bytes()).hexdigest()

def _split_positions_stratified(
    df_for_positions: pd.DataFrame,
    ...
    """Stratified split for a subset of dataset positions; random fallback."""
    from sklearn.model_selection import StratifiedShuffleSplit
    ...
    y = pd.to_numeric(df_for_positions.iloc[positions][target_col], errors="coerce").to_numpy(dtype=float)
    valid = ~np.isnan(y)
    if valid.sum() < 20:
    ...
    try:
        n_bins = min(6, int(valid.sum()) // 10)
        if n_bins < 2:
            raise ValueError("too few bins")
        edges = np.unique(np.percentile(y[valid], np.linspace(0, 100, n_bins + 1)))
        if len(edges) < 3:
            raise ValueError("not enough unique target edges")
        bins = np.digitize(y, edges[1:-1]).astype(int)
        if (~valid).any():
            bins[~valid] = int(np.median(bins[valid]))
        _, counts = np.unique(bins, return_counts=True)
        if (counts < 2).any():
            raise ValueError("some bins have <2 rows")

        idx_all = np.arange(len(positions))
        sss = StratifiedShuffleSplit(n_splits=1, test_size=(1 - train_frac), random_state=seed)
        train_local, temp_local = next(sss.split(idx_all, bins))

        # v4.31 train/test-only mode: when val_frac == 0, all non-train rows
        # are assigned to TEST. A copy of TEST is later used as VAL only to keep
        # legacy logging/checkpoint code functional.
        if val_frac <= 1e-9:
            return positions[train_local], np.asarray([], dtype=int), positions[temp_local]

        test_ratio = (1 - train_frac - val_frac) / max(1 - train_frac, 1e-9)
        if test_ratio < 1e-9:
            return positions[train_local], positions[temp_local], np.asarray([], dtype=int)

        temp_bins = bins[temp_local]
        n_test = max(1, int(len(temp_local) * test_ratio))
        if n_test < len(np.unique(temp_bins)):
            raise ValueError("too few test slots for stratified temp split")
        sss2 = StratifiedShuffleSplit(n_splits=1, test_size=test_ratio, random_state=seed)
        val_local2, test_local2 = next(sss2.split(np.arange(len(temp_local)), temp_bins))
        return positions[train_local], positions[temp_local[val_local2]], positions[temp_local[test_local2]]
    except Exception as exc:
        log.warning("HfO2-family eval stratification failed: %s. Using deterministic random split.", exc)
        perm = rng.permutation(positions)
        n_train = int(len(perm) * train_frac)
        n_val = int(len(perm) * val_frac)
        return perm[:n_train], perm[n_train:n_train+n_val], perm[n_train+n_val:]
...
def build_tier3_hfo2_eval_dataloader(
    df: pd.DataFrame,
    target_col: str,
    aux_cols: List[str],
    train_frac: float,
    ...
    batch_size: int,
    num_workers: int = 4,
    use_saved_test_split: bool = False,
    save_test_split: bool = False,
    overwrite_saved_split: bool = True,
    split_path: Path = TIER3_TEST_SPLIT_PATH,
    use_saved_val_split: bool = False,
    val_split_path: Path = TIER3_VAL_SPLIT_PATH,
    test_only: bool = False,
) -> Tuple[DataLoader, DataLoader, DataLoader, BalancedDistributedSampler, HighKGraphDataset]:
    """Train on the full k-valid Tier-3 set, validate/test only on HfO2 family.

    No leakage: HfO2-family val/test positions are removed from train. All
    non-HfO2-family rows are train-only, so the wider expanded DB acts as an
    auxiliary training corpus rather than changing the headline test metric.
    """
    dataset = HighKGraphDataset(df, target_col=target_col, aux_cols=aux_cols)
    if len(dataset) == 0:
        raise ValueError("No valid Tier-3 rows available for dataloader")

    valid_rows = dataset.df.loc[dataset.valid_idx].reset_index(drop=True)
    all_pos = np.arange(len(dataset), dtype=int)
    eval_mask = valid_rows.apply(is_hfo2_eval_family_row, axis=1).to_numpy(dtype=bool)

    # QUALITY-AWARE-EVAL: headline val/test rows must be verified by default.
    # weak_expanded rows are kept as train-only auxiliary samples.
    if TIER3_TRAIN_CONFIG.get("eval_verified_only", True):
        if "data_quality" in valid_rows.columns:
            quality = valid_rows["data_quality"].fillna("verified").astype(str).str.lower().str.strip()
        else:
            quality = pd.Series("verified", index=valid_rows.index)
        verified_mask = quality.eq("verified") | quality.eq("trusted") | quality.eq("manual_verified")
        n_hfo2_all = int(eval_mask.sum())
        eval_mask = eval_mask & verified_mask.to_numpy(dtype=bool)
        log.info(
            "QUALITY-AWARE-EVAL active: HfO2-family candidates=%d; verified HfO2 eval candidates=%d; train-only nonverified=%d",
            n_hfo2_all, int(eval_mask.sum()), int((~verified_mask.to_numpy(dtype=bool)).sum()),
        )

    eval_pos = all_pos[eval_mask]
    non_eval_pos = all_pos[~eval_mask]

    if len(eval_pos) < 20:
        log.warning(
            "EVAL-FAMILY-SPLIT requested but only %d HfO2-family valid rows found. "
            "Falling back to normal full-dataset split.",
            len(eval_pos),
        )
        return build_dataloader(df, target_col, aux_cols, train_frac, val_frac, batch_size, num_workers)

    loaded_test_pos = load_tier3_test_positions_from_split(valid_rows, split_path) if use_saved_test_split else None

    if loaded_test_pos is not None and len(loaded_test_pos) > 0:
        # BASELINE-SPLIT-LOCK: persistent TEST is exact and is never reused as VAL.
        test_pos = loaded_test_pos.astype(int)
        _saved_test_n = len(pd.read_csv(split_path))
        if len(test_pos) != _saved_test_n:
            raise RuntimeError(
                f"BASELINE-SPLIT-LOCK: TEST identity match {len(test_pos)}/{_saved_test_n}; "
                "partial TEST restoration is forbidden."
            )
        test_set = set(test_pos.tolist())

        loaded_val_pos = None
        if use_saved_val_split and val_split_path.exists():
            loaded_val_pos = load_tier3_test_positions_from_split(valid_rows, val_split_path)
            _saved_val_n = len(pd.read_csv(val_split_path))
            if loaded_val_pos is None or len(loaded_val_pos) != _saved_val_n:
                raise RuntimeError(
                    f"BASELINE-SPLIT-LOCK: VAL identity match "
                    f"{0 if loaded_val_pos is None else len(loaded_val_pos)}/{_saved_val_n}; "
                    "partial VAL restoration is forbidden."
                )

        if test_only:
            train_pos = np.asarray([], dtype=int)
            val_pos = np.asarray([], dtype=int)
            eval_train_pos = np.asarray([], dtype=int)
        elif loaded_val_pos is not None and len(loaded_val_pos) > 0:
            val_pos = loaded_val_pos.astype(int)
            val_set = set(val_pos.tolist())
            if test_set & val_set:
                raise RuntimeError(
                    f"BASELINE-SPLIT-LOCK: persistent VAL/TEST overlap={len(test_set & val_set)}"
                )
            train_pos = np.asarray(
                [p for p in all_pos.tolist() if p not in test_set and p not in val_set], dtype=int
            )
            eval_train_pos = np.asarray(
                [p for p in eval_pos.tolist() if p not in test_set and p not in val_set], dtype=int
            )
        else:
            # First baseline creation only: outer disjoint-VAL block will carve VAL.
            train_pos = np.asarray([p for p in all_pos.tolist() if p not in test_set], dtype=int)
            val_pos = np.asarray([], dtype=int)
            eval_train_pos = np.asarray([p for p in eval_pos.tolist() if p not in test_set], dtype=int)

        log.info(
            "BASELINE-SPLIT-LOCK: restored TEST=%d VAL=%d TRAIN=%d test_only=%s",
            len(test_pos), len(val_pos), len(train_pos), bool(test_only),
        )
    else:
        eval_train_pos, val_pos, test_pos = _split_positions_stratified(
            valid_rows, eval_pos, target_col, train_frac, val_frac, seed=42
        )
        if save_test_split:
            save_tier3_test_split_rows(
                dataset, valid_rows, test_pos, split_path,
                split_name="test", overwrite=overwrite_saved_split,
            )
        if len(val_pos) == 0 and len(test_pos) > 0 and TIER3_TRAIN_CONFIG.get("tier3_disjoint_val", False):
            # Never manufacture VAL from TEST. The outer baseline split block must
            # create or restore a genuinely disjoint validation population.
            log.info("BASELINE-SPLIT: initial VAL empty; awaiting disjoint VAL creation/restoration.")
        elif len(val_pos) == 0 and len(test_pos) > 0:
            raise RuntimeError(
                "Unsafe empty VAL in non-disjoint mode. VAL==TEST fallback has been removed."
            )
        train_pos = np.concatenate([non_eval_pos, eval_train_pos]).astype(int)

    rng = np.random.default_rng(42)
    train_pos = rng.permutation(train_pos)

    log.info("─" * 68)
    log.info(
        "EVAL-FAMILY-SPLIT active: TRAIN=full expanded DB; VAL/TEST=HfO2-family only"
    )
    log.info(
        "  valid rows total=%d | verified-HfO2-family=%d | train-only-other/weak=%d",
        len(dataset), len(eval_pos), len(non_eval_pos),
    )
    if "data_quality" in valid_rows.columns:
        log.info("  data_quality in k-valid rows: %s", valid_rows["data_quality"].fillna("verified").astype(str).value_counts().to_dict())
    log.info(
        "  split positions: train=%d (including HfO2-train=%d + wider=%d)  val=%d  test=%d",
        len(train_pos), len(eval_train_pos), len(non_eval_pos), len(val_pos), len(test_pos),
    )
    for name, pos in [("TRAIN", train_pos), ("VAL-HfO2", val_pos), ("TEST-HfO2", test_pos)]:
        vals = pd.to_numeric(valid_rows.iloc[pos][target_col], errors="coerce")
        src_exp = int((valid_rows.iloc[pos].get("source", pd.Series(index=valid_rows.iloc[pos].index)) == "Experimental").sum()) if len(pos) else 0
        log.info(
            "  %-10s N=%-4d experimental=%-4d %s range=[%.3f, %.3f] mean=%.3f",
            name, len(pos), src_exp, target_col,
            float(vals.min()) if len(vals) else float("nan"),
            float(vals.max()) if len(vals) else float("nan"),
            float(vals.mean()) if len(vals) else float("nan"),
        )
    log.info("─" * 68)

    collate = HighKGraphDataset.collate_fn
    train_ds = Subset(dataset, train_pos.tolist())
    val_ds = Subset(dataset, val_pos.tolist())
    test_ds = Subset(dataset, test_pos.tolist())

    if _DIST["active"]:
        try:
            proc_avail = [
                int(dataset.df.loc[dataset.valid_idx[i], "source"] == "Experimental")
                for i in train_pos.tolist()
            ]
        except Exception:
            proc_avail = [0] * len(train_pos)
        train_sampler = BalancedDistributedSampler(
            train_ds, proc_avail=proc_avail, num_replicas=_DIST["world"],
            rank=_DIST["rank"], shuffle=True,
        )
        shuffle_flag = False
    else:
        train_sampler = None
    ...
    train_loader = DataLoader(
        train_ds, batch_size=batch_size, shuffle=shuffle_flag, sampler=train_sampler,
...
def run_tier2_finetune(
    ...
    if cfg.get("log_transform", False):
        ...
        _t2_true_k = torch.exp(t_valid).cpu().numpy()
        mad_k_exact = float(np.mean(np.abs(_t2_true_k - np.mean(_t2_true_k))))
        mad_mae_k_exact = mad_k_exact / mae_k_exact if mae_k_exact > 0 else float("nan")
        n_k_exact = int(valid.sum().item())
    ...
    # v4.51 DUAL-SCALE: replace the Tier-2 primary row with metrics computed
    # from the same exact TEST prediction/target tensors used above.  This avoids
    # mixing test MAE with a full-dataset MAD and makes Tier-2/Tier-3 reporting identical.
    if cfg.get("log_transform", False) and cfg["target"] in mt_results:
        mt_results[cfg["target"]].update({
            "mae_log": float(test_mae_log),
            "rmse_log": float(test_rmse_log),
            "mae_linear": float(mae_k_exact),
            "rmse_linear": float(rmse_k_exact),
            "mad_linear": float(mad_k_exact),
            "mad_mae_linear": float(mad_mae_k_exact),
            # Backward-compatible aliases remain linear/publication scale.
            "mae": float(mae_k_exact),
            "rmse": float(rmse_k_exact),
            "mad": float(mad_k_exact),
            "mad_mae_ratio": float(mad_mae_k_exact),
            "n": int(n_k_exact),
            "coverage_pct": 100.0,
            "mad_source": "same fixed TEST rows as primary evaluation",
            "metric_scale": "dual: log diagnostic + linear exact",
            "display_name": "k_total",
        })
    ...
    if cfg.get("log_transform", False):
        ...
        primary_metrics["mad_linear_k"]  = mad_k_exact
        primary_metrics["mad_mae_linear"] = mad_mae_k_exact
        primary_metrics["n_test"]        = n_k_exact
...
def _impute_structures(
    ...
    # Build formula -> all donor rows, not just first donor. v4.29 uses this
    # to choose phase/process-aware donors where multiple structures exist.
    formula_to_donors: dict[str, list[pd.Series]] = {}
    for formula, grp in donor_pool.groupby("formula", sort=False):
        formula_to_donors[str(formula)] = [r for _, r in grp.iterrows()]

    def _candidate_donors(material: str) -> list[pd.Series]:
        """Return ordered donor candidates for a material string."""
        m = str(material).strip()
        if not m or m.lower() in ("nan", "none", ""):
            return []
        candidates: list[pd.Series] = []

        def _add_formula(f: str):
            if f in formula_to_donors:
                candidates.extend(formula_to_donors[f])
            f_red = _reduced(f)
            if f_red in reduced_map:
                canon = reduced_map[f_red]
                if canon in formula_to_donors:
                    candidates.extend(formula_to_donors[canon])

        # 1. Exact/reduced formula.
        _add_formula(m)

        # 2. Alias table candidates.
        if m in FORMULA_ALIASES:
            for alias in FORMULA_ALIASES[m]:
                _add_formula(alias)

        # 3. Substring / stack-token candidates.
        first_token = m.split("/")[0].split("_")[0].split(" ")[0].strip()
        if first_token != m:
            _add_formula(first_token)
        for canon in formula_to_donors:
            if m.startswith(canon) or canon.startswith(m):
                _add_formula(canon)

        # 4. Last-resort family fallback.
        if not candidates:
            if "Hf" in m:
                _add_formula("HfO2")
            if "Zr" in m:
        ...
        # Deduplicate by row_hash/jid/mp_id while preserving order.
        seen = set()
        uniq = []
        for d in candidates:
            key = str(d.get("row_hash", d.get("jid", d.get("mp_id", id(d)))))
            if key not in seen:
                seen.add(key)
                uniq.append(d)
        return uniq

    def _phase_hint(row: pd.Series) -> str:
        """Infer a coarse ALD film phase hint from process fields only.

        This is deliberately target-leakage safe: it does NOT inspect measured k.
        It is a practical selector, not a true phase classifier.
        """
        mat = str(row.get("material", row.get("material_system", "")))
        fam = infer_material_family_from_row(row)
        reported_phase = str(row.get("reported_phase", row.get("crystal_phase", ""))).lower()
        anneal = pd.to_numeric(row.get("anneal_temp_C", row.get("anneal_temp_PDA_C", np.nan)), errors="coerce")
        tsub = pd.to_numeric(row.get("substrate_temp_C", row.get("ald_substrate_temp_C", np.nan)), errors="coerce")
        ambient = str(row.get("anneal_ambient", row.get("anneal_atmosphere", ""))).lower()
        has_zr = fam == "hzo_like" or ("Zr" in mat) or ("HZO" in mat.upper()) or ("Hf0." in mat)
        doped = fam.startswith("doped_hfo2") or any(x in mat for x in [":", "Al", "La", "Y", "Gd", "Sr", "Si", "N"])
        if "orthorhombic" in reported_phase:
            return "reported_orthorhombic_like"
        if "tetragonal" in reported_phase:
            return "reported_tetragonal_like"
        if "monoclinic" in reported_phase:
            return "reported_monoclinic_like"
        if "amorph" in reported_phase:
            return "reported_amorphous_like"
        if has_zr:
            if pd.notna(anneal) and anneal >= 400:
                return "hzo_annealed_orthorhombic_like"
            return "hzo_mixed_like"
        if pd.notna(anneal) and anneal >= 450:
            return "annealed_monoclinic_like"
        if pd.notna(tsub) and tsub <= 300 and (pd.isna(anneal) or anneal < 400):
            return "low_temp_amorphous_like"
        if doped:
            return "doped_hfo2_like"
        if "o2" in ambient or "n2" in ambient or "forming" in ambient:
            return "pda_modified_like"
        return "generic_hfo2_like"

    def _score_donor(row: pd.Series, donor: pd.Series) -> float:
        """Score donor candidate using formula/process metadata only."""
        mat = str(row.get("material", row.get("material_system", "")))
        fam = infer_material_family_from_row(row)
        d_formula = str(donor.get("formula", ""))
        source = str(donor.get("source", ""))
        phase_hint = _phase_hint(row)
        score = 0.0

        # Curated material-family preference from compact DB.
        if fam == "hzo_like":
            if "Hf" in d_formula and "Zr" in d_formula:
                score += 70.0
            elif d_formula == "HfO2":
                score -= 25.0
        elif fam == "doped_hfo2_al":
            if "Hf" in d_formula and "Al" in d_formula:
                score += 60.0
            elif d_formula == "HfO2":
                score += 10.0
        elif fam.startswith("doped_hfo2"):
            if "Hf" in d_formula and d_formula != "HfO2":
                score += 40.0
        elif fam == "pure_hfo2" and d_formula == "HfO2":
            score += 60.0

        # Formula/family matching.
        if d_formula == mat:
            score += 100.0
        if _reduced(d_formula) == _reduced(mat):
            score += 60.0
        if "Hf" in mat and "Hf" in d_formula:
            score += 25.0
        if "Zr" in mat and "Zr" in d_formula:
            score += 25.0
        if "Al" in mat and "Al" in d_formula:
            score += 15.0
        if "Si" in mat and "Si" in d_formula:
            score += 15.0

        # Phase/process heuristics. We generally prefer HZO donors for annealed HZO,
        # pure HfO2 for pure HfO2, and avoid mapping HZO to pure HfO2 when a mixed
        # Hf-Zr donor exists.
        if "hzo" in phase_hint:
            if "Hf" in d_formula and "Zr" in d_formula:
                score += 40.0
            elif d_formula == "HfO2":
                score -= 10.0
        if "annealed_monoclinic" in phase_hint and d_formula == "HfO2":
            score += 20.0
        if "low_temp_amorphous" in phase_hint:
            # No amorphous donor exists in JARVIS/MP; stable HfO2 is the least-bad
            # crystalline proxy, but this row will still need process residual.
            if d_formula == "HfO2":
                score += 10.0
        if "doped_hfo2" in phase_hint:
            # If a ternary donor exists, prefer it; otherwise pure HfO2 remains fallback.
            if d_formula not in ("HfO2", "ZrO2") and "Hf" in d_formula:
                score += 25.0

        # Prefer JARVIS/OptB88vdW donors for consistency with Tier2 backbone.
        if source.startswith("JARVIS"):
            score += 5.0

        # Stability tie-break, small effect only.
        ef_abs = pd.to_numeric(donor.get("formation_energy_per_atom", donor.get("formation_energy_peratom", np.nan)), errors="coerce")
        if pd.notna(ef_abs):
            score -= 0.01 * abs(float(ef_abs))
        return score

    def _find_donor(row: pd.Series) -> tuple[Optional[pd.Series], dict]:
        """Return best phase-aware donor and audit metadata."""
        material = row.get("material", row.get("material_system", ""))
        candidates = _candidate_donors(str(material))
        meta = {
            "phase_hint": _phase_hint(row),
            "n_candidates": len(candidates),
            "strategy": "phase_aware" if len(candidates) else "unmatched",
        ...
        if not TIER3_TRAIN_CONFIG.get("phase_aware_donor_selection", True):
            d = candidates[0]
            meta.update({
                "strategy": "first_match_legacy",
                "best_score": 0.0,
                "candidate_formulas": ",".join(sorted({str(c.get("formula", "?")) for c in candidates}))[:200],
            })
            return d, meta

        scored = [(float(_score_donor(row, d)), d) for d in candidates]
        scored.sort(key=lambda x: x[0], reverse=True)
        best_score, best_donor = scored[0]
        meta.update({
            "best_score": best_score,
            "candidate_formulas": ",".join([str(d.get("formula", "?")) for _, d in scored[:TIER3_TRAIN_CONFIG.get("donor_log_topn", 5)]])[:200],
        })
        return best_donor, meta
    ...
    for idx, row in df_imp.iterrows():
        donor, donor_meta = _find_donor(row)
        if donor is not None:
            ...
            df_imp.at[idx, "imputed_donor_formula"] = str(donor.get("formula", ""))
            df_imp.at[idx, "imputed_donor_source"]  = str(donor.get("source", ""))
            df_imp.at[idx, "imputed_donor_strategy"] = donor_meta.get("strategy", "")
            df_imp.at[idx, "imputed_phase_hint"] = donor_meta.get("phase_hint", "")
            df_imp.at[idx, "imputed_donor_score"] = donor_meta.get("best_score", float("nan"))
            df_imp.at[idx, "imputed_candidate_formulas"] = donor_meta.get("candidate_formulas", "")
...
def run_tier3_finetune(
    ...
    _tier3_loader_fn = (
        build_tier3_hfo2_eval_dataloader
        if cfg.get("eval_hfo2_family_only", False)
        else build_dataloader
    )
    if cfg.get("eval_hfo2_family_only", False):
        train_loader, val_loader, test_loader, train_sampler, _main_ds_t3 = _tier3_loader_fn(
            df         = df_phase_b,
            target_col = cfg["target"],
            aux_cols   = cfg["aux_targets"],
            train_frac = cfg["train_ratio"],
            val_frac   = cfg["val_ratio"],
            batch_size = cfg["batch_size"],
            # BASELINE-SPLIT-LOCK: create TEST once, then restore it exactly.
            use_saved_test_split = bool(TIER3_TEST_SPLIT_PATH.exists()),
            save_test_split = bool(not TIER3_TEST_SPLIT_PATH.exists()),
            overwrite_saved_split = False,
        )
    else:
        train_loader, val_loader, test_loader, train_sampler, _main_ds_t3 = _tier3_loader_fn(
            df         = df_phase_b,
    ...
    # v4.41 CANONICAL-SPLIT: once the normal run has saved the split (or the
    # ablation run has loaded it), rebuild all three loaders from that exact
    # one-to-one split. These canonical loaders are reused for epoch-0,
    # intermediate checkpoints, training validation, best checkpoint, dataset
    # composition, and final reporting. num_workers=0 removes worker-dependent
    # graph-drop variability from audit/evaluation.
    if cfg.get("eval_hfo2_family_only", False) and TIER3_TEST_SPLIT_PATH.exists():
        train_loader, val_loader, test_loader, train_sampler, _main_ds_t3 = build_tier3_hfo2_eval_dataloader(
            df         = df_phase_b,
            target_col = cfg["target"],
            aux_cols   = cfg["aux_targets"],
            train_frac = cfg["train_ratio"],
            ...
            batch_size = cfg["batch_size"],
            num_workers = 0,
            use_saved_test_split = True,
            save_test_split = False,
            overwrite_saved_split = False,
            use_saved_val_split = bool(TIER3_VAL_SPLIT_PATH.exists()),
        )
        # v4.43 SCHEMA-CANONICAL-SPLIT FIX:
        # A row can build a valid graph individually yet still be dropped by
        # collate_fn when its graph schema differs from the first graph in the
        # batch. That was the remaining 37 raw rows -> 29 evaluated rows gap.
        # Materialise each held-out row, compute the exact graph + line-graph
        # schema signature used by batching, retain the dominant compatible
        # schema only, and persist exactly that population.
        from collections import Counter

        def _audit_schema_key(item):
            if item is None:
                return None
            def _one(g):
                try:
                    ...
                    return (
                        tuple(sorted(g.ndata.keys())),
                        tuple(sorted(g.edata.keys())),
                        int(nf.shape[-1]) if nf is not None else 0,
                    )
                except Exception:
                    return None
            return (_one(item["graph"]), _one(item["line_graph"]))

        _raw_test_pos = list(getattr(test_loader.dataset, "indices", []))
        _schema_by_pos = {}
        _graph_failed_test_pos = []
        for _p in _raw_test_pos:
            try:
                _item = _main_ds_t3[int(_p)]
            except BaseException:
                _item = None
            _key = _audit_schema_key(_item)
            if _key is None or _key[0] is None or _key[1] is None:
                _graph_failed_test_pos.append(int(_p))
            else:
                _schema_by_pos[int(_p)] = _key

        if not _schema_by_pos:
            raise RuntimeError(
                "v4.43: no graph/schema-evaluable rows remain in canonical Tier3 TEST split"
            )

        _schema_counts = Counter(_schema_by_pos.values())
        _canonical_schema, _canonical_schema_n = _schema_counts.most_common(1)[0]
        _graph_ok_test_pos = [
            p for p in _raw_test_pos if _schema_by_pos.get(int(p)) == _canonical_schema
        ]
        _schema_mismatch_pos = [
            p for p in _raw_test_pos
            if int(p) in _schema_by_pos and _schema_by_pos[int(p)] != _canonical_schema
        ]

        log.info(
            "v4.43 SCHEMA-CANONICAL-SPLIT: raw=%d graph_failed=%d "
            "schema_mismatch=%d canonical=%d schema_groups=%s",
            len(_raw_test_pos), len(_graph_failed_test_pos),
            len(_schema_mismatch_pos), len(_graph_ok_test_pos),
            sorted(_schema_counts.values(), reverse=True),
        )
        if _graph_failed_test_pos:
            log.info("v4.43 graph-failed subset positions: %s", _graph_failed_test_pos)
        if _schema_mismatch_pos:
            log.info("v4.43 schema-mismatch subset positions: %s", _schema_mismatch_pos)

        _canonical_subset = Subset(_main_ds_t3, _graph_ok_test_pos)
        _eval_loader_kwargs = dict(
            batch_size=cfg["batch_size"],
            shuffle=False,
            num_workers=0,
            collate_fn=HighKGraphDataset.collate_fn,
            pin_memory=torch.cuda.is_available(),
        )
        # TEST-only canonicalization. Never construct VAL from TEST.
        test_loader = DataLoader(_canonical_subset, **_eval_loader_kwargs)

        _saved_test_count = len(pd.read_csv(TIER3_TEST_SPLIT_PATH))
        # v4.60.2 BASELINE-SPLIT-LOCK (heal): separate two distinct conditions the
        # v4.60 check wrongly conflated.
        #   (a) restored != saved  -> genuine identity-resolution failure: some
        #       saved TEST rows are missing from the current dataset (DB/key drift).
        #       This must still abort; healing would hide real corruption.
        #   (b) restored == saved but canonical < restored -> all saved rows resolve,
        #       but some are not graph/schema-evaluable on the dominant DGL schema and
        #       therefore CANNOT be part of any evaluated TEST set. The persisted CSV
        #       (raw sampled rows) is simply out of sync with what is evaluable. Heal
        #       by re-persisting the canonical subset as the locked TEST identity. The
        #       canonical rows are a strict subset of the previously-saved rows, so this
        #       only NARROWS the lock to exactly the rows already being evaluated — it
        #       never introduces new rows and does not change the measured TEST set.
        if len(_raw_test_pos) != _saved_test_count:
            raise RuntimeError(
                "BASELINE-SPLIT-LOCK: persistent TEST identity resolution failed "
                f"(saved={_saved_test_count}, restored={len(_raw_test_pos)}). Some saved "
                "TEST rows are missing from the current dataset; refusing to proceed."
            )
        if len(_graph_ok_test_pos) != len(_raw_test_pos):
            _n_dropped = len(_raw_test_pos) - len(_graph_ok_test_pos)
            if not cfg.get("tier3_allow_split_creation", True):
                raise RuntimeError(
                    "BASELINE-SPLIT-LOCK: persistent TEST needs healing "
                    f"(saved={_saved_test_count}, canonical={len(_graph_ok_test_pos)}, "
                    f"drop={_n_dropped}) but tier3_allow_split_creation=False. Re-enable "
                    "split creation for one baseline run to heal, then freeze again."
                )
            log.warning(
                "BASELINE-SPLIT-LOCK: healing persistent TEST identity: saved=%d restored=%d "
                "canonical=%d (dropping %d non-graph/schema-evaluable rows). Re-persisting the "
                "canonical subset as the locked TEST identity.",
                _saved_test_count, len(_raw_test_pos), len(_graph_ok_test_pos), _n_dropped,
            )
            if is_rank0():
                _valid_rows_for_split = _main_ds_t3.df.loc[_main_ds_t3.valid_idx].reset_index(drop=True)
                save_tier3_test_split_rows(
                    _main_ds_t3,
                    _valid_rows_for_split,
                    np.asarray(_graph_ok_test_pos, dtype=int),
                    path=TIER3_TEST_SPLIT_PATH,
                    split_name="test",
                    overwrite=True,
                )
            dist_barrier()
            _healed_count = len(pd.read_csv(TIER3_TEST_SPLIT_PATH))
            if _healed_count != len(_graph_ok_test_pos):
                raise RuntimeError(
                    "BASELINE-SPLIT-LOCK: TEST heal verification failed "
                    f"(re-saved={_healed_count}, canonical={len(_graph_ok_test_pos)})."
                )
            log.info(
                "BASELINE-SPLIT-LOCK: TEST identity healed and locked at N=%d (was %d saved); "
                "these are the same rows evaluated by prior versions.",
                len(_graph_ok_test_pos), _saved_test_count,
            )
        dist_barrier()

        _canonical_test_n = len(_graph_ok_test_pos)
        _canonical_val_n = len(getattr(val_loader.dataset, "indices", []))
        log.info(
            "BASELINE TEST-CANONICAL: train=%d preloaded_val=%d test=%d; TEST identities unchanged",
            len(getattr(train_loader.dataset, "indices", [])),
            _canonical_val_n, _canonical_test_n,
        )
    else:
        _canonical_test_n = len(getattr(test_loader.dataset, "indices", []))
        _canonical_val_n = _canonical_test_n   # DISJOINT-VAL: keep defined for legacy path

    # ══════════════════════════════════════════════════════════════════════
    # DISJOINT-VAL (v4.59): carve a held-out VAL set out of the TRAIN pool so
    # checkpoint selection / early stopping never observe TEST. TEST is left
    # untouched and stays the once-only holdout reported by tier3_evaluate.
    # VAL candidates are eval-family verified rows currently assigned to TRAIN
    # (never TEST). VAL is removed from TRAIN to prevent train/val leakage, then
    # schema-canonicalised exactly like TEST so evaluated N is deterministic.
    #
    # v4.59.1 SILENT-REGRESSION GUARD: when tier3_disjoint_val is requested, an
    # inadequate candidate pool is a HARD ERROR, never a silent downgrade to
    # legacy VAL==TEST. Mode is tracked by an explicit boolean, not by a
    # val/test count coincidence. If the flag is requested but the carve does
    # not complete, the run aborts rather than reporting a selected-on-test
    # number that looks clean.
    # ══════════════════════════════════════════════════════════════════════
    _tier3_disjoint_val_active = False   # v4.59.1: explicit mode flag (not a count inference)
    if cfg.get("tier3_disjoint_val", False) and cfg.get("eval_hfo2_family_only", False):
        from collections import Counter as _CounterDV

        _dv_seed     = int(cfg.get("tier3_disjoint_val_seed", 42))
        _dv_ratio    = float(cfg.get("tier3_disjoint_val_ratio", 0.20))
        _dv_min_pool = int(cfg.get("tier3_disjoint_val_min_pool", 6))

        _train_pos_all = [int(p) for p in getattr(train_loader.dataset, "indices", [])]
        _test_pos_set  = set(int(p) for p in getattr(test_loader.dataset, "indices", []))
        _valid_rows_dv = _main_ds_t3.df.loc[_main_ds_t3.valid_idx].reset_index(drop=True)

        def _is_val_candidate_dv(pos: int) -> bool:
            if pos in _test_pos_set or pos < 0 or pos >= len(_valid_rows_dv):
                return False
            r = _valid_rows_dv.iloc[pos]
            if not is_hfo2_eval_family_row(r):
                return False
            if not cfg.get("eval_verified_only", True):
                return True
            q = str(r.get("data_quality", "verified")).lower().strip()
            return q in ("verified", "trusted", "manual_verified")

        _existing_val_pos = [int(p) for p in getattr(val_loader.dataset, "indices", [])]
        _min_val_rows = int(cfg.get("tier3_disjoint_val_min_rows", 15))
        _persistent_val_loaded = bool(TIER3_VAL_SPLIT_PATH.exists() and _existing_val_pos)

        if _persistent_val_loaded:
            _val_set_dv = set(_existing_val_pos)
            _train_set_dv = set(_train_pos_all)
            if _val_set_dv & _test_pos_set or _val_set_dv & _train_set_dv:
                raise RuntimeError(
                    f"BASELINE-SPLIT-LOCK leakage: val∩test={len(_val_set_dv & _test_pos_set)} "
                    f"val∩train={len(_val_set_dv & _train_set_dv)}"
                )
            if len(_existing_val_pos) < _min_val_rows:
                raise RuntimeError(
                    f"BASELINE-SPLIT-LOCK: persistent VAL has {len(_existing_val_pos)} rows; "
                    f"minimum required is {_min_val_rows}."
                )
            _canonical_val_n = len(_existing_val_pos)
            _tier3_disjoint_val_active = True
            log.info(
                "BASELINE-SPLIT-LOCK: reused persistent identity splits TRAIN=%d VAL=%d TEST=%d",
                len(_train_pos_all), _canonical_val_n, len(_test_pos_set),
            )

        _val_candidate_pos = [p for p in _train_pos_all if _is_val_candidate_dv(p)]

        if not _persistent_val_loaded and len(_val_candidate_pos) < _dv_min_pool:
            # v4.59.1: PATH 1 (small-pool) is now a HARD ERROR. Silently reverting
            # to VAL==TEST here is exactly the selection-on-test contamination this
            # feature exists to prevent, and it would reappear with no signal after
            # a future DB shrink. Fail loudly so a human decides how to proceed.
            raise RuntimeError(
                "DISJOINT-VAL ABORT: tier3_disjoint_val=True but only "
                f"{len(_val_candidate_pos)} eval-family verified TRAIN rows are "
                f"available (need >= {_dv_min_pool}). Refusing to silently fall back "
                "to VAL==TEST, which would reintroduce selecting on the test set. "
                "Fix the data pool, lower tier3_disjoint_val_min_pool deliberately, "
                "or set tier3_disjoint_val=False to run legacy mode on purpose."
            )
        # First accepted baseline run creates VAL once; later runs restore it by identity.
        if not _persistent_val_loaded:
            if TIER3_VAL_SPLIT_PATH.exists() and cfg.get("tier3_lock_persistent_splits", True):
                raise RuntimeError("BASELINE-SPLIT-LOCK: VAL file exists but was not restored exactly.")
            if not cfg.get("tier3_allow_split_creation", True):
                raise RuntimeError("BASELINE-SPLIT-LOCK: split creation disabled and no valid VAL split exists.")
            _cand_arr = np.asarray(_val_candidate_pos, dtype=int)
            # Held-out VAL = the "val" partition of a stratified (keep-train / val)
            # split over the candidate pool; test_frac collapses to 0 here.
            _keep_local, _val_local, _unused_local = _split_positions_stratified(
                _valid_rows_dv, _cand_arr, cfg["target"],
                train_frac = (1.0 - _dv_ratio),
                val_frac   = _dv_ratio,
                seed       = _dv_seed,
            )
            _val_pos_raw = np.asarray(_val_local, dtype=int)
            if _val_pos_raw.size == 0:
                # v4.59.1: PATH 2 (stratifier degenerate). This is NOT a leakage risk —
                # the random hold-out below is still disjoint from TRAIN/TEST and is
                # leakage-guarded downstream — but it is a quality downgrade, so log it
                # loudly rather than let it pass as a clean stratified split.
                log.warning(
                    "DISJOINT-VAL: stratified VAL hold-out degenerated (empty). Falling "
                    "back to a DETERMINISTIC RANDOM disjoint hold-out (seed=%d). VAL is "
                    "still disjoint from TRAIN/TEST, but is not stratified across the k "
                    "range — treat this run's val-selection as lower-confidence.",
                    _dv_seed,
                )
                _rng_dv = np.random.default_rng(_dv_seed)
                _perm_dv = _rng_dv.permutation(_cand_arr)
                _n_val_dv = max(1, int(round(_dv_ratio * len(_perm_dv))))
                _val_pos_raw = _perm_dv[:_n_val_dv]

            # Schema-canonicalise VAL (same rule as v4.43 TEST canonicalisation).
            def _schema_key_dv(item):
                if item is None:
                    return None
                def _one(g):
                    try:
            ...
            _schema_by_pos_dv = {}
            for _p in _val_pos_raw.tolist():
                try:
                    _it_dv = _main_ds_t3[int(_p)]
                except BaseException:
                    _it_dv = None
                _k_dv = _schema_key_dv(_it_dv)
                if _k_dv is not None and _k_dv[0] is not None and _k_dv[1] is not None:
                    _schema_by_pos_dv[int(_p)] = _k_dv

            if _schema_by_pos_dv:
                _canon_schema_dv, _ = _CounterDV(_schema_by_pos_dv.values()).most_common(1)[0]
                _val_pos = [p for p in _val_pos_raw.tolist()
                            if _schema_by_pos_dv.get(int(p)) == _canon_schema_dv]
            else:
                _val_pos = _val_pos_raw.tolist()

            _val_set_dv = set(int(p) for p in _val_pos)
            _new_train_pos = [p for p in _train_pos_all if int(p) not in _val_set_dv]

            # Rebuild TRAIN (minus VAL) and a dedicated disjoint VAL loader.
            _collate_dv = HighKGraphDataset.collate_fn
            _train_ds_dv = Subset(_main_ds_t3, _new_train_pos)
            _val_ds_dv   = Subset(_main_ds_t3, list(_val_pos))

            if _DIST["active"]:
                try:
                    _proc_avail_dv = [
                        int(_main_ds_t3.df.loc[_main_ds_t3.valid_idx[i], "source"] == "Experimental")
                        for i in _new_train_pos
                    ]
                except Exception:
                    _proc_avail_dv = [0] * len(_new_train_pos)
                train_sampler = BalancedDistributedSampler(
                    _train_ds_dv, proc_avail=_proc_avail_dv,
                    num_replicas=_DIST["world"], rank=_DIST["rank"], shuffle=True,
                )
                _shuffle_dv = False
            else:
                train_sampler = None
            ...
            train_loader = DataLoader(
                _train_ds_dv, batch_size=cfg["batch_size"], shuffle=_shuffle_dv,
                sampler=train_sampler, num_workers=0, collate_fn=_collate_dv, drop_last=True,
            )
            val_loader = DataLoader(
                _val_ds_dv, batch_size=cfg["batch_size"], shuffle=False,
                num_workers=0, collate_fn=_collate_dv,
            )
            _canonical_val_n = len(_val_pos)
            if _canonical_val_n < _min_val_rows:
                raise RuntimeError(
                    f"BASELINE-SPLIT-LOCK: newly carved VAL has {_canonical_val_n} graph-evaluable rows; "
                    f"minimum required is {_min_val_rows}. Increase the candidate pool or ratio."
                )

            if is_rank0():
                save_tier3_test_split_rows(
                    _main_ds_t3, _valid_rows_dv,
                    np.asarray(_val_pos, dtype=int),
                    path=TIER3_VAL_SPLIT_PATH, split_name="val", overwrite=True,
                )
            dist_barrier()

            # Leakage guards: VAL ∩ TEST == ∅ and VAL ∩ TRAIN == ∅.
            _leak_vt  = _val_set_dv & _test_pos_set
            _leak_vtr = _val_set_dv & set(_new_train_pos)
            if _leak_vt or _leak_vtr:
                raise RuntimeError(
                    f"DISJOINT-VAL leakage detected: val∩test={len(_leak_vt)} "
                    f"val∩train={len(_leak_vtr)}"
                )

            log.info("─" * 68)
            log.info(
                "DISJOINT-VAL active: VAL carved from TRAIN, disjoint from TEST.  "
                "train=%d  val=%d  test=%d  (val_frac_of_evalfamily_train=%.2f seed=%d)",
                len(_new_train_pos), _canonical_val_n, _canonical_test_n,
                _dv_ratio, _dv_seed,
            )
            log.info("DISJOINT-VAL leakage check passed: val∩test=0 val∩train=0")
            log.info(
                "DISJOINT-VAL selection policy: tier3_best.pt chosen on VAL log-MAE; "
                "TEST sealed during optimization and reported once after the frozen best checkpoint."
            )
            log.info("─" * 68)
            # v4.59.1: mark disjoint mode as genuinely active. Every downstream branch
            # keys off THIS flag, not a val/test count comparison, so a coincidental
            # val==test count can never be misread as legacy mode.
            _tier3_disjoint_val_active = True

    # v4.59.1 EXIT GUARD: if disjoint val was requested but the carve did not
    # complete for ANY reason, abort. This makes "requested but silently didn't
    # run" impossible regardless of which internal path failed to set the flag.
    if cfg.get("tier3_disjoint_val", False) and cfg.get("eval_hfo2_family_only", False) \
            and not _tier3_disjoint_val_active:
        raise RuntimeError(
            "DISJOINT-VAL ABORT: tier3_disjoint_val=True was requested but the "
            "disjoint VAL carve did not complete (_tier3_disjoint_val_active is "
            "False). Refusing to proceed with a possibly VAL==TEST selection."
        )

    if is_rank0():
        log.info(
            "BASELINE SPLIT FINGERPRINTS | VAL=%s | TEST=%s",
            _tier3_split_fingerprint(TIER3_VAL_SPLIT_PATH),
            _tier3_split_fingerprint(TIER3_TEST_SPLIT_PATH),
        )
        split_manifest = {
            "version": "v4.60_locked_disjoint_baseline",
            "val_path": str(TIER3_VAL_SPLIT_PATH),
            "test_path": str(TIER3_TEST_SPLIT_PATH),
            "val_sha256": _tier3_split_fingerprint(TIER3_VAL_SPLIT_PATH),
            "test_sha256": _tier3_split_fingerprint(TIER3_TEST_SPLIT_PATH),
            "val_n": int(_canonical_val_n),
            "test_n": int(_canonical_test_n),
            "selection_metric": "VAL log-MAE only",
            "test_policy": "sealed until final frozen-checkpoint evaluation",
        }
        (SPLIT_ROOT / "tier3_locked_split_manifest.json").write_text(
    ...
    # v4.57 critical wiring fix: v4.56 defined the adapter but never enabled it
    # in the Tier-3 training path, so every adapter run was effectively frozen-base.
    model.configure_structural_adapter(
        enabled=cfg.get("tier3_use_structural_adapter", True),
        rank=cfg.get("tier3_structural_adapter_rank", 16),
        reset=True,
    )
    if cfg.get("bounded_process_residual", False):
        model.reset_bounded_process_residual(
            seed=cfg.get("process_delta_init_seed", 42)
        )
    model.unfreeze_all()
    ...
    def _eval_exact_k_for_diag(loader, split_name: str, stage_name: str) -> dict:
        ...
        # DISJOINT-VAL: VAL and TEST are now distinct populations with distinct N.
        _expected_diag_n = _canonical_val_n if split_name.upper() == "VAL" else _canonical_test_n
        if split_name.upper() in ("VAL", "TEST") and out["n"] != _expected_diag_n:
            raise RuntimeError(
                f"v4.43 AUDIT COUNT MISMATCH at {stage_name}/{split_name}: "
                f"observed N={out['n']} expected N={_expected_diag_n}"
            )
        log.info(
    ...
    def _write_tier3_prediction_audit(
        loader,
        df_ref: pd.DataFrame,
        split_name: str,
        stage_name: str,
        ...
        """Dump per-row Tier-3 prediction, residual, donor, and error diagnostics.

        Output files under highk_project/reports/tier3_prediction_audit/:
          - tier3_<stage>_<split>_prediction_audit.csv
          - tier3_<stage>_<split>_top20_errors.csv
          - tier3_<stage>_<split>_material_statistics.csv
          - tier3_<stage>_<split>_donor_statistics.csv
          - tier3_<stage>_<split>_residual_statistics.csv

        This is deliberately rank-0 only. Val/test loaders are not distributed,
        so rank 0 has the complete split and writes one canonical report.
        """
        if not is_rank0():
            return None

        audit_dir = REPORT_ROOT / "tier3_prediction_audit" / ("ablation" if trainer.ablate_context else "normal")
        audit_dir.mkdir(parents=True, exist_ok=True)
        safe_stage = str(stage_name).replace("/", "_").replace(" ", "_")
        ...
        with torch.no_grad():
            for batch in loader:
                ...
                proc_ctx_original = batch.get("proc_context")
                stack_ctx_original = batch.get("stack_context")
                proc_ctx = proc_ctx_original
                stack_ctx = stack_ctx_original
                if proc_ctx is not None:
                    proc_ctx  = {k: v.to(trainer.device) for k, v in proc_ctx.items()}
                    stack_ctx = {k: v.to(trainer.device) for k, v in stack_ctx.items()}
                if trainer.ablate_context:
                    proc_ctx = stack_ctx = None
                ...
                # Component-level forward for residual-proc diagnostics.
                embedding = model_core.backbone((graph, line_graph, None))
                if not getattr(model_core, "use_bounded_process_residual", False):
                    embedding = model_core.dropout(embedding)
                embedding = model_core._apply_func_conditioning(embedding, func_code)

                task_name = cfg["target"]
                base_log = model_core.task_heads[task_name](embedding).detach().cpu().view(-1)
                raw_delta = torch.zeros_like(base_log)
                bounded_learned_delta = torch.zeros_like(base_log)
                family_offset_delta = torch.zeros_like(base_log)
                delta_log = torch.zeros_like(base_log)
                if getattr(model_core, "use_bounded_process_residual", False) and proc_ctx is not None:
                    dev = trainer.device
                    avail_t = proc_ctx["avail"].to(dev)
                    num_t = proc_ctx["num"].to(dev)
                    num_norm = (num_t - model_core.proc_encoder.num_mean.to(dev)) / model_core.proc_encoder.num_std.to(dev)
                    num_norm = num_norm * avail_t
                    if model_core.process_delta_use_categorical:
                        cat_t = proc_ctx["cat"].to(dev)
                        cat_embs = [
                            model_core.proc_encoder.embeddings[name](cat_t[:, i])
                            for i, name in enumerate(model_core.proc_encoder._cat_cols)
                        ]
                        x_delta = torch.cat([num_norm] + cat_embs, dim=-1)
                    else:
                        n_extra = model_core.process_delta_head[0].in_features - num_norm.shape[-1]
                        x_delta = torch.cat([
                            num_norm,
                            torch.zeros(num_norm.shape[0], max(n_extra, 0), device=dev),
                        ], dim=-1) if n_extra > 0 else num_norm
                    raw_t = model_core.process_delta_head(x_delta)
                    bounded_t = model_core.process_delta_bound * torch.tanh(raw_t) * avail_t
                    family_t = proc_ctx.get("family_offset", torch.zeros_like(avail_t)).to(dev)
                    raw_delta = raw_t.detach().cpu().view(-1)
                    bounded_learned_delta = bounded_t.detach().cpu().view(-1)
                    family_offset_delta = family_t.detach().cpu().view(-1)
                    delta_log = bounded_learned_delta + family_offset_delta
                pred_log = base_log + delta_log

                row_indices = batch.get("row_indices")
                if row_indices is None:
                    row_indices = torch.arange(len(pred_log))
                row_indices = row_indices.detach().cpu().view(-1).tolist()

                proc_avail = None
                proc_num_cpu = None
                proc_cat_cpu = None
                if proc_ctx_original is not None and "avail" in proc_ctx_original:
                    proc_avail = proc_ctx_original["avail"].detach().cpu().view(-1).tolist()
                    proc_num_cpu = proc_ctx_original["num"].detach().cpu()
                    proc_cat_cpu = proc_ctx_original["cat"].detach().cpu()

                for j, row_idx in enumerate(row_indices):
                    if row_idx not in df_ref.index:
                        continue
                    meta = df_ref.loc[row_idx]
                    true_log = float(target[j].item()) if j < len(target) and not torch.isnan(target[j]) else float("nan")
                    pred_l = float(pred_log[j].item())
                    base_l = float(base_log[j].item())
                    delta_l = float(delta_log[j].item())
                    if cfg.get("log_transform", False):
                        k_true = float(math.exp(true_log)) if np.isfinite(true_log) else float("nan")
                        k_pred = float(math.exp(pred_l)) if np.isfinite(pred_l) else float("nan")
                        k_dft  = float(math.exp(base_l)) if np.isfinite(base_l) else float("nan")
                    else:
                        k_true, k_pred, k_dft = true_log, pred_l, base_l
                    abs_error = abs(k_pred - k_true) if np.isfinite(k_true) and np.isfinite(k_pred) else float("nan")
                    log_error = abs(pred_l - true_log) if np.isfinite(true_log) and np.isfinite(pred_l) else float("nan")

                    rows.append({
                        "stage": stage_name,
                        "split": split_name,
                        "dataset_row_idx": int(row_idx),
                        "split_identity": _tier3_split_key_from_row(meta),
                        "row_id": _row_val(meta, "row_id", default=""),
                        "paper_id": _row_val(meta, "paper_id", default=""),
                        "doi": _row_val(meta, "doi", default=""),
                        "data_quality": _row_val(meta, "data_quality", default=""),
                        "source": _row_val(meta, "source", default=""),
                        "material": _row_val(meta, "material", "material_system", default=""),
                        "material_family": _row_val(meta, "material_family", default=""),
                        "material_family_offset_log": _row_val(meta, "material_family_offset_log", default=material_family_log_offset_from_row(meta)),
                        "formula": _row_val(meta, "formula", "composition", default=""),
                        "k_true": k_true,
                        "k_pred": k_pred,
                        "k_dft": k_dft,
                        "raw_process_delta": float(raw_delta[j].item()) if j < len(raw_delta) else 0.0,
                        "bounded_process_delta": float(bounded_learned_delta[j].item()) if j < len(bounded_learned_delta) else 0.0,
                        "family_offset_delta": float(family_offset_delta[j].item()) if j < len(family_offset_delta) else 0.0,
                        "delta_log_pred": delta_l,
                        "delta_k_pred": (k_pred - k_dft) if np.isfinite(k_pred) and np.isfinite(k_dft) else float("nan"),
                        "abs_error": abs_error,
                        "log_error": log_error,
                        "k_true_log": true_log,
                        "k_pred_log": pred_l,
                        "k_dft_log": base_l,
                        "prediction_decomposition_error": (
                            pred_l - (base_l + delta_l)
                            if np.isfinite(pred_l) and np.isfinite(base_l) and np.isfinite(delta_l)
                            else float("nan")
                        ),
                        "proc_avail": proc_avail[j] if proc_avail is not None and j < len(proc_avail) else float("nan"),
                        "process_numeric_feature_count": int(torch.count_nonzero(proc_num_cpu[j]).item()) if proc_num_cpu is not None and j < len(proc_num_cpu) else 0,
                        "process_categorical_feature_count": int(torch.count_nonzero(proc_cat_cpu[j] >= 0).item()) if proc_cat_cpu is not None and j < len(proc_cat_cpu) else 0,
                        "process_feature_count": (
                            int(torch.count_nonzero(proc_num_cpu[j]).item()) + int(torch.count_nonzero(proc_cat_cpu[j] >= 0).item())
                            if proc_num_cpu is not None and proc_cat_cpu is not None and j < len(proc_num_cpu) else 0
                        ),
                        "process_feature_mask": (
                            "|".join(str(int(abs(float(v)) > 0.0)) for v in proc_num_cpu[j].tolist())
                            if proc_num_cpu is not None and j < len(proc_num_cpu) else ""
                        ),
                        "is_experimental": int(str(_row_val(meta, "source", default="")) == "Experimental"),
                        "imputed_structure": bool(_row_val(meta, "imputed_structure", default=False)),
                        "has_structure": bool(_row_val(meta, "has_structure", default=False)),
                        "donor_id": _row_val(meta, "imputed_from", "jid", "mp_id", default=""),
                        "donor_formula": _row_val(meta, "imputed_donor_formula", default=""),
                        "donor_source": _row_val(meta, "imputed_donor_source", default=""),
                        "donor_strategy": _row_val(meta, "imputed_donor_strategy", default=""),
                        "donor_phase": _row_val(meta, "imputed_phase_hint", default=""),
                        "donor_score": _row_val(meta, "imputed_donor_score", default=""),
                        "candidate_formulas": _row_val(meta, "imputed_candidate_formulas", default=""),
                        "substrate_temp_C": _row_val(meta, "substrate_temp_C", "ald_substrate_temp_C", default=""),
                        "precursor_type": _row_val(meta, "precursor_type", "precursor_type_Hf", default=""),
                        "oxidant_type": _row_val(meta, "oxidant_type", default=""),
                        "n_cycles": _row_val(meta, "n_cycles", "ald_cycle_count", default=""),
                        "film_thickness_A": _row_val(meta, "film_thickness_A", default=""),
                        "film_thickness_nm": _row_val(meta, "film_thickness_nm", default=""),
                        "anneal_temp_C": _row_val(meta, "anneal_temp_C", "anneal_temp_PDA_C", default=""),
                        "anneal_ambient": _row_val(meta, "anneal_ambient", "anneal_atmosphere", default=""),
                        "anneal_duration_min": _row_val(meta, "anneal_duration_min", "anneal_duration", default=""),
                        "substrate_type": _row_val(meta, "substrate_type", "substrate", default=""),
                        "stack_structure": _row_val(meta, "stack_structure", "stack_config", default=""),
                    })

        audit_df = pd.DataFrame(rows)
        if safe_split in ("test", "val"):
            # DISJOINT-VAL: VAL is a distinct held-out population from TEST.
            _expected_audit_n = _canonical_val_n if safe_split == "val" else _canonical_test_n
            if len(audit_df) != _expected_audit_n:
                raise RuntimeError(
                    f"v4.41 prediction audit count mismatch {safe_stage}/{safe_split}: "
                    f"{len(audit_df)} != {_expected_audit_n}"
                )
            if "split_identity" in audit_df.columns and audit_df["split_identity"].duplicated().any():
                dups = int(audit_df["split_identity"].duplicated().sum())
                raise RuntimeError(
                    f"v4.41 prediction audit has {dups} duplicate split keys at {safe_stage}/{safe_split}"
                )
        pred_path = audit_dir / f"tier3_{safe_stage}_{safe_split}_prediction_audit.csv"
        top_path  = audit_dir / f"tier3_{safe_stage}_{safe_split}_top{top_n}_errors.csv"
        mat_path  = audit_dir / f"tier3_{safe_stage}_{safe_split}_material_statistics.csv"
        donor_path = audit_dir / f"tier3_{safe_stage}_{safe_split}_donor_statistics.csv"
        resid_path = audit_dir / f"tier3_{safe_stage}_{safe_split}_residual_statistics.csv"

        if len(audit_df) == 0:
            audit_df.to_csv(pred_path, index=False)
            log.warning("Tier3 audit %s/%s: no rows written → %s", stage_name, split_name, pred_path)
            return pred_path

        audit_df = audit_df.sort_values("abs_error", ascending=False, na_position="last")
        audit_df.to_csv(pred_path, index=False)
        audit_df.head(top_n).to_csv(top_path, index=False)

        # Material-level failure statistics.
        if "material" in audit_df.columns:
            mat_stats = (
                audit_df.groupby(["material", "material_family"], dropna=False)
                .agg(
                    n=("abs_error", "count"),
                    mean_abs_error=("abs_error", "mean"),
                    std_abs_error=("abs_error", "std"),
                    max_abs_error=("abs_error", "max"),
                    mean_k_true=("k_true", "mean"),
                    mean_k_pred=("k_pred", "mean"),
                    mean_delta_log=("delta_log_pred", "mean"),
                )
                .reset_index()
                .sort_values(["mean_abs_error", "n"], ascending=[False, False])
            )
            mat_stats.to_csv(mat_path, index=False)

        # Donor-level failure statistics.
        donor_keys = ["donor_formula", "donor_phase", "donor_strategy", "donor_source"]
        if all(k in audit_df.columns for k in donor_keys):
            donor_stats = (
                audit_df.groupby(donor_keys, dropna=False)
                .agg(
                    n=("abs_error", "count"),
                    mean_abs_error=("abs_error", "mean"),
                    std_abs_error=("abs_error", "std"),
            ...
            donor_stats.to_csv(donor_path, index=False)

        # One-row residual summary for quick sanity checking.
        resid_summary = pd.DataFrame([{
            "stage": stage_name,
            "split": split_name,
            "n": int(len(audit_df)),
            "mean_abs_error": float(audit_df["abs_error"].mean()),
            "median_abs_error": float(audit_df["abs_error"].median()),
            "mean_log_error": float(audit_df["log_error"].mean()),
            "mean_k_true": float(audit_df["k_true"].mean()),
            "mean_k_pred": float(audit_df["k_pred"].mean()),
            "mean_k_dft": float(audit_df["k_dft"].mean()),
            "mean_delta_log_pred": float(audit_df["delta_log_pred"].mean()),
            "std_delta_log_pred": float(audit_df["delta_log_pred"].std()),
            "mean_delta_k_pred": float(audit_df["delta_k_pred"].mean()),
            "max_abs_error": float(audit_df["abs_error"].max()),
        }])
        resid_summary.to_csv(resid_path, index=False)

        # v4.40 AUDIT: subgroup metrics expose whether process context helps the
        # experimental/imputed/proc-available rows even when aggregate metrics do not move.
        subgroup_rows = []
        subgroup_defs = {
            "all": pd.Series(True, index=audit_df.index),
            "experimental": audit_df["is_experimental"].astype(bool),
            "native_dft": ~audit_df["is_experimental"].astype(bool),
            "imputed_experimental": audit_df["is_experimental"].astype(bool) & audit_df["imputed_structure"].astype(bool),
            "proc_available": pd.to_numeric(audit_df["proc_avail"], errors="coerce").fillna(0).gt(0),
            "proc_unavailable": pd.to_numeric(audit_df["proc_avail"], errors="coerce").fillna(0).le(0),
            "experimental_proc_available": audit_df["is_experimental"].astype(bool) & pd.to_numeric(audit_df["proc_avail"], errors="coerce").fillna(0).gt(0),
        }
        for subgroup, mask in subgroup_defs.items():
            g = audit_df[mask]
            if len(g) == 0:
                continue
            log_err = pd.to_numeric(g["log_error"], errors="coerce").dropna()
            abs_err = pd.to_numeric(g["abs_error"], errors="coerce").dropna()
            subgroup_rows.append({
                "stage": stage_name, "split": split_name, "subgroup": subgroup,
                "n": int(len(g)),
                "log_mae": float(log_err.mean()) if len(log_err) else float("nan"),
                "linear_mae": float(abs_err.mean()) if len(abs_err) else float("nan"),
                "linear_rmse": float(np.sqrt(np.mean(np.square(pd.to_numeric(g["k_pred"], errors="coerce") - pd.to_numeric(g["k_true"], errors="coerce"))))),
                "mean_raw_delta": float(pd.to_numeric(g["raw_process_delta"], errors="coerce").mean()),
                "mean_abs_bounded_delta": float(pd.to_numeric(g["bounded_process_delta"], errors="coerce").abs().mean()),
                "mean_process_feature_count": float(pd.to_numeric(g["process_feature_count"], errors="coerce").mean()),
            })
        subgroup_path = audit_dir / f"tier3_{safe_stage}_{safe_split}_subgroup_metrics.csv"
        pd.DataFrame(subgroup_rows).to_csv(subgroup_path, index=False)

        log.info(
            "Tier3 audit written [%s/%s]: rows=%d  pred=%s  top=%s  material=%s  donor=%s  residual=%s",
            stage_name, split_name, len(audit_df), pred_path, top_path, mat_path, donor_path, resid_path,
        )
        return pred_path

    def _write_tier3_dataset_composition_audit():
        if not is_rank0() or not cfg.get("audit_write_dataset_composition", False):
            return
        audit_dir = REPORT_ROOT / "tier3_prediction_audit" / ("ablation" if trainer.ablate_context else "normal")
        audit_dir.mkdir(parents=True, exist_ok=True)
        df_a = _main_ds_t3.df.copy()
        # Map dataset positions to persistent split labels.
        split_map = {}
        for label, subset in [("train", train_loader.dataset), ("val", val_loader.dataset), ("test", test_loader.dataset)]:
            for pos in getattr(subset, "indices", []):
                if 0 <= int(pos) < len(_main_ds_t3.valid_idx):
                    split_map[int(_main_ds_t3.valid_idx[int(pos)])] = label
        rows = []
        proc_cols = list(PROCESS_PARAMS_FEATURES["numerical"]) + list(PROCESS_PARAMS_FEATURES["categorical"].keys())
        for idx, r in df_a.iterrows():
            count = sum(0 if pd.isna(r.get(c, np.nan)) else 1 for c in proc_cols)
            rows.append({
                "dataset_row_idx": int(idx),
                "row_id": r.get("row_id", ""),
                "source": r.get("source", ""),
                "material": r.get("material", r.get("material_system", "")),
                "formula": r.get("formula", ""),
                "material_family": infer_material_family_from_row(r),
                "is_experimental": int(str(r.get("source", "")) == "Experimental"),
                "imputed_structure": bool(r.get("imputed_structure", False)),
                "imputed_from": r.get("imputed_from", ""),
                "has_structure": bool(r.get("has_structure", False)),
                "target_valid": bool(pd.notna(r.get(cfg["target"], np.nan))),
                "process_feature_count": int(count),
                "proc_avail": int(count > 0),
                "split": split_map.get(int(idx), "excluded"),
            })
        comp = pd.DataFrame(rows)
        comp_path = audit_dir / "tier3_dataset_composition.csv"
        comp.to_csv(comp_path, index=False)
        summary = comp.groupby(["split", "source", "imputed_structure", "proc_avail"], dropna=False).size().reset_index(name="n")
        summary.to_csv(audit_dir / "tier3_dataset_composition_summary.csv", index=False)
        observed_test = int((comp["split"] == "test").sum())
        observed_val = int((comp["split"] == "val").sum())
        if observed_test != _canonical_test_n:
            raise RuntimeError(
                f"v4.42 composition TEST count {observed_test} != canonical {_canonical_test_n}"
            )
        if _tier3_disjoint_val_active:
            # DISJOINT-VAL: VAL is a separate population carved from TRAIN. It must
            # appear in the composition with exactly _canonical_val_n rows and must
            # not overlap TEST (split_map assigns one label per row; disjointness
            # was already asserted at split-construction time).
            if observed_val != _canonical_val_n:
                raise RuntimeError(
                    f"DISJOINT-VAL composition VAL count {observed_val} != canonical {_canonical_val_n}"
                )
            log.info(
                "DISJOINT-VAL AUDIT dataset composition written: %s (train-disjoint val=%d, test=%d)",
                comp_path, observed_val, observed_test,
            )
        else:
            # Legacy: VAL and TEST reuse the same rows; TEST label takes precedence.
            log.info("v4.41 AUDIT dataset composition written: %s (test=%d)", comp_path, observed_test)

    _write_tier3_dataset_composition_audit()
    ...
    diag_epoch0_test = None
    _write_tier3_prediction_audit(val_loader, _main_ds_t3.df, "VAL", "epoch0_pretrain")
    log.info("BASELINE TEST-SEAL: TEST skipped at epoch 0; selection diagnostics use VAL only.")

    # v4.29: make epoch-0 a real candidate checkpoint. In frozen-DFT-base
    # residual fitting, the safest model is often the loaded Tier-2 DFT baseline.
    # Older versions only started best-checkpoint selection after epoch 1, so a
    # harmful first residual update could overwrite a better epoch-0 model.
    if cfg.get("bounded_process_residual", False) and np.isfinite(diag_epoch0_val.get("mae_log", float("nan"))):
        trainer.best_val_mae = float(diag_epoch0_val["mae_log"])
        trainer.best_epoch = 0
        trainer.patience_ctr = 0
        if is_rank0():
            trainer.save_checkpoint(0, trainer.best_val_mae, tag="best")
            trainer.save_checkpoint(0, trainer.best_val_mae, tag="epoch0")
        log.info(
            "v4.29: epoch-0 DFT baseline registered as best candidate "
            "(val_log_MAE=%.4f). Tier3 residual must improve over this to replace it.",
    ...
    log.info(
        "v4.38 PROCESS-RESIDUAL EXPERIMENT | epochs=%d lr=%.2e bound=%.3f "
        "categorical=%s family_offset=%s train_base_head=%s",
        cfg.get("epochs", 0), cfg.get("learning_rate", 0.0),
        cfg.get("process_delta_bound", 0.0),
        cfg.get("process_delta_use_categorical", False),
        cfg.get("enable_material_family_offset", False),
        cfg.get("process_delta_train_base_head", False),
    )
    ...
    if freeze_backbone:
        if residual_proc_mode:
            ...
            trainer.rebuild_optimizer_for_current_trainables()
            trainer.capture_frozen_base_reference()
        else:
        ...
        if _tier3_disjoint_val_active:
            # DISJOINT-VAL: reuse the already-carved disjoint TRAIN/VAL loaders so
            # checkpoint selection uses VAL (never TEST). Tier-3 aux_targets is []
            # so the module train_loader is already single-task.
            pb_loader      = train_loader
            pb_val_loader  = val_loader
            pb_test_loader = test_loader
            pb_sampler     = train_sampler
            log.info(
                "DISJOINT-VAL Phase B: reusing carved TRAIN(%d)/VAL(%d); TEST(%d) untouched.",
                len(getattr(pb_loader.dataset, "indices", [])),
                len(getattr(pb_val_loader.dataset, "indices", [])),
                _canonical_test_n,
            )
        elif cfg.get("eval_hfo2_family_only", False):
            pb_loader, pb_val_loader, pb_test_loader, pb_sampler, _ds = _tier3_loader_fn(
                df         = df_phase_b,
                target_col = cfg["target"],
                aux_cols   = [],
                train_frac = cfg["train_ratio"],
        ...
        # DISJOINT-VAL: validation population depends on mode.
        _pb_expected_val_n = _canonical_val_n if _tier3_disjoint_val_active else _canonical_test_n
        if len(getattr(pb_val_loader.dataset, "indices", [])) != _pb_expected_val_n:
            raise RuntimeError(
                "Phase-B validation split size %d differs from expected %s split size %d"
                % (
                    len(getattr(pb_val_loader.dataset, "indices", [])),
                    "disjoint-VAL" if _tier3_disjoint_val_active else "TEST",
                    _pb_expected_val_n,
                )
            )

        # v4.56 optimizer was rebuilt immediately after residual freezing with
        # independent process/base-head learning rates. Do not replace it here.
        _saved_optimizer = None
        ...
        # v4.40 AUDIT: exact row-level audits for configured trajectory checkpoints.
        audit_epochs = sorted(set(int(e) for e in cfg.get("audit_checkpoint_epochs", [1, 10])))
        for audit_epoch in audit_epochs:
            candidates = [
                CKPT_ROOT / f"tier3_audit_ep{audit_epoch}.pt",
                CKPT_ROOT / f"tier3_epoch{audit_epoch}.pt",
            ]
            audit_ckpt = next((x for x in candidates if x.exists()), None)
            if audit_ckpt is None:
                log.warning("v4.40 AUDIT: checkpoint for epoch %d not found", audit_epoch)
                continue
            ckpt_a = safe_load_checkpoint(audit_ckpt)
            if ckpt_a is None:
                continue
            state_a = _remap_state_dict(ckpt_a["model_state_dict"])
            missing_a, unexpected_a = trainer.model_core.load_state_dict(state_a, strict=False)
            stage_a = f"epoch{audit_epoch}_ckpt"
            log.info(
                "v4.40 AUDIT: reloaded %s (missing=%d unexpected=%d)",
                audit_ckpt.name, len(missing_a), len(unexpected_a),
            )
            _eval_exact_k_for_diag(val_loader, "VAL", stage_a)
            _write_tier3_prediction_audit(val_loader, _main_ds_t3.df, "VAL", stage_a)
            log.info("BASELINE TEST-SEAL: TEST skipped for %s; checkpoint audit is VAL-only.", stage_a)
        ...
        if residual_proc_mode:
            ...
            pb2_loader, pb2_val_loader, _, pb2_sampler, _ds = _tier3_loader_fn(
                df         = df_phase_b,
                target_col = cfg["target"],
    ...
    # v4.37 FINAL-SPLIT-RELOAD:
    # The normal tier3_finetune run creates/overwrites the persistent Tier-3 TEST
    # split near the beginning of the run.  Before printing final metrics, reload
    # that same saved split and rebuild the TEST loader exactly the same way as
    # tier3_evaluate / --ablate_context.  This guarantees the final finetune
    # table and tier3_evaluate table report the same N_Valid and the same rows,
    # even if an earlier in-memory loader was built before split persistence.
    if cfg.get("eval_hfo2_family_only", False) and TIER3_TEST_SPLIT_PATH.exists():
        try:
            _, _, test_loader, _, _main_ds_t3 = build_tier3_hfo2_eval_dataloader(
                df         = df_phase_b,
                target_col = cfg["target"],
                aux_cols   = cfg["aux_targets"],
                train_frac = cfg["train_ratio"],
                ...
                save_test_split = False,
                overwrite_saved_split = False,
                use_saved_val_split = True,
                test_only = True,
            )
            _final_n = len(getattr(test_loader.dataset, "indices", []))
            if _final_n != _canonical_test_n:
                raise RuntimeError(
                    f"v4.41 final TEST count {_final_n} != canonical {_canonical_test_n}"
                )
            log.info(
                "v4.41 FINAL-SPLIT-RELOAD: exact canonical TEST loader rebuilt; rows=%d batches=%d",
                _final_n, len(test_loader),
            )
        except Exception as exc:
            log.warning(
                "v4.37 FINAL-SPLIT-RELOAD failed; using existing in-memory TEST loader: %s",
                exc,
            )
    ...
    _write_tier3_prediction_audit(test_loader, _main_ds_t3.df, "TEST", "best_checkpoint")
    log.info("BASELINE TEST-UNSEAL: TEST evaluated once after tier3_best.pt was frozen.")

    log.info("─" * 68)
    _adapter_test_diag = trainer.evaluate_adapter_diagnostics(test_loader, cfg["target"], split_name="TEST-BEST")
    ...
    if diag_epoch0_test is not None:
        log.info(
        ...
        # v4.60.1: TEST is sealed during optimization (epoch-0 TEST intentionally
        # skipped). diag_epoch0_test is None here; do not dereference it.
        log.info("  epoch0 TEST: sealed (skipped during optimization; reported once post-freeze)")
    ...
    # v4.48 T3-METRIC-CONSISTENCY: use the exact linear-k primary metric
    # already computed from the fixed TEST prediction tensors. This makes the
    # ALL TASK HEADS table, RESIDUAL-PROC-DIAG, JSON, and audit CSV agree.
    _t3_primary = cfg["target"]
    if _t3_primary in mt_results_t3:
        mt_results_t3[_t3_primary].update({
            "mae_log": float(test_mae_log),
            "rmse_log": float(test_rmse_log),
            "mae_linear": float(mae_k_linear),
            "rmse_linear": float(rmse_k_linear),
            "mad_linear": float(mad_k_linear),
            "mad_mae_linear": float(mad_mae_linear),
            "mae": float(mae_k_linear),
            "rmse": float(rmse_k_linear),
            "mad": float(mad_k_linear),
            "mad_mae_ratio": float(mad_mae_linear),
            "n": int(n_valid),
            "coverage_pct": 100.0,
            "mad_source": "same fixed TEST rows as RESIDUAL-PROC-DIAG",
            "metric_scale": "linear-k exact",
            "display_name": "k_total",
        })
...
def run_tier_evaluate(
    ...
    if tier == 1:
        ...
        # v4.36 EVAL-IMPUTE-SPLITFIX:
        # tier3_finetune builds its Phase-B/eval split AFTER in-memory structure
        # imputation. build_tier3() writes the pre-imputation Tier-3 cache, so a
        # later tier3_evaluate that simply reads tier3_project.h5 drops the
        # process-only experimental rows before split matching. Result: a fresh
        # tier3_finetune can save 37 TEST rows, but tier3_evaluate matches only
        # the few original structural rows (e.g. 7/37) and falls back to a new
        # split with N Valid ~= 9.
        #
        # Fix: repeat the same imputation here before k-valid filtering and
        # before loading the persistent split. This makes tier3_evaluate see the
        # same valid_rows universe as tier3_finetune.
        if "atoms_dict" in df_eval.columns:
            _df_structural_eval = df_eval[df_eval["atoms_dict"].notna()].copy()
            _df_process_only_eval = df_eval[df_eval["atoms_dict"].isna()].copy()
            if len(_df_process_only_eval) > 0:
                _tier2_path = DATA_ROOT / "tier2_domain.h5"
                if _tier2_path.exists():
                    try:
                        _df_tier2_eval = pd.read_hdf(_tier2_path, key="data")
                        _df_donor_pool_wide = _df_tier2_eval[
                            _df_tier2_eval["formula"].apply(
                                lambda f: isinstance(f, str) and "O" in f and
                                          any(el in f for el in HIGH_K_DONOR_ELEMENTS)
                            )
                        ].copy()
                        ...
                        log.warning(
                            "v4.36 EVAL-IMPUTE: failed to load Tier2 donor pool from %s: %s; using structural fallback",
                            _tier2_path, exc,
                        )
                        _df_donor_pool_wide = _df_structural_eval
                else:
                    log.warning(
                        "v4.36 EVAL-IMPUTE: Tier2 donor pool not found at %s; using structural fallback",
                        _tier2_path,
                    )
                    _df_donor_pool_wide = _df_structural_eval

                _df_imputed_eval, _df_unmatched_eval = _impute_structures(
                    _df_process_only_eval, _df_donor_pool_wide
                )
                if len(_df_imputed_eval) > 0:
                    df_eval = pd.concat(
                        [_df_structural_eval, _df_imputed_eval],
                        ignore_index=True, sort=False,
                    )
                    log.info(
                        "v4.36 EVAL-IMPUTE: tier3_evaluate dataframe after imputation rows=%d "
                        "(original structural=%d imputed_experimental=%d unmatched=%d)",
                        len(df_eval), len(_df_structural_eval),
                        len(_df_imputed_eval), len(_df_unmatched_eval),
                    )
                else:
                    df_eval = _df_structural_eval.copy()
                    log.warning(
                        "v4.36 EVAL-IMPUTE: no process-only rows imputed; evaluating structural rows only"
                    )
    ...
    # ── Rebuild/load the exact same test split ───────────────────────────────
    if tier == 3 and cfg.get("eval_hfo2_family_only", False):
        _, _, test_loader, _, test_dataset = build_tier3_hfo2_eval_dataloader(
            df         = df_eval,
            target_col = target_col,
        ...
        log.info("Test loader: %d batches  (BASELINE locked persistent TEST-only split)", len(test_loader))
    else:
    ...
    # v4.58 RELEASE: read runtime architecture metadata before constructing the
    # adapter. This prevents a non-default-rank checkpoint from silently losing
    # adapter tensors through shape-compatible loading.
    ckpt = torch.load(checkpoint_path, map_location="cpu")
    saved_epoch = ckpt.get("epoch", "?")
    saved_mae   = ckpt.get("val_mae", float("nan"))
    if tier == 3:
        _saved_runtime = ckpt.get("adapter_runtime", {}) or {}
        _saved_cfg = ckpt.get("config", {}) or {}
        _adapter_enabled = bool(_saved_runtime.get(
            "enabled", _saved_cfg.get("tier3_use_structural_adapter", cfg.get("tier3_use_structural_adapter", True))))
        _adapter_rank = int(_saved_runtime.get(
            "rank", _saved_cfg.get("tier3_structural_adapter_rank", cfg.get("tier3_structural_adapter_rank", 16))))
        model.configure_structural_adapter(
            enabled=_adapter_enabled,
            rank=_adapter_rank,
            reset=False,
        )
        log.info("v4.58 EVAL-ADAPTER-CONFIG: restored enabled=%s rank=%d from checkpoint metadata",
                 _adapter_enabled, _adapter_rank)

    # CKPT-EVAL-COMPAT: strict=False still raises on same-name tensors with
    # different shapes. Use shape-aware loading for categorical vocab growth.
    missing, unexpected = _load_state_dict_shape_compatible(
        model, ckpt.get("model_state_dict", ckpt),
        checkpoint_label=str(checkpoint_path),
    )
    log.info(
        "Loaded checkpoint: epoch=%s  saved_val_MAE=%.4f  missing=%d unexpected=%d",
        saved_epoch, saved_mae, len(missing), len(unexpected),
    )
    if tier == 3 and _adapter_enabled:
        _adapter_missing = [k for k in missing if k.startswith("tier3_structural_adapter.")]
        _state = ckpt.get("model_state_dict", ckpt)
        _adapter_saved = [k for k in _state if k.startswith("tier3_structural_adapter.")]
        if not _adapter_saved:
            raise RuntimeError(
                "v4.58 EVAL CHECKPOINT ERROR: structural adapter is enabled but the checkpoint "
                "contains no tier3_structural_adapter tensors. Refusing to report a process-only result."
            )
        if _adapter_missing:
            raise RuntimeError(
                "v4.58 EVAL CHECKPOINT ERROR: adapter tensors were not restored: " + str(_adapter_missing)
            )

    # v4.50 T3-EVAL-RESIDUAL-RESTORE:
    # configure_bounded_process_residual() controls Python attributes and is not
    # represented in state_dict.  tier3_finetune enables this path before
    # training, but standalone tier3_evaluate previously only loaded tensors.
    # Consequently, the learned process_delta_head weights were present while
    # model.use_bounded_process_residual remained False, so evaluation returned
    # the frozen/ablated baseline (for example MAD:MAE=1.15) instead of the
    # process-aware best-checkpoint result (for example MAD:MAE=1.44).
    # Restore the exact locked Tier-3 runtime configuration after checkpoint
    # loading and before constructing the trainer/evaluating any batch.
    if tier == 3:
        model.configure_structural_adapter(
            enabled=_adapter_enabled,
            rank=_adapter_rank,
        ...
        model.configure_bounded_process_residual(
            enabled=cfg.get("bounded_process_residual", False),
            bound=cfg.get("process_delta_bound", 0.15),
            use_categorical=cfg.get("process_delta_use_categorical", True),
        )
        log.info(
            "v4.50 T3-EVAL-RESIDUAL-RESTORE: enabled=%s bound=%.3f "
            "use_categorical=%s ablate_context=%s",
            bool(model.use_bounded_process_residual),
            float(model.process_delta_bound),
            bool(model.process_delta_use_categorical),
            bool(ablate_context),
        )
        if not ablate_context and not model.use_bounded_process_residual:
            raise RuntimeError(
                "tier3_evaluate requested a normal process-aware evaluation, "
                "but bounded_process_residual is disabled. Refusing to report "
                "an ablation metric as the Tier-3 checkpoint result."
            )
    ...
    if tier == 3:
        _adapter_eval_diag = trainer.evaluate_adapter_diagnostics(test_loader, target_col, split_name="TEST-EVALUATE")
    log.info("")
    log.info("─" * 68)
    ...
    if tier == 2 and cfg.get("log_transform", False):
        ...
        valid = ~torch.isnan(trues_t)
        if valid.sum() > 0:
           _p_lin = torch.exp(preds_t[valid])
           _t_lin = torch.exp(trues_t[valid])
           mae_k_exact = (_p_lin - _t_lin).abs().mean().item()
           rmse_k_exact = ((_p_lin - _t_lin) ** 2).mean().sqrt().item()
           _t_np = _t_lin.cpu().numpy()
           mad_k_exact = float(np.mean(np.abs(_t_np - np.mean(_t_np))))
           mad_mae_k_exact = mad_k_exact / mae_k_exact if mae_k_exact > 0 else float("nan")
           n_k_exact = int(valid.sum())
        else:
            mae_k_exact = rmse_k_exact = mad_k_exact = mad_mae_k_exact = float("nan")
            n_k_exact = 0
        log.info(" Diagnostic log-space MAE = %.4f [log(k) units]", test_mae_log)
        log.info(" Diagnostic log-space RMSE = %.4f ",test_rmse_log )
    ...
    if tier in (2, 3) and cfg.get("log_transform", False) and target_col in mt_results:
        # v4.51 DUAL-SCALE: both tiers use the exact same TEST tensors for log
        # and exp-space metrics.  MAD:MAE is always linear and test-row matched.
        if tier == 2:
            _true_k_np = torch.exp(trues_t[~torch.isnan(trues_t)]).cpu().numpy()
            mad_k_exact = float(np.mean(np.abs(_true_k_np - np.mean(_true_k_np)))) if len(_true_k_np) else float("nan")
            mad_mae_k_exact = mad_k_exact / mae_k_exact if mae_k_exact > 0 else float("nan")
            n_k_exact = int((~torch.isnan(trues_t)).sum().item())
        mt_results[target_col].update({
            "mae_log": float(test_mae_log),
            "rmse_log": float(test_rmse_log),
            "mae_linear": float(mae_k_exact),
            "rmse_linear": float(rmse_k_exact),
    ...
    if tier == 3 and target_col in mt_results and mt_results[target_col]["n"] > 0:
        km = mt_results[target_col]
        log.info(
            "k_total (linear exact)  MAD=%.2f  MAE=%.4f  MAD:MAE=%.2f  "
    ...
    if tier == 2 and cfg.get("log_transform", False):
        primary.update({"mae_linear_k": mae_k_exact,
                        ...
                        "mad_linear_k": mad_k_exact,
                        "mad_mae_linear": mad_mae_k_exact,
                        "n_test": n_k_exact,
                        "mae": mae_k_exact,
                        "rmse": rmse_k_exact,
                        "exp_mae_multiplier": math.exp(test_mae_log)})
...
# ==============================================================================
# TIER-1 EXTERNAL BENCHMARK EVALUATION (v4.60.3-ext2 dual report)
# ==============================================================================
# This path is deliberately isolated from all train/validation/test builders.
# It loads a frozen manifest, verifies its SHA-256, optionally removes structural
# overlaps with the Tier-1 training cache, loads the already-selected Tier-1
# checkpoint, runs inference under torch.inference_mode(), writes row-level
# predictions and bootstrap uncertainty, and exits. No optimizer, scheduler,
# checkpoint selection, calibration, or parameter update is created.

_EXTERNAL_TARGET_ALIASES = {
    "formation_energy_per_atom": ["formation_energy_per_atom", "target_ef", "formation_energy"],
...
def _external_first_existing(df: pd.DataFrame, names: List[str]) -> Optional[str]:
    return next((name for name in names if name in df.columns), None)


def _external_load_atoms(value: Any, manifest_dir: Path) -> Optional[str]:
    """Return a canonical JSON-encoded JARVIS Atoms dictionary.

    Accepted manifest representations:
      * atoms_dict: JSON string or Python-style dict string
      * structure_path/cif_path: CIF, POSCAR/CONTCAR, JSON JARVIS atoms file
    """
    ...
    if not text:
        return None
    # JSON payload directly embedded in the CSV.
    if text.startswith("{"):
        try:
    ...
    try:
        if path.suffix.lower() == ".json":
            ...
            if "atoms" in payload and isinstance(payload["atoms"], dict):
                payload = payload["atoms"]
            return json.dumps(payload)
        from pymatgen.core import Structure
        structure = Structure.from_file(str(path))
        jatoms = JarvisAtomsAdaptor.get_atoms(structure)
        return json.dumps(jatoms.to_dict())
    except Exception as exc:
        log.debug("External structure parse failed for %s: %s", path, exc)
        return None


def load_tier1_external_manifest(path: Path) -> pd.DataFrame:
    """Load and strictly normalize a frozen Tier-1 external benchmark manifest."""
    if not path.exists():
        raise FileNotFoundError(f"External manifest not found: {path}")
    if path.suffix.lower() in (".h5", ".hdf", ".hdf5"):
        raw = pd.read_hdf(path, key="data")
    elif path.suffix.lower() in (".json", ".jsonl"):
        raw = pd.read_json(path, lines=path.suffix.lower() == ".jsonl")
    else:
        raw = pd.read_csv(path)
    if raw.empty:
        raise ValueError("External benchmark manifest is empty")

    id_col = _external_first_existing(raw, [
        "benchmark_row_id", "external_row_id", "source_material_id",
        "material_id", "oqmd_id", "aflow_id", "row_id",
    ])
    if id_col is None:
        raise ValueError(
            "External manifest requires a stable identity column: "
            "benchmark_row_id/source_material_id/material_id/oqmd_id/aflow_id"
        )
    ids = raw[id_col].astype(str).str.strip()
    if ids.eq("").any() or ids.str.lower().isin(["nan", "none"]).any():
        raise ValueError("External manifest contains missing row identities")
    if ids.duplicated().any():
        dup = ids[ids.duplicated(keep=False)].head(10).tolist()
        raise ValueError(f"External manifest contains duplicate identities: {dup}")

    atoms_col = _external_first_existing(raw, ["atoms_dict", "structure_path", "cif_path"])
    if atoms_col is None:
        raise ValueError("External manifest requires atoms_dict, structure_path, or cif_path")

    df = pd.DataFrame(index=raw.index)
    df["benchmark_row_id"] = ids
    df["source_material_id"] = raw.get("source_material_id", raw[id_col]).astype(str)
    if "source_database" in raw.columns:
        df["source"] = raw["source_database"].astype(str)
    elif "source" in raw.columns:
        df["source"] = raw["source"].astype(str)
    else:
        df["source"] = "ExternalTier1"
    df["atoms_dict"] = [
        _external_load_atoms(v, path.parent) for v in raw[atoms_col].tolist()
    ]
    df["has_structure"] = df["atoms_dict"].notna()

    formulas = []
    for atoms_json in df["atoms_dict"]:
        try:
            formulas.append(JAtoms.from_dict(json.loads(atoms_json)).composition.reduced_formula)
        except Exception:
            formulas.append(None)
    supplied_formula = raw.get("formula", raw.get("reduced_formula", pd.Series(index=raw.index, dtype=object)))
    df["formula"] = [f if f else supplied_formula.iloc[i] for i, f in enumerate(formulas)]

    for canonical, aliases in _EXTERNAL_TARGET_ALIASES.items():
        src = _external_first_existing(raw, aliases)
        df[canonical] = pd.to_numeric(raw[src], errors="coerce") if src else np.nan

    # Tier-1 dielectric head is trained in natural log space.
    df["k_total_log"] = np.nan
    k_mask = df["k_total"].notna() & (df["k_total"] > 0)
    df.loc[k_mask, "k_total_log"] = np.log(df.loc[k_mask, "k_total"].clip(lower=0.1))

    df["functional_code"] = pd.to_numeric(
        raw.get("functional_code", pd.Series(1, index=raw.index)), errors="coerce"
    ).fillna(1).astype(int)
    df["dft_functional"] = raw["dft_functional"].astype(str) if "dft_functional" in raw.columns else "PBE"
    df["is_molecule"] = False
    df["tier"] = 1
    df["row_hash"] = [hashlib.md5(f"EXT_{x}".encode()).hexdigest()[:12] for x in ids]

    invalid_struct = ~df["has_structure"]
    if invalid_struct.any():
        examples = df.loc[invalid_struct, "benchmark_row_id"].head(10).tolist()
        raise ValueError(
            f"{int(invalid_struct.sum())} external rows have invalid/missing structures; "
            f"examples={examples}"
        )
    if not any(df[c].notna().any() for c in ["formation_energy_per_atom", "band_gap", "k_total"]):
        raise ValueError("External manifest has no valid Tier-1 targets")
    return df.reset_index(drop=True)


def _external_overlap_audit(
    external_df: pd.DataFrame,
    tier1_cache: Path,
    policy: str,
    output_dir: Path,
) -> pd.DataFrame:
    """StructureMatcher-based overlap audit, restricted to shared formula groups."""
    audit_rows = []
    if policy == "off":
        external_df = external_df.copy()
        external_df["is_strict_external"] = True
        return external_df
    if not tier1_cache.exists():
        raise FileNotFoundError(
            f"Tier-1 cache required for overlap audit but not found: {tier1_cache}"
        )
    train_df = pd.read_hdf(tier1_cache, key="data")
    train_df = train_df[
        train_df.get("atoms_dict", pd.Series(index=train_df.index, dtype=object)).notna()
    ].copy()
    from pymatgen.analysis.structure_matcher import StructureMatcher
    matcher = StructureMatcher(ltol=0.2, stol=0.3, angle_tol=5)
    train_groups = {str(k): g for k, g in train_df.groupby("formula", dropna=False)}
    overlap_ids = set()

    for _, erow in tqdm(external_df.iterrows(), total=len(external_df), desc="External overlap audit"):
        bid = str(erow["benchmark_row_id"])
        formula = str(erow.get("formula", ""))
        matched = None
        candidates = train_groups.get(formula)
        if candidates is not None:
            try:
                estr = TierDatasetBuilder._atoms_json_to_pymatgen(erow.get("atoms_dict"))
            except Exception:
                estr = None
            if estr is not None:
                for _, trow in candidates.iterrows():
                    tstr = TierDatasetBuilder._atoms_json_to_pymatgen(trow.get("atoms_dict"))
                    if tstr is None:
                        continue
                    try:
                        if matcher.fit(estr, tstr):
                            matched = str(trow.get("jid", trow.get("mp_id", trow.get("row_hash", "?"))))
                            break
                    except Exception:
                        continue
        is_overlap = matched is not None
        if is_overlap:
            overlap_ids.add(bid)
        audit_rows.append({
            "benchmark_row_id": bid,
            "formula": formula,
            "is_overlap": bool(is_overlap),
            "matched_training_id": matched or "",
        })

    audit = pd.DataFrame(audit_rows)
    output_dir.mkdir(parents=True, exist_ok=True)
    audit.to_csv(output_dir / "tier1_external_overlap_audit.csv", index=False)
    n_overlap = int(audit["is_overlap"].sum())
    log.info("External overlap audit: %d/%d structurally overlap Tier-1 cache", n_overlap, len(audit))
    if n_overlap and policy == "fail":
        raise RuntimeError(
            f"External benchmark contamination: {n_overlap} structural overlaps found. "
            "See tier1_external_overlap_audit.csv"
        )
    kept = external_df.copy()
    kept["is_strict_external"] = ~kept["benchmark_row_id"].isin(overlap_ids)
    if policy == "exclude":
        kept = kept[kept["is_strict_external"]].reset_index(drop=True)
        if kept.empty:
            raise RuntimeError("All external rows were removed as Tier-1 overlaps")
    return kept


def _external_metric_arrays(y_true: np.ndarray, y_pred: np.ndarray) -> Dict[str, float]:
    mask = np.isfinite(y_true) & np.isfinite(y_pred)
    y = y_true[mask].astype(float)
    p = y_pred[mask].astype(float)
    if len(y) == 0:
        return {"n": 0}
    err = p - y
    ae = np.abs(err)
    ...
    return {
        "n": int(len(y)), "mae": mae, "rmse": rmse,
        "median_ae": float(np.median(ae)), "p90_ae": float(np.percentile(ae, 90)),
        "mean_signed_error": float(err.mean()), "mad": mad,
        "mad_mae": float(mad / mae) if mae > 0 else float("inf"),
    }


def _external_bootstrap(y: np.ndarray, p: np.ndarray, n_boot: int, seed: int = 20260729) -> Dict[str, float]:
    ...
    if n < 2 or n_boot <= 0:
        return {}
    rng = np.random.default_rng(seed)
    mae_draws = np.empty(n_boot, dtype=float)
    ratio_draws = np.empty(n_boot, dtype=float)
    for i in range(n_boot):
        ...
        mae_draws[i] = mae
        ratio_draws[i] = mad / mae if mae > 0 else np.nan
    q = [2.5, 97.5]
    return {
        "mae_ci_low": float(np.nanpercentile(mae_draws, q[0])),
        "mae_ci_high": float(np.nanpercentile(mae_draws, q[1])),
        "mad_mae_ci_low": float(np.nanpercentile(ratio_draws, q[0])),
        "mad_mae_ci_high": float(np.nanpercentile(ratio_draws, q[1])),
    }


def run_tier1_external_evaluate(
    manifest_path: Path,
    checkpoint_path: Path,
    output_dir: Path,
    ...
    """Evaluate the frozen Tier-1 checkpoint on an immutable external suite."""
    if _DIST["active"] and int(_DIST["world"]) != 1:
        raise RuntimeError(
            "tier1_external_eval is intentionally single-process. Run with `python`, "
            "not multi-rank torchrun, to avoid duplicated benchmark rows."
        )
    output_dir.mkdir(parents=True, exist_ok=True)
    manifest_hash = _external_sha256(manifest_path)
    if expected_hash and manifest_hash.lower() != expected_hash.lower():
        raise RuntimeError(
            f"External manifest SHA-256 mismatch: expected={expected_hash}, actual={manifest_hash}"
        )
    log.info("TIER1-EXTERNAL manifest SHA-256: %s", manifest_hash)
    df_loaded = load_tier1_external_manifest(manifest_path)

    # T1-EXT-FLOW-AUDIT: preserve counts at every external-evaluation stage.
    # This makes reductions such as 2000 manifest rows -> 222 evaluated rows
    # directly attributable to overlap exclusion, target availability, graph
    # construction, or inference output rather than being hidden in final N.
    flow = {
        "manifest_rows_raw": int(len(df_loaded)),
        "structures_parsed_valid": int(df_loaded["has_structure"].fillna(False).sum()),
        "formation_energy_labels_manifest": int(df_loaded["formation_energy_per_atom"].notna().sum()),
        "band_gap_labels_manifest": int(df_loaded["band_gap"].notna().sum()),
        "k_total_labels_manifest": int(df_loaded["k_total"].notna().sum()),
    }

    # T1-EXT-DUAL-REPORT: always retain the fully annotated benchmark for one
    # inference pass, then report metrics for both populations:
    #   1) all_benchmark      -- all OQMD rows (cross-database transfer)
    #   2) strict_external    -- rows with no structural match in Tier-1 cache
    # The requested overlap policy still controls the primary/accepted subset
    # and fail/off behaviour, but no second GPU inference pass is required.
    audit_policy = "report" if overlap_policy in ("exclude", "report") else overlap_policy
    df = _external_overlap_audit(
        df_loaded, TierDatasetBuilder.TIER_PATHS[1], audit_policy, output_dir
    )
    strict_mask = df.get(
        "is_strict_external", pd.Series(True, index=df.index)
    ).fillna(False).astype(bool)
    n_strict = int(strict_mask.sum())
    n_overlap = int(len(df) - n_strict)
    primary_count = n_strict if overlap_policy == "exclude" else int(len(df))
    flow["rows_all_benchmark"] = int(len(df))
    flow["rows_overlap_matched"] = n_overlap
    flow["rows_strict_external"] = n_strict
    flow["rows_after_overlap_policy"] = primary_count
    flow["rows_removed_by_overlap_policy"] = (
        n_overlap if overlap_policy == "exclude" else 0
    )
    flow["strict_external_rows_after_policy"] = n_strict

    task_names = [TIER1_TRAIN_CONFIG["target"]] + TIER1_TRAIN_CONFIG["aux_targets"]
    dataset = HighKGraphDataset(
        df,
        target_col="formation_energy_per_atom",
        aux_cols=["band_gap", "k_total_log"],
    )
    # HighKGraphDataset normally filters on its primary target. External suites
    # may contain task-specific rows, so retain every valid structure that has
    # at least one Tier-1 label and route all labels from the normalized frame.
    valid_any = (
        df["has_structure"].fillna(False) & df["atoms_dict"].notna() &
        df[["formation_energy_per_atom", "band_gap", "k_total_log"]].notna().any(axis=1)
    )
    dataset.valid_idx = df[valid_any].index.tolist()
    flow["rows_with_any_tier1_label_after_overlap"] = int(valid_any.sum())
    flow["formation_energy_labels_after_overlap"] = int(df["formation_energy_per_atom"].notna().sum())
    flow["band_gap_labels_after_overlap"] = int(df["band_gap"].notna().sum())
    flow["k_total_log_labels_after_overlap"] = int(df["k_total_log"].notna().sum())
    flow["dataset_rows_submitted_to_dataloader"] = int(len(dataset.valid_idx))
    loader = DataLoader(
        dataset, batch_size=batch_size, shuffle=False, num_workers=num_workers,
        collate_fn=HighKGraphDataset.collate_fn, drop_last=False,
    )
    if len(dataset) == 0:
        raise RuntimeError("No evaluable external structures remain after validation/overlap policy")

    device = _DIST["device"] if _DIST["active"] else ("cuda" if torch.cuda.is_available() else "cpu")
    model = HighKALIGNN(n_output_tasks=len(task_names), task_names=task_names)
    ckpt = safe_load_checkpoint(checkpoint_path, map_location="cpu")
    if ckpt is None:
        raise RuntimeError(f"Could not load Tier-1 checkpoint: {checkpoint_path}")
    state = _remap_state_dict(ckpt["model_state_dict"])
    missing, unexpected = model.load_state_dict(state, strict=False)
    # All three Tier-1 heads must be present; unrelated later-tier modules may be absent.
    missing_heads = [k for k in missing if k.startswith("task_heads.") and any(f"task_heads.{h}." in k for h in task_names)]
    if missing_heads:
        raise RuntimeError(f"Checkpoint is missing required Tier-1 head tensors: {missing_heads[:10]}")
    log.info("External checkpoint load: missing=%d unexpected=%d", len(missing), len(unexpected))
    model = model.to(device)
    model.eval()

    pred_by_head = {h: {} for h in task_names}
    with torch.inference_mode():
        for batch in tqdm(loader, desc="Tier-1 external inference"):
            if batch is None:
                continue
            graph = batch["graph"].to(device)
            line_graph = batch["line_graph"].to(device)
            fc = batch.get("functional_code")
            if fc is not None:
                fc = fc.to(device)
            preds = model.forward_all_tasks(
                graph, line_graph, proc_context=None, stack_context=None,
                functional_code=fc,
            )
            row_indices = batch["row_indices"].tolist()
            for head, tensor in preds.items():
                vals = tensor.detach().cpu().reshape(-1).numpy()
                for ri, value in zip(row_indices, vals):
                    pred_by_head[head][int(ri)] = float(value)

    # Count rows for which graph construction + batching + model inference
    # succeeded. A row is considered inference-successful when at least one
    # required Tier-1 head produced a finite prediction for its row index.
    inferred_row_ids = set()
    for _head_map in pred_by_head.values():
        inferred_row_ids.update(int(x) for x in _head_map.keys())
    flow["rows_with_successful_graph_and_inference"] = int(len(inferred_row_ids))
    flow["rows_lost_during_graph_or_batch_inference"] = int(
        max(0, len(dataset.valid_idx) - len(inferred_row_ids))
    )
    for _head in task_names:
        flow[f"predictions_emitted_{_head}"] = int(len(pred_by_head.get(_head, {})))
    ...
    head_to_target = {
        "formation_energy_per_atom": "formation_energy_per_atom",
        "band_gap": "band_gap",
        "k_total_log": "k_total_log",
    }

    # First create one row-level audit over all successfully predicted rows.
    for head, target_col in head_to_target.items():
        for ri in dataset.valid_idx:
            y = pd.to_numeric(df.at[ri, target_col], errors="coerce")
            pval = pred_by_head.get(head, {}).get(int(ri), np.nan)
            if pd.isna(y) or not np.isfinite(pval):
                continue
            prediction_rows.append({
                "benchmark_row_id": df.at[ri, "benchmark_row_id"],
                "source_material_id": df.at[ri, "source_material_id"],
                "source": df.at[ri, "source"], "formula": df.at[ri, "formula"],
                "task": head, "y_true": float(y), "y_pred": float(pval),
                "signed_error": float(pval-y), "absolute_error": float(abs(pval-y)),
    ...
    for population, strict_only in populations:
        pop_df = pred_df if strict_only is None else pred_df[pred_df["is_strict_external"]]
        for head in head_to_target:
            hdf = pop_df[pop_df["task"] == head]
            yarr = hdf["y_true"].to_numpy(dtype=float) if len(hdf) else np.array([], dtype=float)
            parr = hdf["y_pred"].to_numpy(dtype=float) if len(hdf) else np.array([], dtype=float)
            stats = _external_metric_arrays(yarr, parr)
            boot = _external_bootstrap(
                yarr, parr, n_boot=n_boot,
                seed=20260729 + len(metric_rows) + (0 if population == "all_benchmark" else 1000),
            )
            metric_rows.append({
                "population": population,
                "task": head,
                "scale": "log" if head == "k_total_log" else "native",
                **stats, **boot,
            })
            flow[f"final_valid_pairs_{population}_{head}"] = int(stats.get("n", 0))
            if head == "k_total_log" and len(yarr):
                ylin, plin = np.exp(yarr), np.exp(parr)
                lstats = _external_metric_arrays(ylin, plin)
                lboot = _external_bootstrap(
                    ylin, plin, n_boot=n_boot,
                    seed=20260829 + (0 if population == "all_benchmark" else 1000),
                )
                metric_rows.append({
                    "population": population, "task": "k_total",
                    "scale": "linear", **lstats, **lboot,
                })

    metrics_df = pd.DataFrame(metric_rows)
    pred_df.to_csv(output_dir / "tier1_external_prediction_audit.csv", index=False)
    metrics_df.to_csv(output_dir / "tier1_external_metrics.csv", index=False)

    flow_rows = [{"stage": key, "count": int(value)} for key, value in flow.items()]
    flow_df = pd.DataFrame(flow_rows)
    flow_df.to_csv(output_dir / "tier1_external_dataset_flow_audit.csv", index=False)

    metadata = {
        "mode": "tier1_external_eval", "checkpoint": str(checkpoint_path),
        "manifest": str(manifest_path), "manifest_sha256": manifest_hash,
        "overlap_policy": overlap_policy,
        "n_manifest_raw": int(flow["manifest_rows_raw"]),
        "n_after_overlap_policy": int(flow["rows_after_overlap_policy"]),
        "n_all_benchmark": int(flow["rows_all_benchmark"]),
        "n_strict_external": int(flow["rows_strict_external"]),
        "dual_population_reporting": True,
        "n_evaluable_structures": int(len(dataset)), "n_boot": int(n_boot),
        "inference_only": True, "checkpoint_epoch": ckpt.get("epoch"),
        "dataset_flow": flow,
    }
    (output_dir / "tier1_external_metadata.json").write_text(json.dumps(metadata, indent=2))

    lines = [
        "# Tier-1 External Benchmark Evaluation", "",
        f"Manifest SHA-256: `{manifest_hash}`", "",
        "## Dataset Flow Audit", "",
        "| Stage | Count |", "|---|---:|",
    ]
    for _stage, _count in flow.items():
        lines.append(f"| {_stage} | {int(_count)} |")
    lines.extend([
        "", "## Metrics", "",
        "| Task | Scale | N | MAE | RMSE | MAD::MAE | 95% CI MAE | 95% CI MAD::MAE |",
        "|---|---:|---:|---:|---:|---:|---:|---:|",
    ])
    for _, r in metrics_df.iterrows():
        lines.append(
            f"| {r.get('population','')} | {r['task']} | {r['scale']} | {int(r.get('n',0))} | {r.get('mae',np.nan):.6f} | "
            f"{r.get('rmse',np.nan):.6f} | {r.get('mad_mae',np.nan):.4f} | "
            f"[{r.get('mae_ci_low',np.nan):.6f}, {r.get('mae_ci_high',np.nan):.6f}] | "
            f"[{r.get('mad_mae_ci_low',np.nan):.4f}, {r.get('mad_mae_ci_high',np.nan):.4f}] |"
        )
    (output_dir / "tier1_external_summary.md").write_text("\n".join(lines) + "\n")
    log.info("TIER1-EXTERNAL DATASET FLOW AUDIT:\n%s", flow_df.to_string(index=False))
    log.info("TIER1-EXTERNAL complete: %s", output_dir)
...
def main():
    ...
    parser.add_argument(
        "--eval_all_tier3_materials", action="store_true",
        help=(
            "Disable the default Tier-3 target-family evaluation split. By default, "
            "Tier-3 trains on the full expanded DB but validates/tests only on "
            "HfO2/HZO/Hf-Zr-O rows. This flag restores full-material val/test."
        ),
    )
    parser.add_argument(
        "--allow_weak_rows_in_eval", action="store_true",
        help=(
            "Disable quality-aware eval filtering. By default, rows with "
            "data_quality=weak_expanded are train-only and excluded from val/test."
        ),
    )
    parser.add_argument("--tier3_adapter_rank", type=int, default=None, help="v4.56 low-rank structural adapter width")
    parser.add_argument("--tier3_adapter_lr", type=float, default=None, help="v4.56 structural adapter LR")
    parser.add_argument("--tier3_process_lr", type=float, default=None, help="v4.56 process residual LR")
    parser.add_argument("--tier3_adapter_magnitude_weight", type=float, default=None)
    parser.add_argument("--tier3_adapter_center_weight", type=float, default=None)
    parser.add_argument("--tier3_process_center_weight", type=float, default=None)
    parser.add_argument("--tier3_adapter_process_orthogonal_weight", type=float, default=None)
    parser.add_argument("--disable_tier3_structural_adapter", action="store_true")
    parser.add_argument("--tier1_external_manifest", type=str, default=None,
                        help="Frozen CSV/JSON/HDF manifest for tier1_external_eval")
    parser.add_argument("--tier1_external_expected_hash", type=str, default=None,
                        help="Optional expected SHA-256 of the frozen external manifest")
    parser.add_argument("--tier1_external_outdir", type=str,
                        default=str(REPORT_ROOT / "tier1_external_eval"))
    parser.add_argument("--tier1_external_overlap_policy",
                        choices=["fail", "exclude", "report", "off"], default="exclude",
                        help="How to handle structural overlap with Tier-1 training cache")
    parser.add_argument("--tier1_external_batch_size", type=int, default=64)
    parser.add_argument("--tier1_external_num_workers", type=int, default=4)
    parser.add_argument("--tier1_external_n_boot", type=int, default=10000)
    args = parser.parse_args()

    if args.eval_all_tier3_materials:
        TIER3_TRAIN_CONFIG["eval_hfo2_family_only"] = False
    if args.allow_weak_rows_in_eval:
        TIER3_TRAIN_CONFIG["eval_verified_only"] = False
    if args.tier3_adapter_rank is not None:
        TIER3_TRAIN_CONFIG["tier3_structural_adapter_rank"] = int(args.tier3_adapter_rank)
    if args.tier3_adapter_lr is not None:
        TIER3_TRAIN_CONFIG["tier3_structural_adapter_learning_rate"] = float(args.tier3_adapter_lr)
    if args.tier3_process_lr is not None:
        TIER3_TRAIN_CONFIG["tier3_process_learning_rate"] = float(args.tier3_process_lr)
        TIER3_TRAIN_CONFIG["learning_rate"] = float(args.tier3_process_lr)
    for arg_name, cfg_name in [
        ("tier3_adapter_magnitude_weight", "tier3_adapter_magnitude_weight"),
        ("tier3_adapter_center_weight", "tier3_adapter_center_weight"),
        ("tier3_process_center_weight", "tier3_process_center_weight"),
        ("tier3_adapter_process_orthogonal_weight", "tier3_adapter_process_orthogonal_weight"),
    ]:
        val = getattr(args, arg_name)
        if val is not None:
            TIER3_TRAIN_CONFIG[cfg_name] = float(val)
    if args.disable_tier3_structural_adapter:
        TIER3_TRAIN_CONFIG["tier3_use_structural_adapter"] = False
    ...
    if args.mode == "tier3_finetune":
        log.info(
            "v4.56 STRUCTURAL-ADAPTER: enabled=%s rank=%d adapter_lr=%.2e process_lr=%.2e mag=%.3g center=%.3g proc_center=%.3g orth=%.3g",
            TIER3_TRAIN_CONFIG.get("tier3_use_structural_adapter", True),
            TIER3_TRAIN_CONFIG.get("tier3_structural_adapter_rank", 16),
            TIER3_TRAIN_CONFIG.get("tier3_structural_adapter_learning_rate", 5e-5),
            TIER3_TRAIN_CONFIG.get("tier3_process_learning_rate", TIER3_TRAIN_CONFIG["learning_rate"]),
            TIER3_TRAIN_CONFIG.get("tier3_adapter_magnitude_weight", 1.0),
            TIER3_TRAIN_CONFIG.get("tier3_adapter_center_weight", 5.0),
            TIER3_TRAIN_CONFIG.get("tier3_process_center_weight", 5.0),
            TIER3_TRAIN_CONFIG.get("tier3_adapter_process_orthogonal_weight", 1.0),
        )
    ...
    if args.mode == "tier1_external_eval":
        if not args.tier1_external_manifest:
            raise ValueError("--tier1_external_manifest is required for tier1_external_eval")
        ckpt_path = Path(args.weights) if args.weights else (CKPT_ROOT / "tier1_best.pt")
        run_tier1_external_evaluate(
            manifest_path=Path(args.tier1_external_manifest),
            checkpoint_path=ckpt_path,
            output_dir=Path(args.tier1_external_outdir),
            expected_hash=args.tier1_external_expected_hash,
            overlap_policy=args.tier1_external_overlap_policy,
            batch_size=int(args.tier1_external_batch_size),
            num_workers=int(args.tier1_external_num_workers),
            n_boot=int(args.tier1_external_n_boot),
        )
        return
