...
==============================================================================
 ...
 Version 4.5  (Tier 3 readiness fixes -- process column rename, tier3_evaluate,
               J_g task weight correction)
==============================================================================
...
class DatasetExtractor:
    ...
    def load_experimental_process_db(
        ...
        # Row hash for dedup — use substrate_temp_C (renamed) for consistency
        df["row_hash"] = df.apply(
            lambda r: hashlib.md5(
                f"EXP_{r.get('doi','?')}_{r.get('material','?')}_{r.get('substrate_temp_C','?')}".encode()
            ).hexdigest()[:12],
            axis=1,
        )
