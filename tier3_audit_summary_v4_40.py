#!/usr/bin/env python3
"""Standalone Tier-3 process-residual audit summary generator (v4.40).

Reads audit artifacts produced by highk_alignn_train_v4_40_expanded_audit.py
from:
  highk_project/reports/tier3_prediction_audit/normal/
  highk_project/reports/tier3_prediction_audit/ablation/

Produces consolidated CSV and Markdown reports without importing the training
script or requiring CUDA/DGL.
"""
...
def _metrics(df: pd.DataFrame, subgroup: str, mask: pd.Series) -> dict:
    ...
    return {
        ...
        "mean_raw_delta": float(pd.to_numeric(g.get("raw_process_delta", 0), errors="coerce").mean()),
        "mean_abs_bounded_delta": float(pd.to_numeric(g.get("bounded_process_delta", 0), errors="coerce").abs().mean()),
        "max_abs_bounded_delta": float(pd.to_numeric(g.get("bounded_process_delta", 0), errors="coerce").abs().max()),
...
def build_comparison(metrics: pd.DataFrame) -> pd.DataFrame:
    ...
    cols = ["n", "log_mae", "linear_mae", "linear_rmse", "mad_mae", "mean_abs_bounded_delta"]
...
def write_markdown(path: Path, metrics: pd.DataFrame, comparison: pd.DataFrame,
                   history: pd.DataFrame, optimizer: pd.DataFrame,
                   composition: pd.DataFrame) -> None:
    lines: List[str] = ["# Tier-3 Process-Residual Audit Summary", ""]
    ...
    if not history.empty:
        keep = [c for c in ["run_type", "epoch", "val_mae", "process_delta_grad_norm", "process_delta_parameter_norm", "process_delta_parameter_change_from_init", "process_delta_mean_abs", "process_delta_max_abs"] if c in history.columns]
        lines += ["## Training trajectory", "", history[keep].to_markdown(index=False), ""]
    lines += ["## Interpretation guide", "",
              "- Gradient = 0 and parameter change = 0: the residual branch is disconnected, masked, or intentionally ablated.",
              "- Gradient > 0 but parameter change = 0: verify optimizer membership and learning rate.",
              "- Parameter change > 0 but bounded delta ≈ 0: inputs/activations may have collapsed or the final layer remains near zero.",
              "- Non-zero delta with no test improvement: process variables are not generalizing on the fixed test split.",
              "- Check `experimental_proc_available` before judging the aggregate `all` subgroup.", ""]
    path.write_text("\n".join(lines))
...
def main() -> int:
    ...
    for run_type in ("normal", "ablation"):
        ...
        for path in sorted(run_dir.glob("tier3_*_(test|val)_prediction_audit.csv")):
            # pathlib glob has no alternation; retained for readability, second glob below does the work.
            pass
        ...
        h = load_history(root, run_type)
        if not h.empty:
            history_frames.append(h)
        o = optimizer_summary(run_dir, run_type)
        if not o.empty:
            opt_frames.append(o)
        c = composition_summary(run_dir, run_type)
        if not c.empty:
    ...
    write_markdown(out / "tier3_audit_summary.md", metrics, comparison, history, optimizer, composition)

    print(f"Audit summary written to: {out}")
    if metrics.empty:
        print("WARNING: no prediction-audit CSV files found. Run normal and ablation Tier-3 flows first.")
        return 2
    return 0
